...
```
omisu bar set omisu.clock format "dddd h:mm AP"
```
...
Run _Update > Timezone_ in the OmiSu menu and pick from the list. If the timezone is right but the clock itself has drifted, _Update > Time_ restarts the time synchronization for you.
...
_Trigger > Speed Test > Disk Speed Test_ measures live read and write speed on your drive, or `omisu disk speedtest` from the terminal.

### Why can't I sign into my Google account in Chromium?

The plain open source Chromium build doesn't ship with the OAuth credentials that Google requires for account sign-in. Run _Install > Service > Chromium Account_ in the OmiSu menu to add them, restart the browser, and the sign-in will go through.
...
```
export OMISU_SCREENSHOT_DIR="$HOME/Pictures/Screenshots"
```

You can do the same for screenrecordings using `OMISU_SCREENRECORD_DIR`.

Just remember to create the directoy you want to save to and restart OmiSu for this to take effect.
...
Remember that you have built-in brightness control in OmiSu for the Apple Displays (both Studio and XDR) using the regular keyboard brightness buttons.
...
And you can use _Remove > Web App_ from the OmiSu menu to remove any of the preinstalled web apps you don't want.
