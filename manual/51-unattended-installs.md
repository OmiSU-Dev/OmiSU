...
The OmiSu ISO can install itself with nobody at the keyboard. If the installer finds a second drive labeled `cidata` carrying its configuration files, it copies them off, skips the setup wizard entirely, and reboots into the finished system on its own. No special ISO build, no extra boot menu entry — with no such drive attached, nothing changes and you get the normal wizard.

This makes OmiSu great as a base image for disposable dev environments: create a VM in Proxmox or with Packer, boot it, walk away, SSH in. `cidata` is the cloud-init `NoCloud` label, so all the common virtualization tooling already knows how to attach such a drive.
...
Then attach it to the VM alongside the OmiSu ISO. Here's a full Proxmox example:

```bash
qm create 101 --name my-omisu \
  ...
  --ide2 local:iso/omisu.iso,media=cdrom \
