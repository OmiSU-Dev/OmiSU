...
| Hotkey                  | Function              |
| ----------------------- | --------------------- |
| `Super + Space`           | OmiSu menu (apps and everything else)    |
...
Usually on Linux, you need `Ctrl + Shift + C/V` to copy'n'paste in the terminal and `Ctrl + C/V` to do it everywhere else. These OmiSu unified clipboard hotkeys work everywhere.
...
All capture options are also accessible under _Trigger > Capture_ in the OmiSu menu (`Super + Space`).
...
Extra background images live in `~/.config/omisu/backgrounds/<theme name>`. Also available via _Install > Style > Background_ in the OmiSu menu.

All style options are also accessible under _Style_ in the OmiSu menu (`Super + Space`).
...
Ghostty terminal is installed using _Install > Terminal_ via the OmiSu menu.
...
You can add more of your own by editing `~/.XCompose`, then running `omisu-restart-xcompose` in the terminal to get the changes picked up.
