...
In addition to the standard Linux tools, OmiSu also ships with a bunch of enhanced shell tools. Here are the key ones.
...
[Zoxide](https://github.com/ajeetdsouza/zoxide) is a replacement for cd. It remembers the directories you've been in, so you can more easily jump to them next time. Say you do `cd ~/.config/omisu` once. Next time, you can just do `cd omisu` (or even just `cd oma`), and Zoxide will take you directly there.
