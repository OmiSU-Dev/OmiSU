...
OmiSu allows you to set your company logo or personal image for both the boot unlock, the screensaver, and the about screen.

### Boot unlock

You can use `omisu plymouth preview` to see what your custom logo and colors would look like. It takes a background color, a text color, a logo png, and a path for the preview image:

```
omisu plymouth preview '#1d2021' '#ebdbb2' logo.png preview.png
```

Then apply the setup with `omisu plymouth set '#1d2021' '#ebdbb2' logo.png`, which will also give the SDDM login screen the same colors and logo. If you want to revert, you can use `omisu plymouth reset`.
...
- **Edit Text** opens `~/.config/omisu/branding/screensaver.txt` in your editor. Type or paste whatever you like — ASCII art, your name, a rude word. Save and quit, and the screensaver fires up immediately so you can see it.
...
- **Restore Default** puts the OmiSu logo back.

### About screen

The same three options are under _Style > About_ for the _About_ screen you get from the OmiSu menu, and they work identically — the file is `~/.config/omisu/branding/about.txt`, and the About window pops up after each change. The About art is converted to a smaller size than the screensaver's, since it has to fit in a window rather than fill your display.
...
Both of the _Set From Image_ options are just calling `omisu transcode ascii`, which you can run directly if you want control over the conversion:

```
omisu transcode ascii ~/logo.svg ~/.config/omisu/branding/screensaver.txt --width 100
```
...
`omisu ascii` draws text in Delta Corps Priest 1, the FIGlet font the OmiSu wordmark itself is drawn in, so a screensaver can say something rather than show a picture:

```
omisu ascii "Back in five" > ~/.config/omisu/branding/screensaver.txt
```
