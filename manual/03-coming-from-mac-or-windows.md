...
If you've spent years on macOS or Windows, your fingers know a hundred things your brain has forgotten it ever learned. This chapter is the translation layer: where those instincts go in OmiSu. The features themselves are covered in depth elsewhere — this is just the map.

### Super is the center of everything

All the muscle memory you've built around Cmd or the Windows key transfers to one key: Super. That's the Windows key on a PC keyboard, and it's the anchor for nearly every hotkey in OmiSu.

Your Spotlight or Raycast or Start-menu reflex becomes `Super + Space`. That opens the OmiSu menu, which launches apps, changes settings, installs software, captures the screen — just about everything. Start typing to filter. There's also a dedicated apps-only menu on `Super + Alt + Space`. See [navigation](04-navigation.md).
...
On the Mac you had Cmd + C everywhere. On Windows you had Ctrl + C everywhere — except the terminal, where it kills your program. OmiSu gives you `Super + C`, `Super + X`, and `Super + V`, and they work everywhere, including the terminal. No separate reflex to learn for the shell.
...
| You reach for | In OmiSu |
| ------------- | ---------- |
| Spotlight / Raycast / Start menu | `Super + Space` — the OmiSu menu |
...
| App Store / downloading an installer | _Install_ in the menu, or `omisu pkg add` — see [other packages](29-other-packages.md) |
...
Updates come through one command — _Update > OmiSu_ — that updates OmiSu itself and every package on the system, taking a snapshot first. No per-app updaters nagging you at random. See [updates](30-updates.md).
...
OmiSu runs well on Intel Macs — see [Mac support](44-mac-support.md). And the keyboard is kind to you: OmiSu doesn't remap anything, and Linux treats the Command key as Super, so Super sits right where Cmd always was. Your thumb won't notice the move.
