...
OmiSu tackles both problems with unified clipboard hotkeys that work (almost) everywhere. They are:
...
The clipboard history is provided by the OmiSu shell and works for both text and images. You trigger it by `Super + Ctrl + V`, select your entry with return, and then that'll be placed on the clipboard ready to paste on `Super + V`.
