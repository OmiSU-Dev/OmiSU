...
Files land in `~/Pictures` by default, named `screenshot-2026-08-13_14-22-05.png`. If you'd rather keep them in their own folder, set `OMISU_SCREENSHOT_DIR` — see [the FAQ](46-faq.md) for where to put session environment variables. OmiSu creates the directory for you if it isn't there. You can swap the editor too with `OMISU_SCREENSHOT_EDITOR`.

From the terminal, `omisu screenshot` takes the same shot, and you can be explicit about it: `omisu capture screenshot region` for freeform only, `windows` to snap to window and monitor rectangles, or `fullscreen` to skip the picker entirely and grab the focused monitor. A second argument of `copy` puts the shot only on the clipboard, and `save` only on disk.
...
You can also call it directly with `omisu-capture-webcam-resize small`, or `reset` to go back to medium.
...
It works from the terminal too, if you already know what you want: `omisu transcode ~/Videos/demo.mov mp4 1080p`. There's also `omisu transcode ascii`, which turns an image into ASCII art — that one's mostly for [branding](41-branding.md).
