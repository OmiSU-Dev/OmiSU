...
You start Obsidian with `Super + Shift + O`. To use theme syncing, you must select the `OmiSu` theme under settings.

## Omawrite

[Omawrite](https://github.com/omacom-io/omawrite) is OmiSu's own dead-simple Markdown writing app. No vaults, no plugins, just you and the words.
...
You can open the Share menu on `Super + Ctrl + S` or under _Trigger > Share_ in the OmiSu menu. It gives you four options:
...
The same thing works from the terminal with `omisu share clipboard`, `omisu share file [path]`, and `omisu share folder [path]`. Leave the path off and you get the picker.

You can also send straight from the file manager: right-click any selection in Nautilus and pick _Send via LocalSend_.

OmiSu's firewall is closed by default except for LocalSend's port, so this works out of the box on a fresh install. See [security](48-security.md).
...
[Omacalc](https://github.com/omacom-io/omacalc) is OmiSu's own dead-simple calculator, which opens in a floating window.
...
You start Signal with `Super + Shift + G`. It's not part of the base install, so the first time you hit that, OmiSu will offer to install it for you (it's also under _Install > Service_ in the OmiSu menu).
...
[OBS Studio](https://obsproject.com/) lets you record or stream video from multiple inputs. You can mix a screencast with a webcam with a microphone input. It's what was used to record the OmiSu screencasts.
...
[Omacut](https://github.com/omacom-io/omacut) is OmiSu's own dead-simple video trimmer. When all you need is to cut the start and end off a clip, it beats firing up a full video editor.
