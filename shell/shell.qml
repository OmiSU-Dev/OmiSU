...
ShellRoot {
  ...
  // The omisu-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. OMISU_PATH is provided by the uwsm
  // session environment and is the single source of truth for this checkout.
  property string omisuPath: Quickshell.env("OMISU_PATH")
  readonly property string shellPath: omisuPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: omisuPath + "/config/omisu/shell.json"
  readonly property string userConfigPath: home + "/.config/omisu/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        ...
        center: [{ id: "omisu.clock", format: "dddd HH:mm" }],
  ...
  Component.onCompleted: {
    console.log("omisu-shell paths",
      "omisuPath=" + shell.omisuPath,
  ...
  readonly property string defaultBarId: "omisu.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("omisuPath" in target) target.omisuPath = shell.omisuPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("omisuPath" in inst) inst.omisuPath = shell.omisuPath
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. omisu.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("omisuPath" in item) item.omisuPath = shell.omisuPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["omisu.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("omisu.image-picker", payload) ? "ok" : "unknown"
    }
  ...
  IpcHandler {
    ...
    function listPlugins(): string {
      ...
      for (var id in plugins) {
        ...
        var metadata = plugins[id].omisu
        var clonedFrom = Util.isPlainObject(metadata) ? String(metadata.clonedFrom || "") : ""
        out.push({
          id: id,
          name: plugins[id].name,
          kinds: kinds,
          // What `omisu plugin enable/disable` toggles: for a widget that is
