...
QtObject {
  ...
  property string pluginsDir: home + "/.config/omisu/plugins"
  ...
  //   - the built-in bar option (`omisu.bar`) is active when `bar.id` is
  //     missing or set to `omisu.bar`.
  ...
  //     silently make `omisu launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "omisu.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.omisu) ? manifest.omisu : null
  ...
  // back, and so a plugin that is both a widget and a menu (omisu.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "omisu.workspaces", center: "omisu.weather", right: "omisu.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.omisu)
        ? candidateManifest.omisu : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "omisu.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "omisu.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `omisu.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("omisu.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party OmiSu plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/omisu/plugins.
    var script = ""
