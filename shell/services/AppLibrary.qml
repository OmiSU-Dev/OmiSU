...
Item {
  ...
  function remove(desktopId, name) {
    var id = String(desktopId || "")
    if (!id) return
    Util.execDetached(Util.shellQuote(root.omisuPath + "/bin/omisu-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
  }
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = root.omisuPath + "/shell/services/hidden-entries.sh"
  ...
  function closeLaunchFeedback(serial) {
    ...
    if (root.launchOsdOpen) {
      Quickshell.execDetached(["omisu-shell", "osd", "close"])
  ...
  FileView {
    path: root.omisuPath + "/default/omisu/launcher.hides"
    watchChanges: true
    printErrors: false
  ...
  Timer {
    ...
    onTriggered: {
      ...
      Quickshell.execDetached(["omisu-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: root.launchOsdMessage, duration: 0 })])
    }
  }
