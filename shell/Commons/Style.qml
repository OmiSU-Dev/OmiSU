...
QtObject {
  ...
  // component follows the fontconfig alias `omisu-font-set` writes.
  ...
  // alias path keeps working when the user runs `omisu font set`), but
  ...
  // OMISU_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("OMISU_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `omisu font
  ...
  // `omisu toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/omisu/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
