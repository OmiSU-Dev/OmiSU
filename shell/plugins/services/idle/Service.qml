...
Item {
  ...
  // Injected by omisu-shell (the first-party service loader).
  property var shell: null

  readonly property string home: Quickshell.env("HOME")
  readonly property string stayAwakeStateDir: home + "/.local/state/omisu/indicators"
  ...
  readonly property string screensaverClass: "org.omisu.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("omisu idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(omisu-shell lock isLocked 2>/dev/null) == \"true\" ]] || omisu-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "omisu-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "omisu-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/omisu/indicators\" && touch \"$HOME/.local/state/omisu/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/omisu/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/omisu/indicators\"; if [[ -f $HOME/.local/state/omisu/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
