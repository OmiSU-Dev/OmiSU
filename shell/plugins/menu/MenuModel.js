...
// against `omisu-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "omisu-channel-current",
  "omisu-default-agent",
  "omisu-default-browser",
  "omisu-default-editor",
  "omisu-default-terminal",
  "omisu-dns"
]
...
function guardHelpers() {
  return 'declare -A __omisu_pkgs=()\n'
    + 'mapfile -t __omisu_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __omisu_pkg in "${__omisu_pkg_names[@]}"; do __omisu_pkgs[$__omisu_pkg]=1; done\n'
    + '__omisu_pkg_has() { [[ -n ${__omisu_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'omisu-pkg-present() { local p; for p in "$@"; do __omisu_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'omisu-pkg-missing() { local p; for p in "$@"; do __omisu_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'omisu-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'omisu-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__omisu_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
