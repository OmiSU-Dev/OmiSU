...
Item {
  ...
  // Injected by omisu-shell when this plugin is summoned.
  property string omisuPath: Quickshell.env("OMISU_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `omisu-shell shell summon omisu.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: omisuPath + "/default/omisu/omisu-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/omisu/extensions/omisu-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(omisu-font-current 2>/dev/null); omisu-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "omisu-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); omisu-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "omisu-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `omisu-shell shell summon omisu.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `omisu menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/omisu/extensions/omisu-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omisu-menu"
