...
Item {
  ...
  property string omisuPath: ""

  readonly property string home: Quickshell.env("HOME")
  readonly property string stateHome: home + "/.local/state"
  readonly property string userName: Quickshell.env("USER") || Quickshell.env("LOGNAME")
  readonly property string currentBackgroundLink: stateHome + "/omisu/current/background"
  ...
  function logEvent(event) {
    ...
    console.log("omisu lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omisu-lock-preview"
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/omisu-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    ...
    command: ["bash", "-c", "omisu-hyprland-session-locked"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "omisu-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "omisu-brightness-keyboard off; omisu-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/omisu-lock-password"
    watchChanges: true
    printErrors: false
