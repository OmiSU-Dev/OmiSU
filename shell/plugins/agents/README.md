...
`omisu-agent-usage-update` writes to `~/.local/state/omisu/agents/usage/`
...
`omisu plugin disable omisu.agents`.

## Data

Each agent is one JSON record in `~/.local/state/omisu/agents/usage/`,
written by `omisu-agent-usage-update`. That command runs one
`omisu-agent-usage-<agent>` collector per agent; the widget invokes it
...
configuration in `~/.config/omisu/agents/fireworks.json`:
...
- IPC: `omisu-shell omisu.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/omisu/shell.json`. The
top-level keys can be set with
`omisu bar set omisu.agents <key> <value>`:
...
```bash
omisu bar set omisu.agents refreshIntervalSec 300 --json
omisu bar set omisu.agents syncDir '~/Sync/agent-usage'
```
...
```bash
omisu bar set omisu.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
