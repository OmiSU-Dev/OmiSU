...
// omisu-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  readonly property string usageDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omisu/agents/usage"
  ...
  function updateCommand(kind, agentIds) {
    var command = ["omisu-agent-usage-update"]
  ...
  readonly property string syncSnapshotPath: syncConfigured() ? syncEffectiveDir + "/" + syncEffectiveFileName : home + "/.cache/omisu/agents-disabled.json"
  ...
  // Snapshots keep the field names older OmiSu versions wrote, so a fleet
