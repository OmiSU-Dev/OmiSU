...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "omisu.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    Quickshell.execDetached([root.omisuPath + "/bin/omisu-menu-emoji-insert", emoji])
  }

  ListModel { id: displayModel }

  FileView {
    path: root.omisuPath + "/shell/plugins/emojis/emojis.json"
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "omisu-emojis"
