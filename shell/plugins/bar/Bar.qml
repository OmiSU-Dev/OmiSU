...
Item {
  ...
  // The omisu-shell host injects omisuPath from OMISU_PATH.
  required property string omisuPath
  ...
  property string omisuConfigDir: home + "/.config/omisu"
  ...
  // to "monospace"), so changing the system font (via `omisu-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "omisu.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, omisuConfigDir)
  ...
  FileView {
    path: root.stateHome + "/omisu/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `omisu-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/omisu/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/omisu/toggles"
    watchChanges: true
    printErrors: false
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "omisu-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "omisu-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "omisu-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
