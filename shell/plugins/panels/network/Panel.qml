...
Panel {
  id: root
  moduleName: "omisu.network"
  ipcTarget: "omisu.network"
  ...
  // `omisu-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `omisu-network-band`. `bandCurrent` is the band
  ...
  // The share card is its own panel plugin (omisu.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("omisu.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["omisu-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["omisu-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (omisu.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("omisu.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "omisu-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "omisu-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["omisu-network-status", "--verbose"]
