# Tailscale OmiSu Widget

Native OmiSu bar widget for Tailscale.
...
`omisu-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `omisu tailscale receive`
...
This widget ships as first-party plugin `omisu.tailscale`. Add it with `omisu plugin enable omisu.tailscale`, then place it with `omisu bar move omisu.tailscale` if desired.
