...
Panel {
  id: root
  moduleName: "omisu.monitor"
  ipcTarget: "omisu.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (omisu-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["omisu-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("omisu.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "omisu-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["omisu-display-text-size", String(px)]
  ...
  Process {
    ...
    // `omisu-brightness-display` races the hardware/driver and can
