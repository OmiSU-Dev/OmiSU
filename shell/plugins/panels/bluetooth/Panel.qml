...
Panel {
  id: root
  moduleName: "omisu.bluetooth"
  ipcTarget: "omisu.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/omisu-bluetooth-device;
  ...
  function setDefaultAudioSink(sink) {
    ...
    if (sink.id !== undefined && sink.name) {
      Quickshell.execDetached([
        "omisu-audio-output-set-default",
        String(sink.id),
        String(sink.name)
      ])
    }
  ...
  function deviceCommand(action, address) {
    return ["omisu-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via omisu-bluetooth-device.
  ...
  // the adapter came back on at the next boot. omisu-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["omisu-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "omisu.bluetooth"
