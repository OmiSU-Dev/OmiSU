...
Panel {
  id: root
  moduleName: "omisu.power"
  ipcTarget: "omisu.power"
  ...
  function setProfile(profile) {
    ...
    actionProc.command = ["omisu-powerprofiles-set", root.discharging ? "battery" : "ac", profile]
  ...
  Process {
    id: batteryProc
    command: ["omisu-battery-status", "--shell"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateKeyValue(text, "battery") }
  }

  Process {
    id: profilesProc
    command: ["omisu-powerprofiles-list", "--active-state"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateProfiles(text) }
  }

  Process {
    id: systemProc
    command: ["omisu-system-stats"]
