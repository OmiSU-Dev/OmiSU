...
Panel {
  id: root
  moduleName: "omisu.weather"
  ipcTarget: "omisu.weather"
  manageIpc: false
  ...
  // omisu-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/omisu/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["omisu-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["omisu-weather-location", "--set", name]
    else
      locationSaveProc.command = ["omisu-weather-location", "--clear"]
  ...
  // Mirrors omisu-weather-icon's wttr.in code → nerd-font glyph mapping.
