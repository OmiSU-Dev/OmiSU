...
Panel {
  id: root
  moduleName: "omisu.audio"
  ipcTarget: "omisu.audio"

  readonly property var sink: Pipewire.defaultAudioSink
  readonly property var source: Pipewire.defaultAudioSource
  readonly property var nodes: Pipewire.nodes ? Pipewire.nodes.values : []
  readonly property var mprisPlayers: Mpris.players ? Mpris.players.values : []
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("omisu.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("omisu_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // omisu-audio-output-sink resolves the *current* default output through any
  ...
  Process {
    ...
    command: ["omisu-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["omisu-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
