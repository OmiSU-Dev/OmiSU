// Notification service for the omisu shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/omisu/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the omisu bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "omisu-notifications"
  ...
  // and rare.
  //   - omisu-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always omisu or system shell scripts —
  ...
  //   - app_name is "omisu-action" (OmiSu's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. OmiSu's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.omisuPath + "/bin/omisu-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `omisu-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "omisu-notifications"
