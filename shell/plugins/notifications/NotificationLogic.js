...
function glyphFromHints(hints) {
  return stringHint(hints, "omisu-glyph")
}

// The click action: a JSON argv string from omisu-notification-send
...
function execArgvFromHints(hints) {
  return stringHint(hints, "omisu-exec-argv")
}

// Validate a persisted omisu-exec-argv into a runnable argv, or null. This is
...
// ~/.local/state/omisu/notifications/ so toasts survive shell restarts
// (e.g. the restart `omisu-update` performs). The file exists exactly as
...
// senders (all OmiSu web apps) delete their scoped /tmp files on close,
