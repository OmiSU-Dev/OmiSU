...
`omisu-shell` is a single long-running [Quickshell](https://quickshell.org/)
instance that hosts the OmiSu desktop. Hyprland autostart launches one shell
...
| `bar`        | A full bar option that can replace the built-in `omisu.bar` |

Only one `bar` plugin is active at a time. Missing or invalid selections fall
back to the built-in `omisu.bar`, so users always have a safe path home.
...
clones it straight into `~/.config/omisu/plugins/<id>/` (named by the
manifest id); updating is a fast-forward pull of that checkout.

```bash
omisu plugin add https://github.com/acme/omisu-weather.git
omisu plugin update acme.weather       # fetches, shows a diff, fast-forwards
omisu plugin update                    # updates every git-managed plugin
omisu plugin remove acme.weather
```

> ⚠️ **Plugins run as unsandboxed code inside `omisu-shell`.** Adding warns
...
```bash
omisu plugin add https://github.com/acme/omisu-weather.git --enable --yes
omisu plugin update --yes
```
...
1. Put it in `~/.config/omisu/plugins/<plugin-id>/` with a `manifest.json`
   plus the QML referenced from its `entryPoints`.
2. `omisu-shell shell rescanPlugins`.
3. `omisu plugin enable <id>`. Bar widgets start in
   `barWidget.defaultSection`, or in the center when it is omitted, and can be
   moved with `omisu bar move`; a full bar replaces the one in use.

The lower-level IPC equivalents remain available via `omisu-shell shell rescanPlugins`,
`omisu-shell shell enablePlugin <id> '{}'`, and `omisu-shell shell listPlugins`.
The `omisu plugin` commands wrap those calls. `omisu bar move` and
`omisu bar set` edit the persisted widget layout in `shell.json`.
...
automatically; `omisu-shell shell rescanPlugins` remains available to force a reload.
...
hooks, the image picker's `image-selector` target). `omisu-menu` uses the
shell target to summon the first-party `omisu.menu` plugin instead of
...
| `reloadConfig`                           | `ok`    | reload `~/.config/omisu/shell.json`                 |
...
```
quickshell ipc -p $OMISU_PATH/shell call shell ping
```

Hyprland autostart launches the shell directly with `quickshell -p
$OMISU_PATH/shell`. Use `omisu-restart-shell` to stop every running
instance of that config and launch one fresh shell process.

A convenience wrapper, [`omisu-shell`](../bin/omisu-shell), forwards IPC
calls to the running shell. It does not start the shell.

```
omisu-shell shell ping
omisu-shell shell toggle omisu.menu '{"menu":"root"}'
omisu-shell shell listPlugins
...
| Path                              | Owner          | Purpose                                                |
|-----------------------------------|----------------|--------------------------------------------------------|
| `~/.config/omisu/shell.json`    | the shell      | full layout + per-entry settings + enabled plugin list |
| `~/.config/omisu/plugins/<id>/` | user           | drop-in third-party plugin source files                |

The `config/omisu/shell.json` default config describes the
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left":   [ { "id": "omisu.menu" }, { "id": "omisu.workspaces" } ],
      "center": [ { "id": "omisu.clock", "format": "HH:mm" } ],
      "right": [
        { "id": "omisu.audio" }
      ]
    }
...
1. **The active bar option is `bar.id`.** Omit it or set it to `omisu.bar`
...
4. **Built-in widget ids are namespaced.** Use ids such as `omisu.clock`,
   `omisu.audio`, and `omisu.network`. The migration rewrites older ids
...
6. **Multiple instances** are allowed when a manifest sets
   ...
   widgets in different timezones are just two `{"id":"omisu.clock", "timezone": ...}`
...
- Phase 1 — `omisu-shell phase 1: host the existing bar in a single shell`
- Phase 2 — `omisu-shell phase 2: plugin registry and bar widget registry`
- Phase 3 — `omisu-shell phase 3: fold bar-settings into the shell as a panel plugin`
- Phase 4 — `omisu-shell phase 4: absorb background-switcher as a plugin`
- Phase 5 — `omisu-shell phase 5: docs, cleanup, and migration crumbs`
- Phase 6 — `omisu-shell phase 6: reviewer cleanup (path traversal, collision, races)`
- Phase 7 — `omisu-shell phase 7: replace socket with IpcHandler, rename to image-picker`
- Phase 8a — `omisu-shell phase 8a: unified shell.json with inline plugin settings`
