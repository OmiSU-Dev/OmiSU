...
class MainActivity: MultiDisplayFlutterActivity(), GamepadsCompatibleActivity, KeyEventForwardingActivity {
    override var embeddedKeyListener: ((KeyEvent) -> Boolean)? = null
    override var embeddedMotionListener: ((android.view.MotionEvent) -> Boolean)? = null
    private val CHANNEL = "com.omisu.launcher/game"
    ...
    private val LAUNCHER_CHANNEL = "com.omisu.launcher/launcher"
    private val LAUNCHER_EVENT_CHANNEL = "com.omisu.launcher/launcher_events"
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        flutterEngine.plugins.add(EmbeddedEmulatorPlugin())
        ...
        secondaryDisplayChannel = MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "com.omisu.launcher/secondary_display")
        ...
        launcherMethodChannel?.setMethodCallHandler { call, result ->
            when (call.method) {
                "setNordiSafeMode" -> {
                    ...
                    getSharedPreferences("nordi_oem", MODE_PRIVATE)
                        .edit()
                        .putBoolean("safe_mode", enabled)
                        .apply()
                    result.success(true)
                }
                "isDefaultLauncher" -> {
                ...
                "openAndroidSystemUpdates" -> {
                    openAndroidSystemUpdates(result)
                }
                "rebootDevice" -> {
                    rebootDevice(result)
                }
    ...
    private fun isScreenshotAccessEnabled(): Boolean {
        ...
        // namespace (com.omisu.launcher.*) while the package picks up the
    ...
    private fun openAndroidSystemUpdates(result: MethodChannel.Result) {
        try {
            ...
            android.util.Log.e("MainActivity", "openAndroidSystemUpdates: ${e.message}")
            try {
                openSystemSettings()
                result.success(true)
            } catch (fallback: Exception) {
                result.error("OPEN_FAILED", fallback.message, null)
            }
        }
    }

    private fun rebootDevice(result: MethodChannel.Result) {
        try {
            val pm = getSystemService(android.content.Context.POWER_SERVICE) as android.os.PowerManager
            pm.reboot(null)
            result.success(true)
        } catch (e: Exception) {
            android.util.Log.e("MainActivity", "rebootDevice: ${e.message}")
    ...
    private fun shutdownDevice(result: MethodChannel.Result) {
        try {
            ...
            android.util.Log.e("MainActivity", "shutdownDevice: ${e.message}")
    ...
    private fun restoreNordiDisplayFromSleep() {
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                val wakeUp = android.os.PowerManager::class.java.getMethod(
                    "wakeUp",
                    Long::class.javaPrimitiveType,
                )
                wakeUp.invoke(pm, SystemClock.uptimeMillis())
            }
            val userActivity = android.os.PowerManager::class.java.getMethod(
                "userActivity",
                Long::class.javaPrimitiveType,
            ...
            Log.w("MainActivity", "restoreNordiDisplayFromSleep: ${e.message}")
        }
    }
    ...
    override fun dispatchKeyEvent(keyEvent: KeyEvent): Boolean {
        ...
        if (nordiDeviceSleepActive) {
            if (keyEvent.action == KeyEvent.ACTION_DOWN &&
                isNordiSleepWakeKey(keyEvent.keyCode)
            ) {
                wakeFromNordiSleep()
                return true
            }
            // Ignore other keys until Start/Menu wakes (avoids stray UI input if the
            // panel turns on without our wake handler running first).
            return true
        }

        if (gamepadBlocked) return true
        embeddedKeyListener?.invoke(keyEvent)?.takeIf { it }?.let { return true }
