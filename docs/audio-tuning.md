...
`on` renders the graph into `~/.config/pipewire/omisu-speaker-tuning.conf.d/`
(plus the shared host config `omisu-speaker-tuning.conf`, from
`default/audio/filter-chain-host.conf`) and runs it as its own PipeWire client via
`omisu-speaker-tuning.service`, rather than
...
```bash
omisu audio tuning on        # install the matching tuning
omisu audio tuning off       # remove it, back to raw speakers
omisu audio tuning status    # installed? in use? what matches?
```
...
node is named `omisu_speaker_tuning` (the install checks for that exact name,
...
tools, in [omisu-audio-tuner](https://github.com/omacom-io/omisu-audio-tuner).
It is not installed by default — almost nobody authoring a tuning, and it needs
python, ffmpeg and mpv.

```bash
omisu pkg add omisu-audio-tuner
```
...
- **Volume lives downstream of the tuning.** A tuning is a virtual sink that
  ...
  `omisu-audio-output-sink` is the single definition of "which sink does this
...
- **The fronted sink is hidden.** The tuning and the physical speakers both exist
  ...
  `omisu-audio-sink-availability` reports it unavailable and
  `omisu-audio-output-switch` skips it, leaving one speaker entry in the panel.
...
- **The tuning's own output must not be moved.** A filter-chain's output is a
  ...
  `omisu-audio-output-set-default` moves only streams that carry an
  `application.name`.
