...
- **`./test/cli`** — one big script, one suite. It owns the CLI router: help
  ...
  the metadata lint — every `omisu-*` executable under `bin/` is checked for a
  `# omisu:summary=` header and against removed or redundant fields — plus
  the theme pipeline: template rendering (`omisu-theme-set-templates`,
  `omisu-theme-color`, `omisu-theme-osc`), the theme sync commands
...
directory or an installed OmiSu.
...
- **Fake `$HOME`, real `$OMISU_PATH`.** Anything touching user state runs
  with `HOME` pointed at a `mktemp -d` directory (cleaned up via
  `trap ... EXIT`) and `OMISU_PATH="$ROOT"`, so tests exercise the checkout
