...
that hosts the OmiSu desktop. The bar, panels, overlays, menus, and
services all run inside as plugins. Hyprland autostart launches the
shell via `omisu-launch-shell`; restart it with `omisu-restart-shell`.
IPC is the canonical way for CLIs to talk to a running shell —
`omisu-shell` forwards a call and fails when the shell is not running
(`-q` makes it quiet best-effort; `OMISU_SHELL_IPC_TIMEOUT` bounds the
wait).
...
| `bar`        | Full bar option that can replace `omisu.bar` |
...
Only one full bar option is active at a time. The built-in `omisu.bar` is
...
`omisuPath`, `shell`, `manifest`, and the registries (`pluginRegistry` /
...
clones it straight into `~/.config/omisu/plugins/<id>/`; updating is a
fast-forward pull:

```bash
omisu plugin add https://github.com/acme/omisu-weather.git
omisu plugin update                # fetches, shows a diff, fast-forwards
...
Cloning `omisu.clock`, for example, creates and switches to
`~/.config/omisu/plugins/<username>.clock/` (e.g. `dhh.clock`), names it
...
Plugins run as **unsandboxed code** inside `omisu-shell`. Adding warns you
before cloning, plugins land disabled so you can review the code before
`omisu plugin enable`, and updates show a diff before touching anything.
...
`~/.config/omisu/plugins/<id>/`, run `omisu-shell shell rescanPlugins`, then
`omisu plugin enable <id>`. A bar widget starts in its declared default
section; enabling a full bar replaces the one in use. `omisu bar` drives the
bar from the CLI — `use | reset | defaults | position | transparent | put |
move | set`, with placement flags such as `--section` and `--index`.
The lower-level IPC methods remain available through `omisu-shell shell ...`.
...
`media`, `notifications`, and per-widget targets such as `omisu.clock`
or `omisu.power`. There is no `bar` target.
...
| `putBarWidget <id> <placementJson>`   | place a widget only where absent (`omisu bar put`) |
...
1. The active bar option is `bar.id`. Omit it or set it to `omisu.bar` for
...
3. Settings are inline on the entry. No `config:` sub-object, no
   merge layers.
4. Built-in bar widget ids are namespaced (`omisu.clock`, `omisu.audio`, …).
...
`config/omisu/shell.json` describes the fresh-install state. When no
...
machine-level `~/.config/omisu/shell.toml` is watched live by the
shell and its keys win over the active theme's `shell.toml`, so
overrides like `omisu display text size` survive theme switches.
...
`omisu-theme-set-templates` (see [`theming.md`](theming.md)).
...
themes don't set it, the user does via `omisu font set <name>`.
...
```json
{ "id": "vpn", "type": "command", "exec": "~/.config/omisu/bar/scripts/vpn-status",
...
Then `~/.config/omisu/bar/modules/gpu.qml` (or set `source` to point
