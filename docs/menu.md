...
The menu is the `omisu.menu` plugin of the Quickshell desktop, with its
content defined as data in `default/omisu/omisu-menu.jsonc` (read at
runtime from `$OMISU_PATH`) and overlaid by the user's
`~/.config/omisu/extensions/omisu-menu.jsonc`. The shell parses both files
...
| `iconFont` | Font family for the glyph when it differs from the menu font — how the private `omisu` font's brand glyphs render |
...
| `aliases` | Alternate `omisu menu summon <name>` routes; also searchable |
...
The sample extension at `config/omisu/extensions/omisu-menu.jsonc`
...
- Package and command presence (`omisu-pkg-present` and friends) are
...
- Commands that several rows read a value from — every Defaults > Browser row
  compares against `$(omisu-default-browser)` — run once, with the captured
  answer substituted into each expression. The reader list is
  `GUARD_READERS` in `MenuModel.js`; a new `$(omisu-...)` reader used by
...
- `disabled` keeps the row listed but dims it, marks it ✓, and makes it
  ...
  list stays a catalog of what OmiSu can install. Since a dimmed row means
...
`bin/omisu-menu` is a thin wrapper over the standard plugin IPC surface:

```bash
omisu menu                    # toggle the root menu
omisu menu toggle system      # open at a route, or close if already open
omisu menu summon style.theme # always open (no close-if-visible)
omisu menu close
omisu menu refresh            # re-parse the JSONC files
omisu menu ping
```
...
The same plugin doubles as the system's dmenu. `omisu-menu-select` and
`omisu-menu-input` summon it with a `mode: select` or `mode: input`
...
how the pickers behind menu actions (`omisu-menu-plugin`,
`omisu-menu-timezone`, ...) present lists without owning any UI.
