...
`~/.local/state/omisu/notifications/` (one JSON line per file, named
`<timestamp>-<id>.json`), so live toasts survive the shell restart that
`omisu-update` performs. When a toast leaves the screen — expiry, dismissal,
...
`~/.local/state/omisu/notifications.json` and toggled via shell IPC
(`omisu-shell notifications toggleDnd` / `setDnd` / `dndState`).
`omisu-toggle-notification-silencing` wraps the toggle and refreshes the
bar's `omisu.indicators` widget, whose Dnd indicator binds directly to the
service's `doNotDisturb` property.

Two kinds of notification punch through DND, chosen to be intentional and
rare:

- `app_name` = `omisu-action` — OmiSu's own user-action confirmation
...
`notify-send`/`omisu-action`) are dropped entirely.

## The sender contract

`bin/omisu-notification-send` is the one way OmiSu code sends
...
|---|---|---|
| `-g` / `--glyph` | hint `omisu-glyph` | Nerd Font glyph for the icon slot when no image icon resolves |
| `--exec <program> [args…]` | hint `omisu-exec-argv` | the click command; consumes the rest of the line, so it comes last. Each word is a discrete argument the shell runs without re-parsing (see below) |
| `--image` | hint `image-path` | the standard freedesktop image hint |
| `-i` / `--icon` | `app_icon` | themed icon name for the toast |
| `--app-name` | `app_name` | defaults to `omisu-action` |
...
The defaults are the point: an unadorned `omisu-notification-send "Done"`
...
window by class via `omisu-hyprland-focus-app` — chat apps rarely register
...
```bash
omisu-notification-send "Download complete" "$title" --exec mpv -- "$file"
```
...
build the `omisu-exec-argv` hint. (The leading `--` on the `busctl` call is a
...
- `omisu-notification-wait [timeout]` — polls until the shell answers IPC
...
- `omisu-notification-dismiss <summary>` — dismiss by summary substring,
  used by the first-run toasts once their action has been clicked.
- `omisu-notification-time` / `-battery` — the hotkey notices: one-line
  low-urgency glyph toasts wrapping `date` and `omisu-battery-status`.
- `omisu-notification-weather` — despite the name, not a sender: it toggles
  the `omisu.weather` shell panel.
...
- **Low battery** — `omisu-battery-low` sends a critical toast and runs the
  `battery-low` hook.
- **Crash capture** — `omisu-crash-watch` follows the systemd-coredump
  journal stream and announces each crashed program (deduped per minute) as a
  critical toast whose click runs `omisu-agent-crash` (via `--exec`, so a
...
- **Pending migrations** — `omisu-migrate-notify` (from its user service
  after `graphical-session.target`) waits for the server, then sends a
  critical toast whose click opens a terminal running `omisu-migrate`,
...
`bin/omisu-reminder <minutes> [message]` creates a transient systemd user
timer via `systemd-run --user --collect --on-active=<minutes>m` under the
unit name `omisu-reminder-<minutes>m-<epoch>`; the timer's payload sends the
reminder toast, deletes its message file, and refreshes the bar indicator.
Custom messages are stashed in `$XDG_RUNTIME_DIR/omisu-reminders/<unit>.message`
...
`systemctl --user list-timers "omisu-reminder-*.timer"` — `show` as a
summary toast, `show --json` as the JSON the bar's Reminder indicator polls
(refreshed by the same `omisu-shell -q omisu.indicators refresh` call the
timers and mutations make). `omisu-reminder -i` summons the
`omisu.reminders` overlay (`shell/plugins/reminders/ReminderFlow.qml`), a
two-step minutes/message prompt that shells back out to `omisu-reminder` to
do the setting.
