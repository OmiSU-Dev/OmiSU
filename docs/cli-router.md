# The OmiSu CLI router

`bin/omisu` maps spaced commands onto the flat `bin/omisu-*` namespace:
`omisu theme set foo` becomes `exec bin/omisu-theme-set foo`. There is no
registry to maintain — every executable `bin/omisu-*` file is a command, and
...
The stem after `omisu-` splits at the first hyphen: `omisu-theme-set` gets
group `theme` and name `set`, with remaining hyphens becoming spaces
(`omisu-hw-asus-rog` → group `hw`, name `asus rog`). A single-segment stem
(`omisu-update`) is the root command of its own group, with an empty name.

Every command registers two routes: the canonical route `omisu <group>
...
does, both keep working — `# omisu:name=gaming xbox-cloud` on
`omisu-install-gaming-xbox-cloud` keeps the hyphen inside the name, so
`omisu install gaming xbox-cloud` is canonical while the filename route
`omisu install gaming xbox cloud` still resolves. An explicitly *empty* `#
omisu:name=` makes a command the root of its group: `omisu-menu-share` sets
`group=share` and an empty name, so its canonical route is `omisu share`
while `omisu menu share` remains as the filename route. Alias routes register
...
wins, dispatch is unaffected, and the conflict is recorded for `omisu
commands --check` to report. Hidden commands (`# omisu:hidden=true`) still
register and dispatch normally — hiding only removes them from listings, which
is how install-time plumbing like `omisu apply hardware` stays callable
...
effect. Malformed `omisu:` lines and unknown keys are ignored rather than
...
executable file: `omisu theme set foo` probes `omisu-theme-set-foo`, then
`omisu-theme-set`, which exists — resolved without reading a single metadata
...
latency (`omisu dev benchmark cli` tracks it), and a filename probe is a few
stat calls. Metadata loads lazily, only for the resolved command when help is
needed.

When no filename matches — metadata-moved routes like `omisu share`, and
aliases like `omisu screenshot` — the router falls back to loading all
...
the flag — `omisu update aur --help` resolves `update` with leftovers `aur
...
so `omisu foo run -- --help` forwards the flag. `--json` alongside `--help`
...
none were given, the router shows help instead of executing — `omisu theme
...
When nothing resolves, the router tries a prefix listing — `omisu hw asus`
...
word) and a pointer to `omisu commands --all`.
...
matches, listed under the route that fits the group being viewed: `omisu
menu --help` shows `omisu-menu-share` as `omisu menu share`, while its
canonical `omisu share` stands alone. On the fast path, group help loads
only that group's filename-prefixed binaries rather than everything.

The top-level `omisu` listing is driven entirely by the hand-curated
`GROUP_DESCRIPTIONS` table in `bin/omisu`, which also titles each group's
...
`omisu commands` prints every non-hidden command with its summary, plus an
...
that resolves to the binary). Per-command JSON comes from `omisu <route>
--help --json`.

`omisu commands --check` is the metadata lint, run by `test/cli`. It fails
on:

- route collisions between binaries
- a missing explicit `# omisu:summary=` — a plain-comment fallback renders
...
When debugging a routing surprise, `omisu <route> --help` shows the resolved
binary and, when it differs, the filename route; `omisu commands --all
