...
import 'package:omisu/screens/search_screen/search_filter.dart';
...
void main() {
  ...
  group('filterAndSortGames', () {
    ...
    test('name query tolerates small typos', () {
      final r = filterAndSortGames(library, const SearchCriteria(query: 'mrio'));
      expect(r.map((g) => g.realName), ['Mario']);
    });

    test('name query ranks stronger matches first', () {
      final games = [
        game(realName: 'Sonic the Hedgehog'),
        game(realName: 'Sonic CD'),
        game(realName: 'Dr. Robotnik Mean Bean Machine'),
      ];
      final r = filterAndSortGames(games, const SearchCriteria(query: 'sonic'));
      expect(r.first.realName, 'Sonic CD');
      expect(r.map((g) => g.realName), hasLength(2));
    });
  ...
  group('matchesRemoteCriteria', () {
    ...
    test('the query matches the name fuzzily', () {
      expect(
      ...
      expect(
        matchesRemoteCriteria(rom, const SearchCriteria(query: 'chron')),
        isTrue,
      );
      expect(
