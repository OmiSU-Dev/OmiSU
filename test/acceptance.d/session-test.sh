...
# The OmiSu shell is running and responsive
wait_until "omisu-shell responds to ping" 60 omisu-shell shell ping

# Core shell plugins are loaded
plugins=$(omisu-shell shell listPlugins)
for plugin in \
  omisu.audio omisu.background omisu.bar omisu.bluetooth \
  omisu.clipboard omisu.emojis omisu.menu \
  omisu.monitor omisu.network omisu.notifications omisu.power \
  omisu.reminders omisu.weather; do
...
wait_until "bar layer is on screen" 30 layer_on_screen "omisu-bar"
wait_until "background layer is on screen" 30 layer_on_screen "omisu-background"
...
restore_bar_visibility() {
  omisu-toggle-bar off >/dev/null 2>&1 || true
}
trap restore_bar_visibility EXIT

omisu-toggle-bar on
wait_until "hidden bar layer stays mapped" 15 layer_present "omisu-bar"
wait_until "hidden bar layer parks off screen" 15 layer_off_screen "omisu-bar"
screenshot "success-bar-hidden"

omisu-toggle-bar off
wait_until "revealed bar layer returns on screen" 15 layer_on_screen "omisu-bar"
...
# OmiSu reports its version
omisu-version >/dev/null || fail "omisu-version works"
pass "omisu-version works"

# No failed units, system or user. OMISU_ACCEPTANCE_IGNORE_UNITS can hold a
# regex of units to overlook (useful on dev machines; a fresh VM should be clean).
failed_units() {
  systemctl "$@" --failed --no-legend --plain | awk '{print $1}' |
    grep -Ev "${OMISU_ACCEPTANCE_IGNORE_UNITS:-^$}" || true
}
