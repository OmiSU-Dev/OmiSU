...
verify_core_packages() {
  ...
  done <"$OMISU_PATH/install/omisu-base.packages"

  (( ${#missing[@]} == 0 )) || fail "all OmiSu core packages are installed" "missing packages: ${missing[*]}"
  pass "all OmiSu core packages are installed (${#missing[@]} missing)"
}

verify_defaults() {
  [[ $(omisu-default-browser) == "chromium" ]] || fail "Chromium is the default browser"
  pass "Chromium is the default browser"

  [[ $(omisu-default-terminal) == "foot" ]] || fail "Foot is the default terminal"
  pass "Foot is the default terminal"

  [[ $(omisu-default-editor) == "nvim" ]] || fail "Neovim is the default editor"
  pass "Neovim is the default editor"

  [[ $(omisu-theme-current) != "Unknown" ]] || fail "a current theme is configured"
  pass "a current theme is configured"

  [[ $(omisu-theme-bg-current) != "Unknown" ]] || fail "a current background is configured"
  pass "a current background is configured"

  [[ -n $(omisu-font-current) ]] || fail "a monospace font is configured"
...
verify_user_setup() {
  ...
  [[ -e $HOME/.local/state/omisu/current/theme ]] || fail "current theme state exists"
  [[ -e $HOME/.local/state/omisu/current/background ]] || fail "current background state exists"
  [[ -s $HOME/.config/omisu/shell.json ]] || fail "shell configuration exists"
  jq empty "$HOME/.config/omisu/shell.json" || fail "shell configuration is valid JSON"
  pass "OmiSu user state and shell configuration exist"
}
