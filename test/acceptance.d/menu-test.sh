...
DEFAULTS_FILE="$OMISU_PATH/config/omisu/shell.json"
...
restore_bar_config() {
  omisu-shell shell hide omisu.menu >/dev/null 2>&1 || true
  ...
  omisu-shell shell reloadConfig >/dev/null 2>&1 || true
  rm -f "$config_backup"
}
...
bar_is_vertical() {
  ...
  read -r width height < <(hyprctl -j layers | jq -r '
    [.. | objects | select(.namespace? == "omisu-bar")][0]
    | [.w, .h] | @tsv
  ')
...
omisu-shell shell summon omisu.menu '{"menu":"root"}' >/dev/null
wait_until "root menu opens" 15 layer_present "omisu-menu"
...
wait_until "menu closes after selecting a position" 15 layer_absent "omisu-menu"
...
omisu-shell shell reloadConfig >/dev/null
