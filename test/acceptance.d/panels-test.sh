...
WEATHER_FILE="$HOME/.local/state/omisu/settings/weather.json"
...
hide_panels() {
  ...
  for plugin in omisu.weather omisu.bluetooth omisu.network omisu.audio omisu.monitor omisu.power; do
    omisu-shell shell hide "$plugin" >/dev/null 2>&1 || true
  done
}
...
open_and_capture_panel() {
  ...
  wait_until "$name panel opens" 15 layer_present "omisu-keyboard-panel"
  sleep 1
  screenshot "success-panel-$name"

  omisu-shell shell hide "$plugin" >/dev/null
  wait_until "$name panel closes" 15 layer_absent "omisu-keyboard-panel"
}
...
omisu-weather-location --set "San Francisco" "37.7749,-122.4194"
omisu-shell shell summon omisu.weather >/dev/null
wait_until "weather panel opens" 15 layer_present "omisu-keyboard-panel"
...
omisu-shell shell hide omisu.weather >/dev/null
wait_until "weather panel closes" 15 layer_absent "omisu-keyboard-panel"

status=0
panels='bluetooth|omisu.bluetooth
network|omisu.network
audio|omisu.audio
monitor|omisu.monitor'

while IFS='|' read -r name plugin; do
  if ! (trap - EXIT; open_and_capture_panel "$name" "$plugin"); then
    status=1
    hide_panels
    wait_until "$name failed panel is dismissed" 15 layer_absent "omisu-keyboard-panel"
  fi
done <<<"$panels"
...
if upower -e | grep '/battery_' >/dev/null; then
  if ! (trap - EXIT; open_and_capture_panel "power" "omisu.power"); then
    status=1
    hide_panels
    wait_until "power failed panel is dismissed" 15 layer_absent "omisu-keyboard-panel"
  fi
else
...
omisu-shell shell summon omisu.bluetooth >/dev/null
...
wtype -k Tab
sleep 2
wait_until "Tab keeps a shell panel open" 15 layer_present "omisu-keyboard-panel"
screenshot "success-panel-navigation-02-next"
hide_panels
wait_until "keyboard-navigated panel closes" 15 layer_absent "omisu-keyboard-panel"
...
omisu-shell shell summon omisu.bluetooth >/dev/null
wait_until "focus-prime panel opens" 15 layer_present "omisu-keyboard-panel"
if (( $(hyprctl -j monitors | jq length) == 1 )); then
  layer_absent "omisu-keyboard-panel-dismiss" || fail "single-monitor panel has no dismissal twin"
  pass "single-monitor panel has no dismissal twin"
fi
omisu-shell shell hide omisu.bluetooth >/dev/null
omisu-shell shell summon omisu.bluetooth >/dev/null
wait_until "focus-prime panel reopens" 15 layer_present "omisu-keyboard-panel"
sleep 1
screenshot "success-panel-focus-prime-reopened"
wtype -k Escape
wait_until "Escape closes a panel reopened during fade" 15 layer_absent "omisu-keyboard-panel"
