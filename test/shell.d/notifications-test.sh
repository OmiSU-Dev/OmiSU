...
assert(notifications.shouldBypassDnd({ appName: 'omisu-action', urgency: 1 }, 2), 'omisu action toasts bypass DND')
...
assert(!notifications.shouldBypassDnd({ appName: 'omisu-menu-keybindings', urgency: 1 }, 2), 'omisu command app names do not bypass DND')
assert(!notifications.isEphemeralApp('omisu-menu-keybindings'), 'notifications treat omisu command app names as normal apps')

// The click action's argv form: parsed from the persisted omisu-exec-argv
...
const execSnapshot = notifications.snapshotOf({
  id: 3,
  appName: 'omisu-action',
  summary: 'Download complete',
  hints: { 'omisu-exec-argv': '["mpv","--","/tmp/clip.mp4"]' }
}, 1)
assertEqual(
...
assertEqual(
  notifications.snapshotOf({ id: 3, hints: { 'omisu-glyph': '!' } }, 1).execArgv,
  '',
...
assert(
  ...
  'notifications service persists popups under the omisu state dir'
)
assert(
