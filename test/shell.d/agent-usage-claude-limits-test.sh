...
read_limits() {
  COLLECTOR="$ROOT/bin/omisu-agent-usage-claude" PAYLOAD="$1" python3 - <<'PY'
...
collect_limits() {
  COLLECTOR="$ROOT/bin/omisu-agent-usage-claude" TOKEN="$1" EXPIRES_AT="$2" CACHED="$3" \
...
probe_with_cache() {
  COLLECTOR="$ROOT/bin/omisu-agent-usage-claude" FORCE="$1" CACHED="$2" PAYLOAD="$3" \
    XDG_CACHE_HOME="$CACHE_HOME" python3 - <<'PY'
import importlib.machinery, importlib.util, io, json, os
