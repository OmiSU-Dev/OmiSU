...
const defaultMenuJsonc = fs.readFileSync(path.join(root, 'default/omisu/omisu-menu.jsonc'), 'utf8')
...
const installed = menu.normalizeItem('install.browser.zen', { label: 'Zen', disabled: 'omisu-pkg-present zen-browser-bin', action: 'install-zen' })
...
assert(
  defaultById['update.omisu'].icon === '\ue900',
  'menu update OmiSu entry uses the OmiSu glyph'
)
assert(
  defaultById['update.omisu'].iconFont === 'omisu',
  'menu update OmiSu entry renders the private glyph with the OmiSu font'
)
assertEqual(
...
assert(
  defaultById['setup.direct-boot'].action.includes('omisu-setup-direct-boot'),
  'menu places Direct Boot directly under Setup'
)
assert(
  defaultById['setup.reset'].action.includes('omisu-system-factory-reset'),
...
const expectedAgents = {
  ...
  pi: { icon: '\ue901', iconFont: 'omisu', label: 'Pi' },
  omp: { icon: '\ue903', iconFont: 'omisu', label: 'omp' },
  opencode: { icon: '\ue902', iconFont: 'omisu', label: 'OpenCode' },
  ori: { icon: '\ue909', iconFont: 'omisu', label: 'Ori' },
  claude: { icon: '󰛄', label: 'Claude' },
  codex: { icon: '\ue905', iconFont: 'omisu', label: 'Codex' },
  grok: { icon: '\ue904', iconFont: 'omisu', label: 'Grok' },
...
assert(
  Object.entries(expectedAgents).every(([agent, expected]) => {
    ...
    return entry
      ...
      && entry.action === `omisu-default-agent ${agent}`
      && !entry.when
...
// dropped, so the list reads as a catalog of what OmiSu can install.
...
assertEqual(
  ...
  'omisu-pkg-present zen-browser-bin',
...
assert(
  defaultItems.filter(item => item.id.startsWith('remove.')).every(item => !item.disabled)
    && defaultById['remove.browser.zen'].when === 'omisu-pkg-present zen-browser-bin',
...
assert(
  defaultById['setup.security.passwordless-sudo'].action.includes('omisu-sudo-passwordless'),
...
assert(
  ['top', 'bottom', 'left', 'right'].every(position => defaultById[`style.bar.position.${position}`].action === `omisu-bar position ${position}`),
...
assert(
  ['enable', 'disable', 'clone', 'remove'].every(
    verb => defaultById[`setup.plugin.${verb}`].action === `omisu-menu-plugin ${verb}`
  ),
...
assert(
  defaultById['setup.plugin.remove'].when.includes('.config/omisu/plugins'),
  'menu hides Remove until a plugin the user installed exists to delete'
)
assert(
  defaultById['setup.plugin.add'].action.includes('omisu-plugin-add'),
  'menu adds a plugin through the CLI, where the trust warning and clone output are visible'
)

const pluginPicker = fs.readFileSync(path.join(root, 'bin/omisu-menu-plugin'), 'utf8')
assert(
...
const pluginAdd = fs.readFileSync(path.join(root, 'bin/omisu-plugin-add'), 'utf8')
const pluginEnable = fs.readFileSync(path.join(root, 'bin/omisu-plugin-enable'), 'utf8')
assert(
  /Now using \$id as the bar/.test(pluginEnable)
    && /omisu-plugin-enable "\$id" "\$\{ENABLE_PLACEMENT\[@\]\}"/.test(pluginAdd),
...
assert(
  /"omisu-plugin-\$1" "\$id"/.test(pluginPicker),
...
assert(
  /omisu-launch-floating-terminal-with-presentation "omisu-plugin-remove/.test(pluginPicker),
...
font_charset=$(fc-query --format='%{charset}' "$ROOT/default/fonts/omisu/omisu.ttf")
[[ $font_charset == *"e900-e909"* ]] || fail "OmiSu icon font includes every custom menu glyph"
pass "OmiSu icon font includes the official agent marks"
