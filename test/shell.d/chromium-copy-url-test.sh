...
grep -q "sendNativeMessage('com.omisu.copy_url'" \
...
native_manifest="$test_home/.config/chromium/NativeMessagingHosts/com.omisu.copy_url.json"

HOME="$test_home" OMISU_PATH="$ROOT" omisu-install-chromium-copy-url

[[ -f $native_manifest ]] || fail "copy-url native host installer creates fresh Chromium profile root"
jq -e --arg path "$ROOT/bin/omisu-chromium-copy-url-host" '
  .name == "com.omisu.copy_url" and
  .path == $path and
  (.allowed_origins | index("chrome-extension://bgpiichlckmfanooecilcjemknkcpngb/"))
' "$native_manifest" >/dev/null || fail "copy-url native host manifest uses OmiSu host path and extension id"
...
# omisu-install-browser, and they mark every migration as already applied.
...
HOME="$fresh_home" OMISU_PATH="$ROOT" PATH="$ROOT/bin:$PATH" \
  bash -euo pipefail -c 'source "$ROOT/install/user/chromium.sh"'

[[ -f $fresh_home/.config/chromium/NativeMessagingHosts/com.omisu.copy_url.json ]] ||
...
copied_url=$(bash -c '
  source "$1"
  wl-copy() { cat; }
  omisu-notification-send() { :; }
  copy_url "$2"
' bash "$ROOT/bin/omisu-chromium-copy-url-host" 'https://example.test/path?q=one&name=two')
...
native_reply=$(bash -c '
  source "$1"
  reply_copied true
' bash "$ROOT/bin/omisu-chromium-copy-url-host" | od -An -v -tx1 | tr -d ' \n')
