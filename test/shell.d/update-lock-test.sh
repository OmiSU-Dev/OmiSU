...
for command in \
  ...
  omisu-shell \
  omisu-update-restart; do
  write_stub "$command" 'exit 0'
done
write_stub omisu-update-available 'exit 1'
write_stub pkexec 'exec "$@"'

# omisu-update should hold the lock before snapshotting, so a second update
# cannot even enter its pre-update snapshot.
update_snapshot_marker="$test_tmp/update-snapshot-started"
write_stub omisu-snapshot 'echo started >"$TEST_MARKER"; sleep 2; exit 0'

OMISU_UPDATE_LOGGED=1 TEST_MARKER="$update_snapshot_marker" run_with_lock_env "$ROOT/bin/omisu-update" -y >"$test_tmp/update-first.out" 2>&1 &
update_pid=$!
...
[[ -f $update_snapshot_marker ]] || fail "first omisu-update reached snapshot under lock"

set +e
OMISU_UPDATE_LOGGED=1 TEST_MARKER="$test_tmp/update-second-snapshot-started" run_with_lock_env "$ROOT/bin/omisu-update" -y >"$test_tmp/update-second.out" 2>&1
update_second_status=$?
set -e

wait "$update_pid"

[[ $update_second_status -ne 0 ]] || fail "second omisu-update exits non-zero while update lock is held"
grep -q "already running" "$test_tmp/update-second.out" || fail "second omisu-update reports held update lock"
[[ ! -f $test_tmp/update-second-snapshot-started ]] || fail "second omisu-update did not snapshot while lock was held"
pass "omisu-update prevents overlapping top-level updates"
...
# later update and silencing omisu-migrate-notify, which reads the same lock.
inhibit_pid_file="$test_tmp/inhibit-pid"
keyring_marker="$test_tmp/keyring-started"
write_stub omisu-snapshot 'exit 0'
write_stub systemd-inhibit 'echo "$$" >"$INHIBIT_PID_FILE"; exec sleep 30'
write_stub omisu-update-keyring 'echo started >"$TEST_MARKER"; sleep 3; exit 0'

OMISU_UPDATE_LOGGED=1 TEST_MARKER="$keyring_marker" INHIBIT_PID_FILE="$inhibit_pid_file" \
  run_with_lock_env "$ROOT/bin/omisu-update" -y >"$test_tmp/update-inhibit.out" 2>&1 &
inhibit_update_pid=$!
...
lock_target=$(readlink -f "$runtime_dir/omisu-update.lock")
...
pass "omisu-update keeps the update lock out of its sleep inhibitor"

kill -0 "$inhibitor_pid" 2>/dev/null &&
  fail "update waits for its sleep inhibitor to stop before continuing"
pass "omisu-update waits for its sleep inhibitor to stop"
...
SH
  ...
  run_with_lock_env "$ROOT/bin/omisu-update-stay-awake" stop
...
write_stub omisu-snapshot 'exit 0'
write_stub omisu-update-keyring 'exit 0'
write_stub omisu-toggle-idle '
state_file="$HOME/.local/state/omisu/indicators/stay-awake"
case "$1" in
  stay-awake)
...
write_stub omisu-update-restart '
state_file="$HOME/.local/state/omisu/indicators/stay-awake"
...
rm -f "$test_home/.local/state/omisu/indicators/stay-awake"
OMISU_UPDATE_LOGGED=1 run_with_lock_env "$ROOT/bin/omisu-update" -y
[[ ! -f $test_home/.local/state/omisu/indicators/stay-awake ]] || fail "update clears its Stay Awake state before restart handling"

mkdir -p "$test_home/.local/state/omisu/indicators"
touch "$test_home/.local/state/omisu/indicators/stay-awake"
OMISU_UPDATE_LOGGED=1 EXPECT_STAY_AWAKE=1 run_with_lock_env "$ROOT/bin/omisu-update" -y
[[ -f $test_home/.local/state/omisu/indicators/stay-awake ]] || fail "update preserves pre-existing Stay Awake state"
pass "omisu-update restores only its own Stay Awake state before restart handling"

# Stale cleanup state from a killed update must not override a Stay Awake choice
# the user made afterward.
stay_awake_helper_state="$runtime_dir/omisu-update-stay-awake"
stay_awake_state="$test_home/.local/state/omisu/indicators/stay-awake"
