...
[[ $OMISU_PATH == "$TEST_EXPECTED_OMISU_PATH" ]]
...
if ! HOME="$test_home" OMISU_PATH="$test_root" "$ROOT/bin/omisu-migrate" --pending >"$test_tmp/pending.out"; then
...
TEST_EXPECTED_OMISU_PATH="$test_root" \
TEST_CALLS="$calls" \
  "$ROOT/bin/omisu-migrate" >"$test_tmp/first-run.out"
...
[[ -f $test_home/.local/state/omisu/migrations/100-first.sh ]] || fail "migration runner records first migration marker"
[[ -f $test_home/.local/state/omisu/migrations/200-second.sh ]] || fail "migration runner records second migration marker"
...
TEST_EXPECTED_OMISU_PATH="$test_root" \
TEST_CALLS="$calls" \
  "$ROOT/bin/omisu-migrate" >"$test_tmp/second-run.out"
[[ $(wc -l <"$calls") -eq 2 ]] || fail "migration runner skips completed migrations"
pass "migration runner skips completed migrations"

if HOME="$test_home" OMISU_PATH="$test_root" "$ROOT/bin/omisu-migrate" --pending >"$test_tmp/not-pending.out"; then
...
failure_root="$test_tmp/failure-omisu"
...
set +e
HOME="$failure_home" \
OMISU_PATH="$failure_root" \
TEST_CALLS="$calls" \
  "$ROOT/bin/omisu-migrate" >"$test_tmp/failure.out" 2>"$test_tmp/failure.err"
failure_status=$?
set -e
[[ $failure_status -ne 0 ]] || fail "migration runner exits non-zero when a migration fails"
[[ ! -f $failure_home/.local/state/omisu/migrations/500-fail.sh ]] || fail "migration runner does not mark failed migration complete"
