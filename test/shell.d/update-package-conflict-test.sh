...
update_env() {
  printf '%s\n' \
    "OMISU_REPLACED_DIR=$test_tmp/replaced" \
    "PACMAN_ATTEMPTS=$test_tmp/attempts" \
    "PACMAN_CALLS=$test_tmp/calls" \
    "CONFLICT_REPORT=$test_tmp/report" \
    "OWNED_PATHS=" \
    "OMISU_UPDATE_UNATTENDED=${OMISU_UPDATE_UNATTENDED:-}" \
    "OMISU_UPDATE_INTERACTIVE=${OMISU_UPDATE_INTERACTIVE:-}" \
...
run_headless() {
  ...
  env "${environment[@]}" bash "$ROOT/bin/omisu-update-system-pkgs" \
    </dev/null >"$test_tmp/out" 2>"$test_tmp/err"
}

# script gives the update the pty that omisu-update always runs it on, so the
...
run_on_terminal() {
  mapfile -t environment < <(update_env)
  env "${environment[@]}" \
    script -qec "bash '$ROOT/bin/omisu-update-system-pkgs' ${1:-}" "$test_tmp/out" >/dev/null 2>&1
}
...
grep -q 'omisu update' "$test_tmp/err" ||
...
write_conflict_report
if OMISU_UPDATE_UNATTENDED=1 run_on_terminal; then
...
write_conflict_report
if OMISU_UPDATE_INTERACTIVE=1 run_headless; then
