...
CURRENT_THEME="$TEST_HOME/.local/state/omisu/current/theme"
mkdir -p "$FAKE_BIN" "$CURRENT_THEME"

cat >"$FAKE_BIN/omisu-cmd-present" <<'EOF'
#!/bin/bash
printf '%s\n' "$1" >>"$EDITOR_PROBE_LOG"
exit 1
EOF

cat >"$FAKE_BIN/omisu-toggle-enabled" <<'EOF'
#!/bin/bash
exit 1
EOF
...
EDITOR_PROBE_LOG="$TEST_HOME/editor-probes.log" \
  ...
  HOME="$TEST_HOME" \
  "$ROOT/bin/omisu-theme-set-vscode"
