...
cat >"$WORKDIR/bin/omisu-notification-send" <<SH
#!/bin/bash
...
receive() {
  ...
  : >"$WORKDIR/notifications"
  PATH="$WORKDIR/bin:$PATH" "$@" "$ROOT/bin/omisu-tailscale-receive" --once "$downloads"
...
[[ -z $(ls -A "$downloads/.omisu-taildrop") ]] ||
  fail "taildrop receive empties its staging directory" "$(ls -A "$downloads/.omisu-taildrop")"
