...
command="$ROOT/bin/omisu-windows-key"
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
...
run_helper() {
  OMISU_MSDM_PATH=$1 PATH="$ROOT/bin:$PATH" "$command"
}
...
output=$(PATH="$tmp/bin:$ROOT/bin:$PATH" OMISU_MSDM_PATH="$tmp/msdm" "$command")
...
if OMISU_MSDM_PATH="$tmp/empty" PATH="$ROOT/bin:$PATH" "$command" >"$tmp/out" 2>"$tmp/err"; then
...
if OMISU_MSDM_PATH="$tmp/missing" PATH="$ROOT/bin:$PATH" "$command" >"$tmp/out" 2>"$tmp/err"; then
