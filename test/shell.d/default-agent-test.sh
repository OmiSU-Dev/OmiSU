...
agent_file="$test_home/.config/omisu/defaults/agent"
...
cat >"$mock_bin/omisu-notification-send" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >>"$OMISU_TEST_NOTIFICATION_HISTORY"
SH

cat >"$mock_bin/omisu-cmd-missing" <<'SH'
#!/bin/bash
[[ $1 == ${OMISU_TEST_MISSING_COMMAND:-} ]]
SH

cat >"$mock_bin/omisu-launch-tui" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >"$OMISU_TEST_AGENT_LAUNCH_LOG"
SH

cat >"$mock_bin/omisu-launch-floating-terminal-with-presentation" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >"$OMISU_TEST_AGENT_TERMINAL_LOG"
SH

cat >"$mock_bin/opencode" <<'SH'
#!/bin/bash
printf '%s\0' opencode "$@" >"$OMISU_TEST_AGENT_INLINE_LOG"
SH

cat >"$mock_bin/omisu-mise-install" <<'SH'
#!/bin/bash
printf '%s\n' "$*" >>"$OMISU_TEST_STUB_LOG"
SH
...
#!/bin/bash
printf '%s\0' "$@" >"$OMISU_TEST_MISE_LOG"
printf '%s\n' "$*" >>"$OMISU_TEST_MISE_HISTORY"

if [[ $1 == "where" ]]; then
  [[ ${OMISU_TEST_AGENT_INSTALLED:-false} == "true" ]]
  exit
fi

[[ ${OMISU_TEST_MISE_FAIL:-false} != "true" ]]
SH

cat >"$mock_bin/omisu-menu" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >"$OMISU_TEST_AGENT_MENU_LOG"
SH

cat >"$mock_bin/omisu-test-noop" <<'SH'
#!/bin/bash
exit 0
SH

for command in gum hyprctl omisu-webapp-remove-all omisu-tui-remove-all omisu-pkg-drop; do
  ln -s omisu-test-noop "$mock_bin/$command"
done
...
export OMISU_TEST_NOTIFICATION_HISTORY="$notification_history"
export OMISU_TEST_AGENT_OPEN_LOG="$agent_open_log"
export OMISU_TEST_AGENT_LAUNCH_LOG="$launch_log"
export OMISU_TEST_AGENT_INLINE_LOG="$inline_log"
export OMISU_TEST_MISE_LOG="$mise_log"
export OMISU_TEST_MISE_HISTORY="$mise_history"
export OMISU_TEST_STUB_LOG="$stub_log"
export OMISU_TEST_AGENT_TERMINAL_LOG="$terminal_log"
export OMISU_TEST_AGENT_MENU_LOG="$menu_log"
export OMISU_PATH="$ROOT"
...
assert_lazy_stub() {
  ...
  : >"$mise_history"
  "$ROOT/bin/omisu-mise-install" "$package" "$command"
...
printf '%s\n' gemini >"$agent_file"
"$ROOT/bin/omisu-mise-install" gemini
export OMISU_TEST_MISSING_COMMAND=agy
source "$ROOT/migrations/1786719479.sh" >/dev/null
unset OMISU_TEST_MISSING_COMMAND
...
pass "Antigravity migration only removes the Gemini wrapper OmiSu wrote"

[[ -L "$test_home/.gemini/config/skills/omisu" && $(readlink "$test_home/.gemini/config/skills/omisu") == "$ROOT/default/agents/skills/omisu" ]] ||
   fail "Antigravity migration provisions the omisu skill"
...
: >"$stub_log"
mkdir -p "$test_home/.local/state/omisu"
touch "$test_home/.local/state/omisu/preinstalls-removed"
export OMISU_TEST_MISSING_COMMAND=agy
source "$ROOT/migrations/1786719479.sh" >/dev/null
...
: >"$stub_log"
rm "$test_home/.local/state/omisu/preinstalls-removed"
source "$ROOT/migrations/1786719479.sh" >/dev/null
...
mkdir -p "$test_home/.local/state/omisu"
touch "$test_home/.local/state/omisu/preinstalls-removed"
"$ROOT/bin/omisu-mise-install" oh-my-pi omp
: >"$stub_log"
source "$ROOT/migrations/1785617047.sh" >/dev/null
source "$ROOT/migrations/1785846769.sh" >/dev/null
source "$ROOT/migrations/1787342993.sh" >/dev/null
...
omisu-remove-preinstalls >/dev/null
...
[[ -z $(omisu-default-agent) ]] || fail "default agent is unset until one is chosen"
pass "default agent is unset until one is chosen"

: >"$launch_log"
if omisu-agent >"$test_tmp/no-agent-output" 2>&1; then
...
cat >"$mock_bin/omisu-agent" <<'SH'
#!/bin/bash
printf '%s\0' omisu-agent "$@" >"$OMISU_TEST_AGENT_OPEN_LOG"
SH
chmod +x "$mock_bin/omisu-agent"
hash -r
...
for selection in "${!expected_agents[@]}"; do
  ...
  : >"$agent_open_log"
  OMISU_TEST_AGENT_INSTALLED=true omisu-default-agent "$selection"
  [[ $(omisu-default-agent) == $expected ]] || fail "default agent canonicalizes $selection"
  ...
  [[ ${#agent_open_args[@]} == 1 && ${agent_open_args[0]} == "omisu-agent" ]] ||
...
[[ -f $agent_file && ! -e $test_home/.local/state/omisu/defaults/agent ]] ||
  fail "default agent stores its selection in OmiSu user config"
pass "default agent stores its selection in OmiSu user config"

OMISU_TEST_AGENT_INSTALLED=true omisu-default-agent pi
: >"$notification_history"
: >"$agent_open_log"
: >"$terminal_log"
omisu-default-agent github-copilot
mapfile -d '' -t terminal_args <"$terminal_log"
[[ ${terminal_args[0]} == "omisu-default-agent" && ${terminal_args[1]} == "--install" && ${terminal_args[2]} == "copilot" ]] ||
...
[[ $(omisu-default-agent) == "pi" ]] || fail "missing agent installation waits to change the selection"

omisu-default-agent --install github-copilot >"$test_tmp/install-output"
mapfile -d '' -t mise_args <"$mise_log"
...
[[ $(omisu-default-agent) == "copilot" ]] || fail "visible agent installation changes the selection after mise succeeds"
...
mapfile -d '' -t agent_open_args <"$agent_open_log"
[[ ${#agent_open_args[@]} == 2 && ${agent_open_args[0]} == "omisu-agent" && ${agent_open_args[1]} == "--inline" ]] ||
...
: >"$agent_open_log"
: >"$terminal_log"
OMISU_TEST_AGENT_INSTALLED=true omisu-default-agent github-copilot
...
: >"$agent_open_log"
if omisu-default-agent unsupported >"$test_tmp/invalid-output" 2>&1; then
  fail "default agent rejects unsupported providers"
fi
grep -F "Usage: omisu-default-agent" "$test_tmp/invalid-output" >/dev/null ||
  fail "default agent explains supported providers"
[[ $(omisu-default-agent) == "copilot" ]] || fail "invalid selection preserves the current default agent"
...
: >"$notification_history"
: >"$agent_open_log"
if OMISU_TEST_MISE_FAIL=true omisu-default-agent --install codex >"$test_tmp/install-failure-output" 2>&1; then
  fail "default agent rejects a failed mise installation"
fi
[[ $(omisu-default-agent) == "copilot" ]] || fail "failed installation preserves the current default agent"
...
: >"$notification_history"
: >"$agent_open_log"
if OMISU_TEST_AGENT_INSTALLED=true OMISU_TEST_MISE_FAIL=true omisu-default-agent codex >"$test_tmp/setup-failure-output" 2>&1; then
  fail "default agent rejects a failed mise activation"
fi
[[ $(omisu-default-agent) == "copilot" ]] || fail "failed activation preserves the current default agent"
...
assert_launched() {
  ...
  local expected=(--app-id=org.omisu.agent "$@")
...
assert_launch() {
  ...
  printf '%s\n' "$agent" >"$agent_file"
  omisu-agent-prompt "Review this" project
...
[[ ${launch_args[*]} == "--app-id=org.omisu.agent opencode --auto" ]] ||
...
omisu-agent-prompt --inline "Review this project"
...
mapfile -d '' -t launch_args <"$launch_log"
[[ ${launch_args[*]} == "--app-id=org.omisu.agent opencode --auto" ]] ||
  fail "omisu agent routes to the launcher"
...
omisu agent prompt "Review this project"
mapfile -d '' -t launch_args <"$launch_log"
[[ ${launch_args[*]} == "--app-id=org.omisu.agent opencode --auto --prompt Review this project" ]] ||
  fail "omisu agent prompt routes the prompt to the launcher"

: >"$launch_log"
if omisu agent Review this project >"$test_tmp/positional-output" 2>&1; then
  fail "omisu agent rejects a positional prompt"
fi
grep -F "omisu agent prompt" "$test_tmp/positional-output" >/dev/null ||
  fail "omisu agent points a positional prompt at the prompt route"
[[ ! -s $launch_log ]] || fail "omisu agent starts nothing for a positional prompt"
pass "omisu agent keeps prompts on the prompt route"

printf '%s\n' "missing" >"$agent_file"
if OMISU_TEST_MISSING_COMMAND=missing omisu-agent >"$test_tmp/missing-output" 2>&1; then
