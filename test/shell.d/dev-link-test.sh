...
conf_file="$test_tmp/omisu.conf"
sudoers_file="$test_tmp/omisu-dev-path"
mkdir -p "$stub_bin" "$test_tmp/home"

cat >"$stub_bin/sudo" <<'SH'
#!/bin/bash

printf 'sudo' >>"$OMISU_DEV_LINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$OMISU_DEV_LINK_TEST_LOG"
done
printf '\n' >>"$OMISU_DEV_LINK_TEST_LOG"

case "$1" in
  tee)
    cat >"$OMISU_DEV_LINK_TEST_CONF"
    ;;
  install)
    # The staged file is the second-to-last argument.
    cp "${@: -2:1}" "$OMISU_DEV_LINK_TEST_SUDOERS"
    ;;
esac
SH
...
printf 'gum' >>"$OMISU_DEV_LINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$OMISU_DEV_LINK_TEST_LOG"
done
printf '\n' >>"$OMISU_DEV_LINK_TEST_LOG"
...
cat >"$stub_bin/omisu-system-reboot" <<'SH'
#!/bin/bash

printf 'reboot\n' >>"$OMISU_DEV_LINK_TEST_LOG"
SH
chmod +x "$stub_bin/omisu-system-reboot"

run_link() {
  HOME="$test_tmp/home" \
    OMISU_DEV_LINK_TEST_LOG="$log_file" \
    OMISU_DEV_LINK_TEST_CONF="$conf_file" \
    OMISU_DEV_LINK_TEST_SUDOERS="$sudoers_file" \
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/omisu-dev-link" "$@"
}
...
[[ $(<"$conf_file") == "export OMISU_PATH=\"$checkout\"" ]] ||
  fail "dev link points OMISU_PATH at the checkout" "$(<"$conf_file")"
pass "dev link points OMISU_PATH at the checkout"

# sudo reads secure_path, not the caller's PATH, so the checkout has to come
# first there too or `sudo omisu-*` runs the packaged copy.
...
grep -Eq $'^sudo\tinstall\t-Dm440\t-o\troot\t-g\troot\t[^\t]+\t/etc/sudoers\\.d/omisu-dev-path$' "$log_file" ||
...
grep -F "sudo now resolves omisu-* from $checkout/bin" "$test_tmp/link.out" >/dev/null ||
