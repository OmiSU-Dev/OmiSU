...
rm -f "$home/.config/google-chrome/SingletonLock" "$preferences.omisu-copy-url-repair.bak"
...
rm -f "$preferences.omisu-copy-url-repair.bak"
...
[[ -f $preferences.omisu-copy-url-repair.bak ]] ||
...
rm "$preferences.omisu-copy-url-repair.bak"
repaired_hash=$(sha256sum "$preferences" | cut -d' ' -f1)
open_browser
run_migration || fail "migration reruns cleanly after the repair"
[[ $(sha256sum "$preferences" | cut -d' ' -f1) == "$repaired_hash" && ! -e $preferences.omisu-copy-url-repair.bak ]] ||
...
rm -f "$stub_bin/python3" "$preferences.omisu-copy-url-repair.bak"
close_browser
...
[[ -f $preferences.omisu-copy-url-repair.bak ]] || fail "verification scenario has a repair backup"
open_browser
