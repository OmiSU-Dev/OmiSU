...
# A theme installed with `omisu theme install` is a stranger's git repo, so
# omisu-theme-set drops the files that would run its code -- Lua, terminal
...
home="$test_tmp/home"
state="$home/.local/state/omisu/current"
themes="$home/.config/omisu/themes"
mkdir -p "$state" "$themes"

marker="omisu-theme-staging-marker"

set_theme() {
  HOME="$home" OMISU_PATH="$ROOT" PATH="$ROOT/bin:$PATH" \
    OMISU_THEME_HEADLESS=1 OMISU_THEME_SKIP_BACKGROUND=1 \
    XDG_RUNTIME_DIR="$test_tmp" \
    bash "$ROOT/bin/omisu-theme-set" "$1" 2>"$test_tmp/stderr" || return $?
}
...
set_theme hostile || fail "omisu-theme-set applies a theme that ships disallowed files"
...
# These run code, so the theme's versions must lose to OmiSu's generated ones
...
for generated in hyprland.lua neovim.lua gum_env.lua kitty.conf alacritty.toml foot.ini ghostty.conf; do
  assert_staged "$generated" "$generated is generated from OmiSu's template"
  assert_no_marker "$generated" "an installed theme cannot supply $generated"
done

# Colour is kept, including a file OmiSu would otherwise have generated.
...
grep -q 'hyprland.lua' "$test_tmp/stderr" || fail "omisu-theme-set names the files it ignored"
! grep -q 'README.md' "$test_tmp/stderr" || fail "omisu-theme-set does not report a theme's documentation"
...
set_theme linked || fail "omisu-theme-set applies a theme whose icons.theme is a symlink"
...
set_theme legacy || fail "omisu-theme-set applies a theme that only ships alacritty.toml"
...
set_theme "Tokyo Night" || fail "omisu-theme-set applies a stock theme with a user overlay"
...
set_theme mine || fail "omisu-theme-set applies a theme the user wrote"
...
set_theme mine-link || fail "omisu-theme-set applies a symlinked working copy"
...
for name in .. . "../../evil"; do
  if set_theme "$name" >/dev/null; then
    fail "omisu-theme-set rejects the theme name '$name'"
  fi
done
...
pass "every file OmiSu generates is classified as code or colour"
