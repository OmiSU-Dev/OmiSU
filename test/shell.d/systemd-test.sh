...
sleep_service="$ROOT/default/systemd/user/omisu-sleep-lock.service"
grep -Fx 'ExecStart=/usr/bin/omisu-system-sleep-monitor' "$sleep_service" >/dev/null
...
grep -Fx 'ConditionEnvironment=OMISU_PATH' "$sleep_service" >/dev/null ||
  fail "sleep lock monitor can start without the OmiSu shell path"
...
grep -F 'omisu-sleep-lock.service' "$first_run_units" >/dev/null
...
grep -F 'ExecStart=%h/.local/share/omisu/bin/omisu-system-sleep-monitor' "$upgrade_to_quattro" >/dev/null
grep -F 'ExecStart=/usr/bin/omisu-system-sleep-monitor' "$upgrade_to_quattro" >/dev/null
grep -F 'reset-failed omisu-sleep-lock.service' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade repairs the legacy sleep lock unit path"

[[ -e $ROOT/default/systemd/user/omisu-update-user-notify.path ]] &&
  fail "the retired migration watcher is back; pacman writing the migration directory during omisu update would notify about migrations that update is already applying"
grep -rlE '^(Path[A-Za-z]+|DirectoryNotEmpty)=.*/usr/share/omisu/migrations' "$ROOT/default/systemd/user" >/dev/null 2>&1 &&
...
notify_service="$ROOT/default/systemd/user/omisu-migrate-notify.service"
grep -Fx 'ExecStart=/usr/bin/omisu-migrate-notify' "$notify_service" >/dev/null
...
grep -F 'omisu-migrate-notify.service' "$first_run_units" >/dev/null ||
  fail "first-run does not enable the login migration notifier"
grep -F 'omisu-update-user-notify' "$first_run_units" >/dev/null &&
  fail "first-run still enables the retired notifier units"
pass "first-run enables the login-only migration notifier"

fcitx_service="$ROOT/default/systemd/user/omisu-fcitx5.service"
...
oomd_conf="$ROOT/etc/systemd/oomd.conf.d/10-omisu.conf"
