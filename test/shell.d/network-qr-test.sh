...
run_success_case() {
  ...
  output=$(PATH="$tmp/bin:$PATH" "$ROOT/bin/omisu-network-qr" "$@")
...
if PATH="$tmp/bin:$PATH" "$ROOT/bin/omisu-network-qr" wlan0 >"$tmp/enterprise-output" 2>"$tmp/enterprise-error"; then
