...
dns="$ROOT/bin/omisu-dns"
sudoers_file="$ROOT/etc/sudoers.d/omisu-dns"
rule='%wheel ALL=(root) NOPASSWD: /usr/bin/omisu-dns Cloudflare, /usr/bin/omisu-dns Google, /usr/bin/omisu-dns DHCP'
...
grep -Fx 'PACKAGED_PATH=/usr/bin/omisu-dns' "$dns" >/dev/null ||
  fail "omisu-dns elevates the path the sudoers rule names"

# `sudo -l` on its own answers whether a command is permitted, not whether it
# is passwordless, and OmiSu ships a blanket %wheel rule that permits
# everything. Only the long listing prints the matched entry's tags.
grep -E 'sudo -n -l -l' "$dns" >/dev/null ||
  fail "omisu-dns reads the grant from the long sudo listing"
...
# (etc/sudoers.d/omisu-dev-path) prepends a user-writable checkout bin/ to it.
...
grep -Eq '^\s*export PATH=/usr/local/sbin:/usr/local/bin:/usr/bin' "$dns" ||
  fail "omisu-dns pins PATH to trusted system directories when it holds root"
...
[[ $gated == *"export PATH=/usr/local/sbin:/usr/local/bin:/usr/bin"* ]] ||
  fail "omisu-dns gates the trusted-PATH pin on holding root"
...
SH
  ...
  if ! PATH="$poison_dir:$PATH" "${root_runner[@]}" bash "$dns" </dev/null >/dev/null 2>&1; then
    rm -rf "$poison_dir"
    fail "root omisu-dns failed its read-only trusted-PATH probe"
  fi
  if [[ -e $poison_ran ]]; then
    rm -rf "$poison_dir"
    fail "root omisu-dns resolved a bare helper from the front of PATH instead of a trusted system path"
  fi
  rm -rf "$poison_dir"
  pass "root omisu-dns resolves system helpers from a trusted PATH, not the invocation PATH"
else
...
# STUB_GRANTED, the providers etc/sudoers.d/omisu-dns covers on this machine,
# and logs the elevation otherwise. STUB_GRANTED empty stands for an install
# whose omisu-settings predates the file.
...
for provider in Cloudflare Google DHCP; do
  ...
  [[ $elevation == "sudo /usr/bin/omisu-dns $provider" ]] ||
    fail "omisu-dns takes the passwordless sudo grant for $provider without a terminal" "got: $elevation"
done

pass "omisu-dns elevates the stock providers through sudo, not polkit"
...
dev_linked=$(OMISU_PATH="$test_tmp/checkout" elevation_for Cloudflare)
[[ $dev_linked == "sudo /usr/bin/omisu-dns Cloudflare" ]] ||
  fail "omisu-dns elevates the system install wherever OMISU_PATH points" "got: $dev_linked"

custom=$(elevation_for Custom)
[[ $custom == "pkexec /usr/bin/omisu-dns Custom" ]] ||
  fail "omisu-dns leaves Custom on the polkit path, since no sudoers rule covers it" "got: $custom"

# The grant is what makes sudo passwordless, so its absence -- an install still
# on an older omisu-settings, or a user outside %wheel the rule cannot match
...
[[ $ungranted == "pkexec /usr/bin/omisu-dns Cloudflare" ]] ||
  fail "omisu-dns falls back to polkit where the sudoers grant is not installed" "got: $ungranted"

pass "omisu-dns falls back to polkit wherever the grant does not reach"
