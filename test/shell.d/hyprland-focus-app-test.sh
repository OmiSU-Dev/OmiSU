...
#!/bin/bash
if [[ $1 == "clients" ]]; then
  printf '%s\n' "$OMISU_TEST_CLIENTS_JSON"
elif [[ $1 == "dispatch" ]]; then
  printf '%s\n' "$2" >"$OMISU_TEST_FOCUS_DISPATCH"
fi
SH
...
PATH="$mock_bin:$PATH" OMISU_TEST_CLIENTS_JSON="$clients_json" \
  OMISU_TEST_FOCUS_DISPATCH="$dispatch_log" \
  bash "$ROOT/bin/omisu-hyprland-focus-app" '^chromium$'
...
clients_json='[
  ...
  {"address":"0xagent","class":"org.omisu.agent","initialClass":"org.omisu.agent","initialTitle":"kitty"}
]'
PATH="$mock_bin:$PATH" OMISU_TEST_CLIENTS_JSON="$clients_json" \
  OMISU_TEST_FOCUS_DISPATCH="$dispatch_log" \
  bash "$ROOT/bin/omisu-hyprland-focus-app" kitty
...
rm -f "$dispatch_log"
if PATH="$mock_bin:$PATH" OMISU_TEST_CLIENTS_JSON="$clients_json" \
  OMISU_TEST_FOCUS_DISPATCH="$dispatch_log" \
  bash "$ROOT/bin/omisu-hyprland-focus-app" Mail; then
