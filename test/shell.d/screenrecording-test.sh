...
[[ ${OMISU_TEST_NO_WEBCAM:-false} == "true" ]] && exit 0
...
--list-devices)
  ...
  if [[ ${OMISU_TEST_RAW_WEBCAM:-false} != "true" ]]; then
  ...
  if [[ ${OMISU_TEST_DUAL_NODE_WEBCAM:-false} == "true" ]]; then
...
cat >"$stub_bin/omisu-menu-select" <<'SH'
#!/bin/bash

printf '%s\n' "$@" >"$OMISU_TEST_MENU_ARGS"
printf '%s\n' "$3"
SH

cat >"$stub_bin/omisu-capture-screenrecording" <<'SH'
#!/bin/bash

printf '%s\n' "$@" >"$OMISU_TEST_RECORDER_ARGS"
SH

cat >"$stub_bin/omisu-notification-send" <<'SH'
#!/bin/bash

printf '%s\n' "$@" >"$OMISU_TEST_NOTIFICATION_ARGS"
SH
...
export OMISU_TEST_MENU_ARGS="$tmp_dir/menu-args"
export OMISU_TEST_RECORDER_ARGS="$tmp_dir/recorder-args"
export OMISU_TEST_NOTIFICATION_ARGS="$tmp_dir/notification-args"

mapfile -t capture_devices < <(omisu-capture-webcam-list)
...
dual_node=$(OMISU_TEST_DUAL_NODE_WEBCAM=true omisu-capture-webcam-list) ||
...
if "$ROOT/bin/omisu-hw-webcam"; then
...
if OMISU_TEST_RAW_WEBCAM=true "$ROOT/bin/omisu-hw-webcam"; then
...
if OMISU_TEST_NO_WEBCAM=true "$ROOT/bin/omisu-hw-webcam"; then
...
if OMISU_TEST_RAW_WEBCAM=true "$ROOT/bin/omisu-capture-screenrecording-with-webcam"; then
  fail "screenrecording webcam picker rejects output-only video devices"
fi
grep -Fx 'No webcam devices found' "$OMISU_TEST_NOTIFICATION_ARGS" >/dev/null || \
...
"$ROOT/bin/omisu-capture-screenrecording-with-webcam"
...
if ! cmp -s "$OMISU_TEST_MENU_ARGS" "$expected_menu_args"; then
  fail "screenrecording webcam picker passes each webcam as a menu option" "$(diff -u "$expected_menu_args" "$OMISU_TEST_MENU_ARGS")"
fi
...
if ! cmp -s "$OMISU_TEST_RECORDER_ARGS" "$expected_recorder_args"; then
  fail "screenrecording webcam picker starts recording with selected device" "$(diff -u "$expected_recorder_args" "$OMISU_TEST_RECORDER_ARGS")"
fi
pass "screenrecording webcam picker starts recording with selected device"

first_webcam=$(omisu-capture-webcam-list | sed -n '1s/[[:space:]].*//p')
[[ $first_webcam == "/dev/video42" ]] || fail "screenrecording auto-detection selects the first capture device"
grep -F 'WEBCAM_DEVICE=$(omisu-capture-webcam-list' "$ROOT/bin/omisu-capture-screenrecording" >/dev/null || \
...
clients)
  printf '[{"address":"0xabc","title":"%s","size":[%s,%s],"monitor":2}]\n' \
    "${OMISU_TEST_CLIENT_TITLE:-WebcamOverlay}" \
    "${OMISU_TEST_CLIENT_WIDTH:-178}" \
    "${OMISU_TEST_CLIENT_HEIGHT:-200}"
  ;;
monitors)
  printf '[{"id":2,"x":1280,"y":-100,"width":%s,"height":%s,"scale":%s}]\n' \
    "${OMISU_TEST_MONITOR_WIDTH:-2560}" \
    "${OMISU_TEST_MONITOR_HEIGHT:-1600}" \
    "${OMISU_TEST_MONITOR_SCALE:-2}"
  ;;
dispatch)
  printf '%s\n' "$*" >>"$OMISU_TEST_HYPRCTL_ARGS"
  ;;
esac
SH
chmod +x "$stub_bin/hyprctl"

export OMISU_TEST_HYPRCTL_ARGS="$tmp_dir/hyprctl-args"

"$ROOT/bin/omisu-capture-webcam-resize" smaller
...
if ! cmp -s "$OMISU_TEST_HYPRCTL_ARGS" "$expected_hyprctl_args"; then
  fail "webcam resize preserves its aspect ratio and corner anchor" "$(diff -u "$expected_hyprctl_args" "$OMISU_TEST_HYPRCTL_ARGS")"
fi
pass "webcam resize preserves its aspect ratio and corner anchor"

: >"$OMISU_TEST_HYPRCTL_ARGS"
OMISU_TEST_MONITOR_WIDTH=1920 \
  OMISU_TEST_MONITOR_HEIGHT=1080 \
  OMISU_TEST_MONITOR_SCALE=1 \
  OMISU_TEST_CLIENT_WIDTH=128 \
  OMISU_TEST_CLIENT_HEIGHT=144 \
  "$ROOT/bin/omisu-capture-webcam-resize" reset
...
if ! cmp -s "$OMISU_TEST_HYPRCTL_ARGS" "$expected_hyprctl_args"; then
  fail "webcam default size adapts to monitor resolution" "$(diff -u "$expected_hyprctl_args" "$OMISU_TEST_HYPRCTL_ARGS")"
fi
pass "webcam default size adapts to monitor resolution"

: >"$OMISU_TEST_HYPRCTL_ARGS"
OMISU_TEST_CLIENT_TITLE="Other Window" "$ROOT/bin/omisu-capture-webcam-resize" larger

if [[ -s $OMISU_TEST_HYPRCTL_ARGS ]]; then
  fail "webcam resize ignores other windows" "$(cat "$OMISU_TEST_HYPRCTL_ARGS")"
fi
pass "webcam resize ignores other windows"

region_file="$XDG_RUNTIME_DIR/omisu-screenrecord-region"
...
if ! cmp -s "$OMISU_TEST_HYPRCTL_ARGS" "$expected_hyprctl_args"; then
  fail "webcam anchors to the recorded region" "$(diff -u "$expected_hyprctl_args" "$OMISU_TEST_HYPRCTL_ARGS")"
fi
...
for region in "not-a-region" ""; do
  ...
  if ! cmp -s "$OMISU_TEST_HYPRCTL_ARGS" "$expected_hyprctl_args"; then
    fail "webcam falls back to the monitor for an unusable region" "$(diff -u "$expected_hyprctl_args" "$OMISU_TEST_HYPRCTL_ARGS")"
  fi
done
...
for size in small medium large; do
  "$ROOT/bin/omisu-capture-webcam-resize" "$size"
done
...
if ! cmp -s "$OMISU_TEST_HYPRCTL_ARGS" "$expected_hyprctl_args"; then
  fail "webcam sizes stay distinct and inside a narrow region" "$(diff -u "$expected_hyprctl_args" "$OMISU_TEST_HYPRCTL_ARGS")"
fi
pass "webcam sizes stay distinct and inside a narrow region"

rm -f "$region_file"

grep -F 'o.bind("SUPER + ALT + code:34", "Make webcam overlay smaller", "omisu-capture-webcam-resize smaller")' \
  "$ROOT/default/hypr/bindings/utilities.lua" >/dev/null || fail "webcam smaller hotkey is configured"
grep -F 'o.bind("SUPER + ALT + code:35", "Make webcam overlay larger", "omisu-capture-webcam-resize larger")' \
...
grep -F -- '--wayland-app-id="WebcamOverlay-$WEBCAM_SIZE"' \
  "$ROOT/bin/omisu-capture-screenrecording" >/dev/null || fail "webcam uses a dedicated size-specific app id"
