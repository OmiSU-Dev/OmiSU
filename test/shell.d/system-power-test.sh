...
for command in omisu-state omisu-hyprland-window-close-all sleep; do
...
run_power_command() {
  ...
  : >"$call_log"
  PATH="$mock_bin:$PATH" CALL_LOG="$call_log" "$ROOT/bin/omisu-system-$action"
}
...
omisu-state clear re*-required
omisu-hyprland-window-close-all 
sleep 1
EOF
...
for action in reboot shutdown; do
  : >"$call_log"
  if PATH="$mock_bin:$PATH" CALL_LOG="$call_log" FAIL_SYSTEMD_RUN=true "$ROOT/bin/omisu-system-$action"; then
