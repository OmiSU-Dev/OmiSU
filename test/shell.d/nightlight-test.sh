...
SHELL_LOG="$TMPDIR/omisu-shell-log"
...
chmod +x "$TMPDIR/bin/hyprctl" "$TMPDIR/bin/pgrep" "$TMPDIR/bin/omisu-shell"

nightlight_cli() {
  PATH="$TMPDIR/bin:$PATH" \
  HYPRSUNSET_STATE="$STATE" \
  OMISU_SHELL_LOG="$SHELL_LOG" \
    "$ROOT/bin/omisu-toggle-nightlight" "$@"
}
...
if rg -q 'omisu.indicators' "$ROOT/bin/omisu-toggle-nightlight"; then
