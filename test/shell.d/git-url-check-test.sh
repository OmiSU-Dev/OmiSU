...
# omisu-git-url-check decides which URLs omisu-theme-install and
# omisu-plugin-add are willing to hand to `git clone`. git resolves a remote
...
check() {
  "$ROOT/bin/omisu-git-url-check" "$@" 2>&1
}
...
for url in "ext::sh -c id" "fd::0,1" "gcrypt::x" "a+b::x" "a.b::x" "a-b::x" "1::x"; do
  output=$(check "$url") &&
    fail "omisu-git-url-check refuses the transport helper '$url'" "$output"
  grep -qF "names a git option or transport helper" <<<"$output" ||
    fail "omisu-git-url-check names the helper rejection for '$url'" "$output"
done
...
for url in "ext://sh -c id" "fd://17" "gcrypt://example.com/x" "zzz://a" "ZZZ://a" "HTTPS://github.com/a/b"; do
  output=$(check "$url") &&
    fail "omisu-git-url-check refuses the '$url' transport" "$output"
  grep -qF "which OmiSu does not clone from" <<<"$output" ||
    fail "omisu-git-url-check names the transport rejection for '$url'" "$output"
done
...
for url in "-x" "--upload-pack=touch /tmp/pwned" "-oProxyCommand=x"; do
  output=$(check "$url") &&
    fail "omisu-git-url-check refuses the option '$url'" "$output"
done

pass "a URL shaped like a git option is refused"

output=$(check "") && fail "omisu-git-url-check refuses an empty URL" "$output"
output=$(check) && fail "omisu-git-url-check refuses a missing URL" "$output"
...
for url in \
  "https://github.com/acme/omisu-weather.git" \
  ...
  "repo"; do
  output=$(check "$url") ||
    fail "omisu-git-url-check accepts the legitimate URL '$url'" "$output"
done
