...
mkdir -p "$TEST_HOME/.config/omisu/agents"
cat >"$TEST_HOME/.config/omisu/agents/fireworks.json" <<'EOF'
{
  "accountId": "example",
  "fundedAmount": 20,
  "fundedAt": "2026-07-01"
...
no_key=$(HOME="$TEST_HOME" XDG_CONFIG_HOME="$TEST_HOME/.config" XDG_DATA_HOME="$TEST_HOME/.local/share" \
  FIREWORKS_API_KEY="" FIREWORKS_AUTH_PATH="$TEST_HOME/missing.ini" "$ROOT/bin/omisu-agent-usage-fireworks")
...
result=$(python3 - "$ROOT/bin/omisu-agent-usage-fireworks" "$auth_file" "$TEST_HOME/.config" "$TEST_HOME/.local/share" <<'PY'
