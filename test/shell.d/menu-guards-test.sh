...
const items = {
  'setup.default.browser.brave': { id: 'setup.default.browser.brave', when: 'omisu-pkg-present brave-bin', checked: '[[ "$(omisu-default-browser)" == "brave" ]]' },
  'setup.default.browser.zen': { id: 'setup.default.browser.zen', when: 'omisu-pkg-present zen-browser-bin', checked: '[[ "$(omisu-default-browser)" == "zen" ]]' },
  'install.browser.zen': { id: 'install.browser.zen', disabled: 'omisu-pkg-present zen-browser-bin' },
  'plain': { id: 'plain', label: 'No guards' }
}
const script = menu.guardScript(items)
const browserSlot = `\${__omisu_read_${menu.guardReaders.indexOf('omisu-default-browser')}}`

assert(
  script.includes('if { omisu-pkg-present brave-bin; } >/dev/null 2>&1; then echo setup.default.browser.brave:w:1; else echo setup.default.browser.brave:w:0; fi'),
...
assert(
  script.includes('if { omisu-pkg-present zen-browser-bin; } >/dev/null 2>&1; then echo install.browser.zen:d:1; else echo install.browser.zen:d:0; fi'),
...
assertEqual(
  (script.match(/^__omisu_read_\d+=\$\(omisu-default-browser /gm) || []).length,
  1,
  'guard script reads a value command once for the whole batch'
)
assert(
  script.includes(`[[ "${browserSlot}" == "brave" ]]`) && !script.includes('"$(omisu-default-browser)"'),
  'guard script substitutes the captured answer into the expression'
)
assert(
  script.indexOf('__omisu_read_') < script.indexOf('if { omisu-pkg-present'),
...
const untouched = menu.guardScript({
  a: { id: 'a', when: 'command -v omisu-dns' },
  b: { id: 'b', when: '[[ "$(OMISU_PATH=/usr/share/omisu omisu-channel-current)" == "stable" ]]' },
  c: { id: 'c', when: '(( $(omisu-default-browser | wc -l) == 1 ))' }
})
assert(
  untouched.includes('command -v omisu-dns')
    && untouched.includes('$(OMISU_PATH=/usr/share/omisu omisu-channel-current)')
    && untouched.includes('$(omisu-default-browser | wc -l)'),
  'guard script leaves every form but the plain substitution to run the real command'
)
assert(
  !/^__omisu_read_/m.test(untouched),
...
const fs = require('fs')
const defaultItems = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/omisu/omisu-menu.jsonc'), 'utf8'))
const guardText = defaultItems.map(item => `${item.when}\n${item.checked}\n${item.disabled}`).join('\n')
const repeated = [...new Set(
  (guardText.match(/\$\((omisu-[a-z0-9-]+)\)/g) || []).map(match => match.slice(2, -1))
...
for helper in omisu-pkg-present omisu-pkg-missing; do
...
for helper in omisu-cmd-present omisu-cmd-missing; do
  for case in "${cmd_cases[@]}"; do
    read -r -a argv <<<"$case"
    assert_helper_agrees "guard prelude resolves commands as the real helper does" "$helper" "${argv[@]}"
  done
done
pass "guard prelude resolves commands as omisu-cmd-present and omisu-cmd-missing do"
...
reader_script=$(node -e '
  ...
  process.stdout.write(menu.guardScript({
    hit: { id: "hit", checked: "[[ \"$(omisu-dns)\" == \"Cloudflare\" ]]" },
    miss: { id: "miss", checked: "[[ \"$(omisu-dns)\" == \"Google\" ]]" }
  }))
')
reader_result=$(bash -c '
omisu-dns() { printf "Cloudflare\n"; }
export -f omisu-dns
'"$reader_script")
...
# Update > Extra Themes runs omisu-theme-update, which pulls the themes that
...
# omisu-theme-extras today; the shapes below are what would tell us if one
# of them stopped.
themes_guard=$(node -e '
  const fs = require("fs")
  const path = require("path")
  const menu = require(path.join(process.env.ROOT, "shell/plugins/menu/MenuModel.js"))
  const items = menu.parseMenuJsonc(fs.readFileSync(path.join(process.env.ROOT, "default/omisu/omisu-menu.jsonc"), "utf8"))
...
assert_themes_guard_agrees() {
  ...
  [[ -n $(HOME="$home" PATH="$ROOT/bin:$stub_dir:$PATH" "$ROOT/bin/omisu-theme-update" 2>/dev/null) ]] || updated=1
...
mkdir -p "$themes_home/empty/.config/omisu/themes"
mkdir -p "$themes_home/copied/.config/omisu/themes/handmade"
mkdir -p "$themes_home/cloned/.config/omisu/themes/tokyo-night/.git"
mkdir -p "$themes_home/linked/.config/omisu/themes" "$themes_home/checkout/.git"
ln -s "$themes_home/checkout" "$themes_home/linked/.config/omisu/themes/in-progress"
mkdir -p "$themes_home/worktree/.config/omisu/themes/branch"
printf 'gitdir: /elsewhere\n' >"$themes_home/worktree/.config/omisu/themes/branch/.git"

for shape in missing:1 empty:1 copied:1 cloned:0 linked:1 worktree:1; do
  assert_themes_guard_agrees \
    "Extra Themes shows exactly when omisu-theme-update has something to pull" \
    "$themes_home/${shape%:*}" "${shape#*:}"
done
pass "Extra Themes shows exactly when omisu-theme-update has something to pull"
...
# directory.
many="$themes_home/many/.config/omisu/themes"
mkdir -p "$many/tokyo night/.git" "$many/zen/.git" "$many/handmade"
ln -s "$themes_home/checkout" "$many/in-progress"

listed=$(HOME="$themes_home/many" LC_ALL=C "$ROOT/bin/omisu-theme-extras")
[[ $listed == "$many/tokyo night"$'\n'"$many/zen" ]] ||
  fail "omisu-theme-extras lists every clone and nothing else" "got: $listed"
pass "omisu-theme-extras lists every clone and nothing else"
...
HOME="$themes_home/many" LC_ALL=C GIT_CALLS="$git_calls" PATH="$ROOT/bin:$stub_dir:$PATH" \
  "$ROOT/bin/omisu-theme-update" >/dev/null 2>&1
pulled=$(<"$git_calls")
[[ $pulled == "<-C><$many/tokyo night><pull>"$'\n'"<-C><$many/zen><pull>" ]] ||
  fail "omisu-theme-update pulls each clone by its whole path" "got: $pulled"
pass "omisu-theme-update pulls each clone by its whole path"
