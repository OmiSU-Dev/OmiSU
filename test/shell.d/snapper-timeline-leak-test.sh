...
PATH="$fake_bin:$PATH" \
OMISU_SNAPPER_CONFIG_PATH="$snapper_config" \
...
# omisu-migrate runs under set -e, so a batch that dies on a DBus timeout
...
# omisu-migrate writes the completion marker even when the drain gave up, so
...
TEST_LOG="$test_tmp/calls.log" \
PATH="$fake_bin:$PATH" \
OMISU_SNAPPER_CONFIG_PATH="$test_tmp/missing" \
  bash -euo pipefail "$leak_migration" >/dev/null
