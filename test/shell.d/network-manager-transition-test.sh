...
! grep -F 'systemd-networkd' "$dns" >/dev/null || fail "omisu-dns no longer restarts systemd-networkd"
grep -F 'NetworkManager/conf.d/20-omisu-dns.conf' "$dns" >/dev/null
...
if grep -F 'nmcli general reload conf,dns-full' "$dns" >/dev/null; then
  fail "omisu-dns must not push DNS before reapplying active profiles"
fi
pass "omisu-dns configures DNS through NetworkManager"
...
grep -F 'omisu-networkd-retired' "$hardware_network" >/dev/null
pass "hardware setup retires archinstall networkd state"

grep -F 'OMISU_UPGRADE_TO_QUATTRO_LIVE' "$migration" >/dev/null
