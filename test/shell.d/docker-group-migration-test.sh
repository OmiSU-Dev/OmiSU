...
#
# The real omisu-remove-security-sudoless-docker and omisu-state run here
...
home="$test_dir/home"
omisu_path="$test_dir/omisu"
stub_bin="$test_dir/bin"
mkdir -p "$home/.local/share/applications" "$omisu_path/applications" "$stub_bin"

printf 'NEW-LAUNCHER\n' >"$omisu_path/applications/Docker.desktop"
...
# must still NOT reboot (it defers to omisu-update-restart), so neither should
# be reached.
...
touch "${REBOOT_CALLED:?}"
STUB
chmod +x "$stub_bin/id" "$stub_bin/sudo" "$stub_bin/gpasswd" "$stub_bin/gum" "$stub_bin/omisu-system-reboot"
...
run_migration() {
  ...
  HOME="$home" OMISU_PATH="$omisu_path" USER="tester" STUB_GROUPS="$1" \
