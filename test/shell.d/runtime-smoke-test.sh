...
shell_ipc() {
  OMISU_PATH="$test_root" "$ROOT/bin/omisu-shell" "$@"
}

shell_ipc_quiet() {
  OMISU_PATH="$test_root" "$ROOT/bin/omisu-shell" -q "$@"
}
...
# Every plugin under ~/.config/omisu/plugins hot-reloads, whoever wrote it.
hot_reload_id="acme.hot-reload"
hot_reload_dir="$test_home/.config/omisu/plugins/$hot_reload_id"
...
cat >"$stub_bin/omisu-update-available" <<'SH'
#!/bin/bash
echo "OmiSu update available (test)"
exit 0
SH
chmod +x "$stub_bin/omisu-update-available"
...
jq -e '
  map(.id) as $ids |
  all(["omisu.menu", "omisu.notifications", "omisu.clock", "omisu.osd"][]; $ids | index(.)) and
...
[[ $(shell_ipc shell summon omisu.emojis "{}") == "ok" ]] ||
  fail_with_log "calls to a cloned source id do not reach its enabled clone"
shell_ipc_quiet shell hide omisu.emojis >/dev/null
...
[[ $(shell_ipc shell summon omisu.menu '{"menu":"apps"}') == "ok" ]] || fail_with_log "shell IPC summons menu apps overlay"
shell_ipc_quiet shell hide omisu.menu >/dev/null
...
shell_ipc_quiet omisu.system-update refresh >/dev/null 2>&1 || true
sleep 0.8

default_ids=$(jq -c '(.bar.layout.left + .bar.layout.center + .bar.layout.right) | map(.id // .)' "$ROOT/config/omisu/shell.json")
visible_default_ids='[
  "omisu.menu",
  "omisu.workspaces",
  "omisu.clock",
...
jq -e '
  ...
  ($center | index("omisu.weather")) != null and
  ($center | index("omisu.system-update")) != null and
  ($center | index("omisu.indicators")) != null and
  (($center | index("omisu.weather")) < ($center | index("omisu.system-update"))) and
  (($center | index("omisu.system-update")) < ($center | index("omisu.indicators")))
' <<<"$geometry" >/dev/null || {
  printf 'Geometry:\n' >&2
  jq . <<<"$geometry" >&2
...
for panel_id in omisu.audio omisu.bluetooth omisu.monitor omisu.network omisu.power; do
...
HOME="$test_home" OMISU_PATH="$test_root" PATH="$ROOT/bin:$PATH" "$ROOT/bin/omisu-plugin-disable" omisu.audio

for _ in {1..80}; do
  shell_config=$(shell_ipc shell listShellConfig 2>/dev/null || true)
  geometry=$(shell_ipc shell debugBarGeometry 2>/dev/null || true)
  if jq -e 'all(.bar.layout.right[]; (.id // .) != "omisu.audio")' <<<"$shell_config" >/dev/null 2>&1 && \
     jq -e 'all(.[]; .id != "omisu.audio")' <<<"$geometry" >/dev/null 2>&1; then
    break
  fi
  if ! kill -0 "$QS_PID" 2>/dev/null; then
...
jq -e 'all(.bar.layout.right[]; (.id // .) != "omisu.audio")' <<<"$shell_config" >/dev/null || {
  printf 'Shell config after reload:\n%s\n' "$shell_config" | jq . >&2
  fail_with_log "plugin disable reloads shell config"
}

jq -e 'all(.[]; .id != "omisu.audio")' <<<"$geometry" >/dev/null || {
...
bar_put() {
  HOME="$test_home" OMISU_PATH="$test_root" PATH="$ROOT/bin:$PATH" "$ROOT/bin/omisu-bar" put "$@"
}
...
bar_put omisu.keyboard-layout --after omisu.clock >/dev/null
for _ in {1..80}; do
  [[ $(center_ids) == *omisu.keyboard-layout* ]] && break
...
jq -e '
  ...
  | ($ids | index("omisu.clock")) as $clock
  | ($ids | index("omisu.keyboard-layout")) as $widget
...
placed=$(center_ids)
bar_put omisu.keyboard-layout --section right >/dev/null
sleep 0.5
[[ $(center_ids) == "$placed" ]] ||
  fail_with_log "bar put left a widget already on the bar alone (was $placed, now $(center_ids))"
jq -e 'all(.bar.layout.right[]; (.id // .) != "omisu.keyboard-layout")' \
  <<<"$(shell_ipc shell listShellConfig)" >/dev/null ||
