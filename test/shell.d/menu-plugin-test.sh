...
# The picker reads the plugin list from omisu-plugin-list and hands what it
...
cat >"$STUB_DIR/omisu-plugin-list" <<'STUB'
#!/bin/bash
cat "$FAKE_PLUGINS"
STUB

for command in omisu-plugin-enable omisu-plugin-disable; do
...
cat >"$STUB_DIR/omisu-menu-select" <<'STUB'
#!/bin/bash
cat >"$FAKE_ROWS"
printf '%s\n' "$FAKE_PICK"
STUB

cat >"$STUB_DIR/omisu-notification-send" <<'STUB'
#!/bin/bash
printf 'notification: %s\n' "$*" >>"$FAKE_CALLS"
STUB

cat >"$STUB_DIR/omisu-launch-floating-terminal-with-presentation" <<'STUB'
#!/bin/bash
printf 'terminal: %s\n' "$*" >>"$FAKE_CALLS"
STUB
...
pick() {
  ...
  HOME="$TMPDIR/home" \
    ...
    FAKE_PICK="$choice" \
    "$ROOT/bin/omisu-menu-plugin" "$verb" >/dev/null 2>&1
...
[
  {"id": "omisu.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": true},
...
[[ $CALLS == *"omisu-plugin-remove tester.clock"* ]] \
...
[[ $CALLS == *"omisu-plugin-enable acme.weather"* ]] \
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "omisu.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": true, "active": false, "canDisable": true, "firstParty": true},
  {"id": "acme.weather", "name": "Weather", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": false}
]
JSON

pick clone "$(printf 'Clock\tomisu.clock')"
[[ $ROWS == *"Clock"* && $ROWS != *"Weather"* ]] ||
  fail "clone picker offers only built-in plugins" "$ROWS"
pass "clone picker offers built-in plugins"
[[ $CALLS == *"terminal: omisu-plugin-clone omisu.clock --edit"* ]] ||
...
[
  {"id": "omisu.clock", "name": "Clock", "kinds": ["bar-widget"], "enabled": true, "active": false, "canDisable": true, "firstParty": true},
  {"id": "tester.clock", "name": "My Clock", "kinds": ["bar-widget"], "enabled": false, "active": false, "canDisable": true, "firstParty": false, "clonedFrom": "omisu.clock"}
]
JSON
...
[[ $CALLS == *"omisu-plugin-enable acme.fancy"* && $CALLS != *"--section"* ]] \
...
pick disable "$(printf 'Clock\tomisu.clock')"
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "omisu.bar", "name": "Bar", "kinds": ["bar"], "enabled": false, "active": false, "canDisable": false, "firstParty": true},
  {"id": "tester.neon-bar", "name": "Neon Bar", "kinds": ["bar"], "enabled": true, "active": true, "canDisable": false, "firstParty": false}
]
JSON

pick enable "$(printf 'Bar\tomisu.bar')"
...
[[ $CALLS == *"omisu-plugin-enable omisu.bar"* ]] \
...
cat >"$TMPDIR/plugins.json" <<'JSON'
[
  {"id": "omisu.bar", "name": "Bar", "kinds": ["bar"], "enabled": true, "active": true, "canDisable": false, "firstParty": true}
]
JSON
