...
if [[ -n ${TAKEN_USERS:-} ]]; then
  omisu_username_taken() { [[ " $TAKEN_USERS " == *" $1 "* ]]; }
fi
...
((OMISU_FORM_BACK == 1)) || fail "Esc reports status 1"
((OMISU_FORM_SIGNAL == 130)) || fail "Ctrl+C reports status 130"
[[ $(printf '%s\n' "$OMISU_KEYBOARD_LAYOUTS" | head -n 1) == "English (US)|us" ]] ||
...
run_prompt omisu_prompt_keyboard "0:German"
...
run_prompt omisu_prompt_keyboard "1:"
assert_status "$OMISU_FORM_BACK" "keyboard prompt reports Esc as back"
assert_returned "keyboard prompt survives Esc under set -e"

run_prompt omisu_prompt_keyboard "130:"
assert_status "$OMISU_FORM_SIGNAL" "keyboard prompt reports Ctrl+C as the caller's signal"
...
TAKEN_USERS=dhh run_prompt omisu_prompt_username "0:Not A Username" "0:root" "0:dhh" "0:david"
...
run_prompt omisu_prompt_username "1:"
assert_status "$OMISU_FORM_BACK" "username prompt reports Esc as back"
assert_returned "username prompt survives Esc under set -e"

run_prompt omisu_prompt_username "130:"
assert_status "$OMISU_FORM_SIGNAL" "username prompt reports Ctrl+C as the caller's signal"
...
run_prompt omisu_prompt_password "0:one" "0:two" "0:" "0:" "0:s3cret" "0:s3cret"
...
run_prompt omisu_prompt_password "0:s3cret" "1:"
assert_status "$OMISU_FORM_BACK" "password prompt reports Esc on the confirmation as back"
assert_returned "password confirmation survives Esc under set -e"

run_prompt omisu_prompt_password "0:s3cret" "130:"
assert_status "$OMISU_FORM_SIGNAL" "password prompt reports Ctrl+C on the confirmation as the caller's signal"
...
run_prompt omisu_prompt_identity "0:" "0:"
...
run_prompt omisu_prompt_identity "0:David" "1:"
assert_status "$OMISU_FORM_BACK" "identity prompt reports Esc on the email as back"
assert_returned "identity prompt survives Esc under set -e"
pass "identity prompt propagates Esc from its second field"

# Hostname

run_prompt omisu_prompt_hostname "0:-nope-" "0:workshop"
...
run_prompt omisu_prompt_hostname "0:"
assert_status 0 "hostname prompt accepts an empty hostname"
[[ $(field hostname) == "$OMISU_HOSTNAME_DEFAULT" ]] || fail "hostname prompt falls back to the default hostname"
pass "hostname prompt rejects malformed names and defaults an empty one"

run_prompt omisu_prompt_hostname "1:"
assert_status "$OMISU_FORM_BACK" "hostname prompt reports Esc as back"
assert_returned "hostname prompt survives Esc under set -e"
pass "hostname prompt propagates Esc without dying under set -e"

# Timezone

TZ_GUESS=Europe/Copenhagen run_prompt omisu_prompt_timezone "0:Europe/Copenhagen"
...
run_prompt omisu_prompt_timezone "0:America/Chicago"
...
run_prompt omisu_prompt_timezone "0:"
...
run_prompt omisu_prompt_timezone "1:"
assert_status "$OMISU_FORM_BACK" "timezone prompt reports Esc as back"
assert_returned "timezone prompt survives Esc under set -e"

TZ_GUESS=Europe/Copenhagen run_prompt omisu_prompt_timezone "130:"
assert_status "$OMISU_FORM_SIGNAL" "timezone prompt reports Ctrl+C as the caller's signal"
