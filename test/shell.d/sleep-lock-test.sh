...
sleep_lock="$ROOT/bin/omisu-system-sleep-lock"
tmpdir=$(mktemp -d)
...
printf '%s\n' "\$*" >>"$notify_log"
SH
  chmod +x "$mock_bin/omisu-notification-send"
}
...
mock_clamshell() {
  cat >"$mock_bin/omisu-hyprland-monitor-clamshell" <<SH
#!/bin/bash

echo clamshell >>"\$CALL_LOG"
sleep ${1:-0}
SH
  chmod +x "$mock_bin/omisu-hyprland-monitor-clamshell"
}
