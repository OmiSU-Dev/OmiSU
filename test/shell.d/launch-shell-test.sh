...
# Each launch consumes the next status from OMISU_TEST_QS_STATUSES; "run"
...
printf 'watcher=%s popup=%s\n' \
  "${QS_DISABLE_FILE_WATCHER:-unset}" "${QS_NO_RELOAD_POPUP:-unset}" >>"$OMISU_TEST_QS_ENV_LOG"

launches=$(wc -l <"$OMISU_TEST_QS_LOG")
status=$(awk -v n="$launches" 'NR == n { print; found = 1 } END { if (!found) print "0" }' <<<"$OMISU_TEST_QS_STATUSES")

if [[ $status == "run" ]]; then
  trap 'touch "$OMISU_TEST_QS_TERMINATED"; exit 143' TERM
...
[[ ${OMISU_TEST_COMPOSITOR_GONE:-0} == 1 ]] && exit 4

# Refuse the first OMISU_TEST_HYPRCTL_MISSES queries, then answer.
if (( ${OMISU_TEST_HYPRCTL_MISSES:-0} > 0 )); then
  misses=$(cat "$OMISU_TEST_HYPRCTL_MISS_COUNT" 2>/dev/null || printf '0')
  if (( misses < OMISU_TEST_HYPRCTL_MISSES )); then
    printf '%s\n' "$(( misses + 1 ))" >"$OMISU_TEST_HYPRCTL_MISS_COUNT"
    exit 4
  fi
fi
...
launch_shell() {
  ...
  PATH="$fake_bin:$PATH" \
  OMISU_PATH="$shell_root" \
  OMISU_TEST_QS_LOG="$qs_log" \
  OMISU_TEST_QS_ENV_LOG="$qs_env_log" \
  OMISU_TEST_QS_STATUSES="$1" \
  OMISU_TEST_COMPOSITOR_GONE="${2:-0}" \
  OMISU_TEST_LOGGER_LOG="$logger_log" \
  OMISU_TEST_QS_TERMINATED="$qs_terminated" \
  OMISU_TEST_HYPRCTL_MISSES="${3:-0}" \
  OMISU_TEST_HYPRCTL_MISS_COUNT="$hyprctl_misses" \
    timeout 30 "$ROOT/bin/omisu-launch-shell"
}
...
grep -F -- "-n -p $shell_root/shell" "$qs_log" >/dev/null || fail "the shell launches from OMISU_PATH"
...
OMISU_TEST_QS_LOG="$qs_log" \
OMISU_TEST_QS_ENV_LOG="$qs_env_log" \
OMISU_TEST_QS_STATUSES=$'255\n0' \
OMISU_TEST_COMPOSITOR_GONE=0 \
OMISU_TEST_LOGGER_LOG="$logger_log" \
OMISU_TEST_QS_TERMINATED="$qs_terminated" \
  "$ROOT/bin/omisu-launch-shell" &
launch_pid=$!
