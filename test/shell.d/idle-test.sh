...
HOME="$test_home" "$ROOT/bin/omisu-toggle-idle" stay-awake >/dev/null
[[ -f $test_home/.local/state/omisu/indicators/stay-awake ]] || fail "Stay Awake toggle persists enabled state"

HOME="$test_home" "$ROOT/bin/omisu-toggle-idle" allow-idle >/dev/null
[[ ! -f $test_home/.local/state/omisu/indicators/stay-awake ]] || fail "Stay Awake toggle persists disabled state"

if rg -q 'omisu-shell' "$ROOT/bin/omisu-toggle-idle"; then
