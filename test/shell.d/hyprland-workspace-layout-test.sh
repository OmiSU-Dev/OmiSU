...
cat >"$stub_dir/omisu-notification-send" <<'EOF'
#!/bin/bash
:
EOF
chmod +x "$stub_dir/omisu-notification-send"

HOME="$home_dir" HYPRCTL_LOG="$log_file" PATH="$stub_dir:$PATH" \
  "$ROOT/bin/omisu-hyprland-workspace-layout-toggle"

layout_file="$home_dir/.local/state/omisu/workspace-layouts/3.lua"
...
if HOME="$home_dir" HYPRCTL_LOG="$log_file" HYPRCTL_BROKEN=1 PATH="$stub_dir:$PATH" \
  "$ROOT/bin/omisu-hyprland-workspace-layout-toggle" 2>/dev/null; then
  fail "workspace layout toggle exits nonzero without a workspace id"
fi
[[ -f "$home_dir/.local/state/omisu/workspace-layouts/null.lua" ]] &&
...
HOME="$home_dir" OMISU_PATH="$ROOT" lua <<'LUA'
local rules = {}
