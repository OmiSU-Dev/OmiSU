...
jq -e '
  any(.[]; .id == "omisu.menu" and .visible == true and .width > 0 and .height > 0) and
  any(.[]; .id == "omisu.clock" and .visible == true and .width > 0 and .height > 0)
' <<<"$geometry" >/dev/null || {
  printf 'Geometry:\n' >&2
  jq . <<<"$geometry" >&2
...
screenshot=$(
  OMISU_PATH="$test_root" \
  OMISU_SCREENSHOT_DIR="$screenshot_dir" \
  HOME="$test_home" \
  PATH="$stub_bin:$ROOT/bin:$PATH" \
    "$ROOT/bin/omisu" capture screenshot fullscreen save 2>"$screenshot_err" | tail -n 1
)
