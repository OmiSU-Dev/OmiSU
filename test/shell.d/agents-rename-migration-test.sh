...
cat >"$test_dir/bin/omisu-agent-usage-update" <<'STUB'
#!/bin/bash
...
mkdir -p "$home/.config/omisu" "$home/.cache/omisu/model-usage"
cat >"$config" <<'JSON'
{
  "bar": {
    "layout": {
      "center": ["omisu.model-usage"],
      "right": [
        { "id": "omisu.tray" },
        { "id": "omisu.model-usage", "syncMode": "On", "syncDir": "~/Sync/agent-usage" }
      ]
    }
  },
  "disabledPlugins": ["omisu.model-usage", "omisu.weather"]
}
JSON

run_migration

[[ $(jq -c '.bar.layout.right[1]' "$config") == '{"id":"omisu.agents","syncMode":"On","syncDir":"~/Sync/agent-usage"}' ]] ||
