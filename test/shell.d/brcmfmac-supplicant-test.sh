...
cat >"$stub_bin/omisu-state" <<'SH'
#!/bin/bash

printf 'omisu-state' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
SH
...
run_migration() {
  ...
  WIFI_ID="$wifi_id" T2_HARDWARE="$t2" PATH="$stub_bin:$PATH" TEST_LOG="$calls" \
    OMISU_BRCMFMAC_DMI_VENDOR="$test_tmp/dmi/sys_vendor" \
    OMISU_BRCMFMAC_CONF="$conf" \
...
grep -Fq $'omisu-state\tset\treboot-required' "$calls" ||
