...
ShellRoot {
  ...
  readonly property string resultPath: Quickshell.env("OMISU_QML_TEST_RESULT")
  ...
  function runChecks() {
    var scan = ""
    scan += block("firstparty", "/first/widgets/clock", manifest("omisu.first-widget", ["bar-widget"], { barWidget: "Widget.qml" }))
    scan += block("firstparty", "/first/bar", manifest("omisu.bar", ["bar"], { bar: "Bar.qml" }))
    scan += block("firstparty", "/first/panels/grouped", manifest("omisu.grouped-panel", ["panel"], { panel: "Panel.qml" }))
    scan += block("firstparty", "/first/hybrid", manifest("omisu.hybrid", ["menu", "bar-widget"], { menu: "Menu.qml", barWidget: "Widget.qml" }))
    ...
    localWidget.omisu = { clonedFrom: "omisu.first-widget" }
    ...
    localWeather.omisu = { clonedFrom: "omisu.weather" }
    ...
    root.assertTrue(registry.installedPlugins["omisu.first-widget"].__isFirstParty === true, "first-party manifests are stamped")
    root.assertTrue(registry.installedPlugins["third.panel"].__isFirstParty === false, "third-party manifests are stamped")
    root.assertEqual(registry.installedPlugins["omisu.grouped-panel"].__sourceDir, "/first/panels/grouped", "grouped plugin source paths are preserved")
    ...
    root.assertTrue(!has("omisu.reserved"), "third-party omisu namespace ids are rejected")
    ...
    root.assertTrue(registry.isEnabled("omisu.first-widget"), "first-party plugins are implicitly enabled")
    root.assertTrue(registry.isEnabled("omisu.bar"), "built-in bar option is active by default")
    ...
    root.assertEqual(registry.resolveEnabledId("omisu.first-widget"), "omisu.first-widget", "inactive clones do not replace their source id")
    ...
    root.assertTrue(!registry.isEnabled("omisu.bar"), "selecting third-party bar options deactivates built-in bar")
    ...
    root.assertTrue(registry.isEnabled("omisu.bar"), "built-in bar option returns after reset")
    ...
    root.config = {
      ...
      bar: {
        layout: {
          left: [{ id: "omisu.workspaces" }, { id: "omisu.menu" }],
          center: [{ id: "omisu.weather" }, { id: "omisu.clock" }],
          right: [{ id: "omisu.tray" }]
        }
      },
      plugins: []
    ...
    root.assertDeepEqual(
      ...
      [{ id: "omisu.workspaces" }, { id: "third.widget" }, { id: "omisu.menu" }],
    ...
    root.assertDeepEqual(
      ...
      [{ id: "omisu.weather" }, { id: "third.center-widget" }, { id: "omisu.clock" }],
    ...
    root.assertDeepEqual(
      ...
      [{ id: "omisu.tray" }, { id: "third.right-widget" }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "omisu.weather" }], right: [] } },
      plugins: []
    }
    root.assertTrue(
      !registry.setEnabled("third.center-widget", true, { after: "omisu.first-widget" }),
      "enabling against a widget the bar does not carry is refused"
    )
    root.assertEqual(
      registry.lastEnableError,
      "could not find target widget omisu.first-widget",
      "a refused enable names the target it could not find"
    )
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "omisu.weather" }], "a refused enable places nothing")
    root.assertEqual(
      registry.putBarWidget("third.center-widget", { after: "omisu.first-widget" }),
      "",
      "put accepts a placement target the bar does not carry"
    )
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "omisu.weather" }, { id: "third.center-widget" }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "local.first-widget" }, { id: "omisu.weather" }], right: [] } },
      plugins: []
    }
    root.assertEqual(
      registry.putBarWidget("third.center-widget", { after: "omisu.first-widget" }),
    ...
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "local.first-widget" }, { id: "third.center-widget" }, { id: "omisu.weather" }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "local.weather" }, { id: "omisu.clock" }], right: [] } },
      plugins: []
    }
    root.assertEqual(
      registry.putBarWidget("third.center-widget", { after: "omisu.first-widget" }),
    ...
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "local.weather" }, { id: "third.center-widget" }, { id: "omisu.clock" }],
    ...
    root.assertEqual(registry.putBarWidget("omisu.first-widget", {}), "", "put accepts a widget whose clone is on the bar")
    root.assertDeepEqual(
      root.config.bar.layout.center,
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [{ id: "omisu.first-widget", size: 4 }], right: [] } },
      plugins: []
    }
    ...
    root.assertEqual(registry.resolveEnabledId("omisu.first-widget"), "local.first-widget", "enabled clones receive calls made to their source id")
    registry.setEnabled("omisu.first-widget", true)
    root.assertDeepEqual(
      root.config.bar.layout.center,
      [{ id: "omisu.first-widget", size: 4 }],
    ...
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: ["omisu.first-widget"], right: [] } },
      plugins: []
    }
    registry.setEnabled("local.first-widget", true)
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "local.first-widget" }], "clone replacement normalizes string entries")
    registry.setEnabled("local.first-widget", false)
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "omisu.first-widget" }], "clone restoration normalizes string entries")

    root.config = {
      version: 1,
      bar: { layout: { left: [{ id: "omisu.hybrid" }], center: [], right: [] } },
      plugins: []
    }
    ...
    root.assertDeepEqual(root.config.disabledPlugins, ["omisu.hybrid"], "enabling a multi-kind clone disables the source")
    registry.setEnabled("local.hybrid", false)
    root.assertDeepEqual(root.config.bar.layout.left, [{ id: "omisu.hybrid" }], "disabling a multi-kind clone restores its widget")
    ...
    root.assertDeepEqual(root.config.disabledPlugins, ["omisu.grouped-panel"], "enabling an ordinary clone disables the source")
    ...
    root.config = {
      ...
      bar: { layout: { left: [], center: [], right: [] } },
      plugins: [],
      disabledPlugins: ["omisu.grouped-panel"]
    }
    registry.setEnabled("local.grouped-panel", true)
    root.assertDeepEqual(root.config.disabledPlugins, ["omisu.grouped-panel"], "cloning an already-disabled source keeps it disabled")
    root.assertTrue(root.config.cloneSourceRestores === undefined, "an already-disabled source is not marked for restoration")
    registry.setEnabled("local.grouped-panel", false)
    root.assertDeepEqual(root.config.disabledPlugins, ["omisu.grouped-panel"], "disabling the clone preserves the source's prior disabled state")
    ...
    root.config = { version: 1, bar: { layout: { left: [], center: [], right: [] } }, plugins: [] }
    registry.setEnabled("omisu.grouped-panel", false)
    root.assertDeepEqual(root.config.disabledPlugins, ["omisu.grouped-panel"], "disabling a first-party plugin records it")
    root.assertTrue(!registry.isEnabled("omisu.grouped-panel"), "a recorded first-party plugin is disabled")
    root.assertDeepEqual(root.config.plugins, [], "disabling a first-party plugin leaves the plugins array alone")
    registry.setEnabled("omisu.grouped-panel", true)
    root.assertTrue(root.config.disabledPlugins === undefined, "re-enabling drops the disabled record entirely")
    root.assertTrue(registry.isEnabled("omisu.grouped-panel"), "a first-party plugin returns to enabled")
    ...
    // follow it down, or a plugin that is both widget and menu (omisu.menu)
    // would be locked out of the shell by taking its button off the bar.
    root.config = {
      version: 1,
      bar: { layout: { left: [], center: [], right: [{ id: "omisu.first-widget" }] } },
      plugins: []
    }
    root.assertTrue(registry.inBar("omisu.first-widget"), "inBar sees a widget in the layout")
    registry.setEnabled("omisu.first-widget", false)
    ...
    root.assertTrue(!registry.inBar("omisu.first-widget"), "inBar follows the widget out of the layout")
    root.assertTrue(registry.isEnabled("omisu.first-widget"), "a first-party widget stays loadable off the bar")
    registry.setEnabled("omisu.first-widget", true)
    root.assertDeepEqual(root.config.bar.layout.center, [{ id: "omisu.first-widget" }], "a widget without a default section falls back to center")
    ...
    registry.setEnabled("omisu.hybrid", false)
    ...
    root.assertTrue(registry.isEnabled("omisu.hybrid"), "a multi-kind built-in remains loadable without its widget")
  ...
  PluginRegistry {
    id: registry
    firstPartyDir: ""
    pluginsDir: Quickshell.env("HOME") + "/.config/omisu/plugins"
