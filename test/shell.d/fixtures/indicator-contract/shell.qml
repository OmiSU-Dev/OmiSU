...
ShellRoot {
  ...
  property string resultPath: Quickshell.env("OMISU_QML_TEST_RESULT")
  ...
  QtObject {
    ...
    function firstPartyServiceFor(id) {
      if (id === "omisu.notifications") return notificationService
      if (id === "omisu.idle") return idleService
      if (id === "omisu.nightlight") return nightlightService
      return null
    }
  }
  ...
  readonly property string rootPath: Quickshell.env("OMISU_PATH")
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (screenRecording) {
        ...
        root.assertTrue(root.commandCount("omisu-menu toggle trigger.capture.screenrecord") === 1, "Screen Recording left click opens capture menu when idle")
        screenRecording.recording = true
        screenRecording.triggerPress(Qt.LeftButton)
        root.assertTrue(root.commandCount("omisu-capture-screenrecording --stop-recording") === 1, "Screen Recording left click stops active recording")
      }
      ...
      if (dictation) {
        ...
        root.assertTrue(root.commandCount("omisu-voxtype-config") === 2, "Dictation clicks run config command")
        root.assertTrue(root.commandCount("omisu-voxtype-model") === 0, "Dictation clicks do not run model command")
      }
