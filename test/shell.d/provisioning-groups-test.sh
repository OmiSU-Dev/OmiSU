...
# no longer granted at install time (opt in with omisu-setup-security-sudoless-docker).
...
export OMISU_PROVISIONING_DIR="$TMPDIR/provisioning"
...
OMISU_INSTALL_USER="" bash -eE "$ROOT/install/config/docker.sh"
OMISU_INSTALL_USER="" bash -eE "$ROOT/install/hardware/input-group.sh"

[[ -f $OMISU_PROVISIONING_DIR/groups ]] || fail "groups file written without an install user"
grep -qxF input "$OMISU_PROVISIONING_DIR/groups" || fail "input group recorded"
...
! grep -qxF docker "$OMISU_PROVISIONING_DIR/groups" || fail "docker group must not be recorded"
pass "docker group is not recorded at install"

# Missing user (defensive): no usermod either.
OMISU_INSTALL_USER=ghost bash -eE "$ROOT/install/hardware/input-group.sh"
...
OMISU_INSTALL_USER="" bash -eE "$ROOT/install/hardware/input-group.sh"
[[ $(grep -cxF input "$OMISU_PROVISIONING_DIR/groups") == 1 ]] || fail "input group recorded once"
pass "group recording is idempotent"

# Existing user: usermod applies the recorded groups, and docker is never among them.
OMISU_INSTALL_USER=existing bash -eE "$ROOT/install/config/docker.sh"
OMISU_INSTALL_USER=existing bash -eE "$ROOT/install/hardware/input-group.sh"
