...
FAKE_HYPRCTL_LOG="$hyprctl_log" \
  ...
  OMISU_HYPRLAND_RELOAD_GUARD_RUN_ROOT="$run_root" \
  OMISU_HYPRLAND_RELOAD_GUARD_STATE_DIR="$state_dir" \
  "$ROOT/bin/omisu-hyprland-reload-guard" pause 2>"$test_tmp/pause-stderr"
...
FAKE_HYPRCTL_LOG="$hyprctl_log" \
  ...
  OMISU_HYPRLAND_RELOAD_GUARD_RUN_ROOT="$run_root" \
  OMISU_HYPRLAND_RELOAD_GUARD_STATE_DIR="$state_dir" \
  "$ROOT/bin/omisu-hyprland-reload-guard" resume
...
rm -rf "$state_dir"
OMISU_HYPRLAND_RELOAD_GUARD_STATE_DIR="$state_dir" "$ROOT/bin/omisu-hyprland-reload-guard" paused &&
