...
output=$(printf 'OmiSu' | omisu-ascii)
...
# through `omisu` itself, not just through the binary.
output=$(printf 'OmiSu' | omisu ascii)
[[ $output == "$expected" ]] || fail "piped text renders through the omisu route" "got:
$output"
pass "piped text renders through the omisu route"
...
tight=$(omisu-ascii "Hi" | columns)
spaced=$(omisu-ascii "H i" | columns)
...
status=0
warning=$(omisu-ascii "OmiSu 4.0" 2>&1 >/dev/null) || status=$?
...
output=$(omisu-ascii "OmiSu 4.0" 2>/dev/null)
...
status=0
output=$(omisu-ascii "4.0" 2>/dev/null) || status=$?
...
status=0
output=$(omisu-ascii --width 40 2>&1 >/dev/null) || status=$?
...
output=$(omisu-ascii -- Hi | columns)
(( output == 18 )) || fail "text after -- is still text" "got $output columns"
pass "text after -- is still text"

output=$(omisu-ascii --help)
[[ $output == *"Usage: omisu-ascii"* ]] || fail "help renders"
pass "help renders"
