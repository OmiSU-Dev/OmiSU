...
test_bin=$(mktemp -d)
log_file=$(mktemp)
hook_path="$test_home/.config/omisu/hooks/post-update.d/setup-agent.hook"
...
[[ -f $test_home/.local/state/omisu/done/agent-setup-invitation ]] || fail "agent invitation records completion"
...
grep -qx 'exec:omisu menu summon setup.default.agent' "$log_file" ||
...
fresh_home=$(mktemp -d)
mkdir -p "$fresh_home/.config/omisu/defaults"
printf 'claude\n' >"$fresh_home/.config/omisu/defaults/agent"
: >"$log_file"
cp "$ROOT/install/user/first-run/setup-agent.hook" "$hook_path"
...
[[ ! -f $fresh_home/.local/state/omisu/done/agent-setup-invitation ]] ||
