...
manifest_path="$test_home/.config/chromium/NativeMessagingHosts/com.omisu.ytdlp.json"

HOME="$test_home" OMISU_PATH="$ROOT" omisu-install-chromium-ytdlp
...
jq -e --arg path "$ROOT/bin/omisu-chromium-ytdlp-host" '
  .name == "com.omisu.ytdlp" and
  .path == $path and
  (.allowed_origins | index("chrome-extension://dedjgknigfeelejglamclffonmophnfl/"))
' "$manifest_path" >/dev/null
pass "yt-dlp native host manifest uses OmiSu host path and extension id"

parse_result=$(bash -c '
  OMISU_PATH="$3"
  source "$1"
  parse_url "$2"
' bash "$ROOT/bin/omisu-chromium-ytdlp-host" '{"url":"https://example.test/watch?v=\"quoted\"&name=a\\b"}' "$ROOT")
...
bash -c '
  ...
  source "$1"
  valid_url "$2"
' bash "$ROOT/bin/omisu-chromium-ytdlp-host" "javascript:alert(1)" "$ROOT" &&
  fail "yt-dlp native host rejects non-web URLs"
pass "yt-dlp native host rejects non-web URLs"

host_fn() {
  OMISU_PATH="$ROOT" OMISU_YTDLP_DIR="${download_dir:-$TMPDIR}" bash -c '
    source "$1"
    shift
    "$@"
  ' bash "$ROOT/bin/omisu-chromium-ytdlp-host" "$@"
}
...
while IFS= read -r line; do
  case $line in
  OMISU_FILE*)
    resolved=$(resolve_download_file "${line#OMISU_FILE$'\t'}") || continue
    filepath=$resolved
    ;;
  esac
done
...
poisoned=$(
  printf '%s\n' \
    $'OMISU_FILE\t'"$good_file" \
    $'OMISU_FILE\tPlay me\t--include=not-a-file' \
    $'\t'"$good_file" |
    OMISU_PATH="$ROOT" OMISU_YTDLP_DIR="$download_dir" bash "$parse_script" "$ROOT/bin/omisu-chromium-ytdlp-host"
)

[[ $poisoned == "$expected" ]] ||
  fail "yt-dlp native host keeps a real file after a forged OMISU_FILE record" "$poisoned"
pass "yt-dlp native host keeps a real file after a forged OMISU_FILE record"
...
for arg in "$@"; do
  if [[ $arg == "--no-simulate" ]]; then
    printf 'OMISU_FILE\t%s\n' "$YTDLP_FAKE_FILE"
    [[ -n ${YTDLP_SKIP_TITLE:-} ]] || printf 'OMISU_TITLE\t%s\n' '"My Great Clip"'
    exit 0
  fi
done
exit 0
...
cat >"$fake_root/bin/omisu-notification-send" <<'EOF'
#!/bin/bash
printf '%s\n' "$*" >>"$NOTIFY_ARGV_LOG"
EOF

for stub in omisu-osd omisu-shell ffmpeg; do
  printf '#!/bin/bash\nexit 0\n' >"$fake_root/bin/$stub"
done
chmod +x "$fake_root/bin/"*

YTDLP_ARGV_LOG="$ytdlp_argv" NOTIFY_ARGV_LOG="$notify_argv" YTDLP_FAKE_FILE="$fake_file" \
  OMISU_PATH="$fake_root" OMISU_YTDLP_DIR="$fake_dir" \
  bash -c '
    source "$1"
    download_url "$2"
  ' bash "$ROOT/bin/omisu-chromium-ytdlp-host" "https://example.test/watch" >/dev/null 2>&1
...
grep -qF -- $'OMISU_TITLE\t%(title)j' "$ytdlp_argv" ||
...
grep -qF -- $'OMISU_FILE\t%(title)s' "$ytdlp_argv" &&
...
: >"$ytdlp_argv"
YTDLP_ARGV_LOG="$ytdlp_argv" NOTIFY_ARGV_LOG="$notify_argv" YTDLP_FAKE_FILE="$fake_file" \
  YTDLP_SKIP_TITLE=1 OMISU_PATH="$fake_root" OMISU_YTDLP_DIR="$fake_dir" \
  bash -c '
    source "$1"
    download_url "$2"
  ' bash "$ROOT/bin/omisu-chromium-ytdlp-host" "https://example.test/watch" >/dev/null 2>&1
