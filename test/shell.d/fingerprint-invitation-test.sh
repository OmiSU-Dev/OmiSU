...
if [[ -f /etc/pam.d/omisu-lock-fingerprint ]]; then
...
log_file=$(mktemp)
hw_marker=$(mktemp -u)
hook_path="$test_home/.config/omisu/hooks/post-update.d/setup-fingerprint.hook"
...
cat >"$test_bin/omisu-hw-fingerprint" <<'EOF'
#!/bin/bash
[[ -f $TEST_HW_MARKER ]]
EOF
chmod +x "$test_bin/omisu-hw-fingerprint"

cat >"$test_bin/omisu-notification-send" <<'EOF'
#!/bin/bash
...
EOF
chmod +x "$test_bin/omisu-notification-send"
...
[[ ! -f $test_home/.local/state/omisu/done/fingerprint-setup-invitation ]] || fail "fingerprint invitation stays pending without a reader"
[[ ! -s $log_file ]] || fail "fingerprint invitation does nothing without a reader"

touch "$hw_marker"
run_invitation_hook

[[ -f $test_home/.local/state/omisu/done/fingerprint-setup-invitation ]] || fail "fingerprint invitation records completion"
...
grep -qx 'exec:omisu-launch-floating-terminal-with-presentation omisu-setup-security-fingerprint' "$log_file" ||
