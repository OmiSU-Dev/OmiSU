...
case "$1" in
  ...
  show-hooks)
    cat <<'HOOKS'
after-select-window[0] run-shell -b "omisu-tmux-alert track #{window_id} #{window_activity}"
alert-activity
alert-bell[0] run-shell -b "omisu-shell -q omisu.indicators refresh"
client-focus-in[100] run-shell -b "omisu-tmux-alert track #{window_id} #{window_activity}"
...
cat >"$test_dir/bin/omisu-restart-tmux" <<'STUB'
#!/bin/bash

echo restart >>"$TMUX_RESTARTS"
STUB

cat >"$test_dir/bin/omisu-restart-shell" <<'STUB'
#!/bin/bash
...
cat >"$test_dir/bin/omisu-state" <<'STUB'
#!/bin/bash
...
shell_config="$home/.config/omisu/shell.json"
...
reset_home() {
  rm -rf "$home"
  mkdir -p "$home/.config/tmux" "$home/.config/omisu"
}
...
awk '
  /^# Status bar$/ {
    print "# Alerts"
    print "set-hook -g alert-bell '\''run-shell -b \"omisu-shell -q omisu.indicators refresh\"'\''"
    print "set-hook -g alert-activity '\''run-shell -b \"omisu-shell -q omisu.indicators refresh\"'\''"
    print "set-hook -g alert-silence '\''run-shell -b \"omisu-shell -q omisu.indicators refresh\"'\''"
    print "set-hook -g after-select-window '\''run-shell -b \"omisu-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print "set-hook -g client-session-changed '\''run-shell -b \"omisu-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print "set-hook -g client-focus-out[100] '\''run-shell -b \"omisu-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print "set-hook -g client-focus-in[100] '\''run-shell -b \"omisu-tmux-alert track #{window_id} #{window_activity}\"'\''"
    print ""
  }
  { print }
...
# Alerts
set-hook -g alert-bell 'run-shell -b "omisu-shell -q omisu.indicators refresh"'
set-hook -g alert-activity 'run-shell -b "omisu-shell -q omisu.indicators refresh"'
set-hook -g after-select-window 'run-shell -b "omisu-shell -q omisu.indicators refresh"'
  set-hook -g client-session-changed 'run-shell -b "omisu-tmux-alert track #{window_id} #{window_activity}"'
set-hook -g client-focus-out[100] 'run-shell -b "omisu-tmux-alert track #{window_id} #{window_activity}"'
EOF
...
grep -Fxq 'set-option -wqu -t @0 @omisu_unfocused_activity' "$TMUX_CALLS" || fail "alert removal clears tracked window options"
grep -Fxq 'set-option -wqu -t @3 @omisu_unfocused_activity' "$TMUX_CALLS" || fail "alert removal clears every tracked window"
...
cat >"$shell_config" <<'EOF'
{
  "bar": {
    "layout": {
      "left": [
        { "id": "omisu.indicators", "items": ["TmuxAlert", "Dnd", { "id": "TmuxAlert" }, { "id": "NightLight" }] }
      ],
      "center": [
        { "id": "omisu.indicators", "items": ["TmuxAlert"] },
        { "id": "omisu.clock" }
      ],
      "right": [
        { "id": "omisu.indicators", "indicators": ["TmuxAlert", "Dnd"] },
        { "id": "omisu.indicators", "items": [] }
      ]
    }
  }
}
...
[[ $(jq -c '[.bar.layout.center[].id]' "$shell_config") == '["omisu.clock"]' ]] ||
...
[[ $(jq -c '.bar.layout.right[1]' "$shell_config") == '{"id":"omisu.indicators","items":[]}' ]] ||
