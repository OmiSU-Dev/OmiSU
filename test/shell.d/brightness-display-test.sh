...
cat >"$mock_bin/omisu-hyprland-monitor-focused-apple" <<'SH'
#!/bin/bash
exit 1
SH

cat >"$mock_bin/omisu-hyprland-monitor-focused" <<'SH'
#!/bin/bash
printf '%s\n' "${FOCUSED_MONITOR:-eDP-1}"
SH

cat >"$mock_bin/omisu-hw-display" <<'SH'
#!/bin/bash
...
run_brightness() {
  CALL_LOG="$call_log" XDG_RUNTIME_DIR="$runtime_dir" PATH="$mock_bin:$ROOT/bin:$PATH" \
    "$ROOT/bin/omisu-brightness-display" "$@"
}
...
rm -f "$runtime_dir/omisu-brightness-display-ddc/DP-1.bus"
detect_count=$(grep -c ' detect --brief' "$call_log")
...
printf '7 80 0\n' >"$runtime_dir/omisu-brightness-display-ddc/DP-1.bus"
get_count=$(grep -c ' getvcp 10 ' "$call_log")
...
PATH="$mock_bin:$PATH" "$ROOT/bin/omisu-hyprland-monitor-focused-apple" DP-2 || \
  fail "named Apple display is detected independently of focus"
if PATH="$mock_bin:$PATH" "$ROOT/bin/omisu-hyprland-monitor-focused-apple"; then
