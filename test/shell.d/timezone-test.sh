...
timezone_menu="$ROOT/bin/omisu-menu-timezone"
sudoers_file="$ROOT/etc/sudoers.d/omisu-tzupdate"
...
grep -F 'omisu-shell -q omisu.clock refresh' "$timezone_menu" >/dev/null ||
  fail "timezone menu refreshes the namespaced clock IPC target"

! grep -F 'omisu-shell -q Clock refresh' "$timezone_menu" >/dev/null ||
