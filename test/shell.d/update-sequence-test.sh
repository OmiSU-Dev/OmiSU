...
# Every step omisu-update runs, recorded in order with the unattended flag it
# was handed. One of them can be told to fail.
steps=(
  omisu-update-lock
  omisu-update-requires-free-space
  omisu-update-confirm
  omisu-update-pkg-prune
  omisu-snapshot
  omisu-update-stay-awake
  omisu-update-dev
...
#!/bin/bash
printf '%s unattended=%s\n' "${0##*/}" "${OMISU_UPDATE_UNATTENDED:-}" >>"$STEP_LOG"
[[ ${FAILING_STEP:-} != "${0##*/}" ]] || exit 1
STUB
  chmod +x "$stub_bin/$step"
done

# OMISU_UPDATE_LOGGED stands in for the script(1) wrapper the update re-execs
# itself under; the stubbed lock reports itself already held.
run_update() {
  : >"$test_tmp/steps"
  STEP_LOG="$test_tmp/steps" \
    FAILING_STEP="${FAILING_STEP:-}" \
    OMISU_UPDATE_LOGGED=1 \
    PATH="$stub_bin:$PATH" \
    bash "$ROOT/bin/omisu-update" "$@" >"$test_tmp/out" 2>"$test_tmp/err"
}
...
grep -q '^omisu-update-system-pkgs unattended=1$' "$test_tmp/steps" ||
...
grep -q '^omisu-update-system-pkgs unattended=$' "$test_tmp/steps" ||
...
if FAILING_STEP=omisu-update-system-pkgs run_update -y; then
  fail "an update whose packages did not upgrade passes for a whole one"
fi
for step in omisu-migrate omisu-hook omisu-update-aur-pkgs omisu-update-restart; do
