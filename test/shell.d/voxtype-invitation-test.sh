...
test_bin=$(mktemp -d)
log_file=$(mktemp)
hook_path="$test_home/.config/omisu/hooks/post-update.d/install-voxtype.hook"
...
[[ -f $test_home/.local/state/omisu/done/voxtype-install-invitation ]] || fail "Voxtype invitation records completion"
...
grep -qx 'exec:omisu-launch-floating-terminal-with-presentation omisu-voxtype-install' "$log_file" ||
