...
lid_close="$ROOT/bin/omisu-system-lid-close"
tmpdir=$(mktemp -d)
...
setup_scenario() {
  ...
  cat >"$mock_bin/omisu-hw-laptop-closed" <<SH
#!/bin/bash
exit $closed
SH
  cat >"$mock_bin/omisu-hw-external-monitors" <<SH
#!/bin/bash
exit $docked
SH
  for command in omisu-system-lock omisu-hyprland-monitor-clamshell; do
...
cat >"$mock_bin/omisu-system-lock" <<'SH'
#!/bin/bash
echo omisu-system-lock >>"$CALL_LOG"
exit 1
SH
chmod +x "$mock_bin/omisu-system-lock"
run_lid_close
