...
other_packages="$ROOT/install/omisu-other.packages"
...
cat >"$stub_bin/omisu-pkg-present" <<'SH'
#!/bin/bash
...
printf 'omisu-pkg-drop' >>"$TEST_LOG"
printf '\t%s' "$@" >>"$TEST_LOG"
printf '\n' >>"$TEST_LOG"
SH
...
cat >"$limine_conf" <<'EOF'
# Generated by OmiSu installer for T2 Mac support
...
PATH="$stub_bin:$PATH" \
  ...
  OMISU_T2_LIMINE_CONF="$limine_conf" \
  OMISU_T2_FAN_CONF="$fan_conf" \
  OMISU_T2_RUNNING_CMDLINE="$running_cmdline" \
  OMISU_T2_REPAIR_MARKER="$repair_marker" \
...
grep -Fq $'omisu-pkg-drop\ttiny-dfr' "$calls" ||
...
! grep -Eq $'^(sudo\t)?(sed|tee|systemctl|omisu-pkg-drop)(\t|$)' "$calls" ||
