...
run_dev_update() {
  OMISU_PATH="$1" \
    ...
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/omisu-update-dev"
}

: >"$git_log"
run_dev_update /usr/share/omisu
...
grep -F "OMISU_PATH is not a git checkout: $checkout" "$test_tmp/invalid.err" >/dev/null ||
...
grep -qE '^ *omisu-update-dev$' "$ROOT/bin/omisu-update" ||
