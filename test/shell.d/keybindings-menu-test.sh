...
keybindings() {
  env -i PATH="$stub_bin:$ROOT/bin:$PATH" HOME="$home" \
    XDG_CACHE_HOME="$tmpdir/cache" OMISU_PATH="$ROOT" \
    bash "$ROOT/bin/omisu-menu-keybindings" --print
}

# Closing a window and toggling the scratchpad are two of the actions OmiSu
# binds twice on purpose. The last bind carries the longest description OmiSu
...
# characters at the heading size. The longest entry OmiSu ships sits at 74, so
...
# Sharing a row is something OmiSu names an action for, not something two
# chords earn by looking alike. Alt + Tab and Shift + Alt + Tab both read
# "Reveal active window on top" and cycle opposite ways.
stub_hyprctl <<BINDS
$(exec_bind 64 "SUPER + Y" "Zoom in" "omisu-zoom in")
$(exec_bind 64 "SUPER + Z" "Zoom in" "omisu-zoom in")
BINDS

rendered=$(keybindings)
(( $(grep -c '→ Zoom in$' <<<"$rendered") == 2 )) ||
  fail "an action OmiSu did not name keeps its chords on separate rows" "$rendered"
pass "an action OmiSu did not name keeps its chords on separate rows"
...
stub_hyprctl <<BINDS
$(lua_bind 64 "SUPER + W" "Close window")
$(exec_bind 64 "SUPER + X" "Close window" "omisu-hyprland-window-close-all")
BINDS
...
eval "$(sed -n '/^alternative_chord_actions()/,/^}/p' "$ROOT/bin/omisu-menu-keybindings")"

[[ $(alternative_chord_actions) == "$(printf '%s\n' "${expected_alternatives[@]}")" ]] ||
  fail "the menu pairs up the actions OmiSu means it to" "$(alternative_chord_actions)"
pass "the menu pairs up the actions OmiSu means it to"
