...
packaged_defaults="$ROOT/etc/limine-entry-tool.d/omisu-defaults.conf"
...
defaults_conf="$test_tmp/omisu-defaults.conf"
...
run_migration() {
  PATH="$stub_bin:$PATH" \
    TEST_LOG="$calls" \
    OMISU_LIMINE_DEFAULTS_CONF="$defaults_conf" \
    OMISU_RUNNING_CMDLINE="$running_cmdline" \
    OMISU_LIMINE_REBUILD_MARKER="$rebuild_marker" \
