...
OMISU_PATH="$ROOT" lua - <<'LUA' || fail "the console covers half the work area at any monitor scale"
...
assert(current().on_created_empty:find("omisu%-agent"), "console is seeded with the default agent")
...
assert(final.on_created_empty:find("omisu%-agent"), "refitting keeps the console seeded")
