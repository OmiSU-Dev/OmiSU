...
about="$ROOT/bin/omisu-launch-about"
...
mkdir -p "$HOME/.config/omisu/branding"
printf '%s\n' '████████' '████████' >"$HOME/.config/omisu/branding/about.txt"
...
[[ ${handed[0]} == "$HOME/.config/omisu/branding/about.txt" ]] || fail "the sheen is given the logo About draws" "${handed[0]}"
...
# of them ahead of OmiSu's own can put the logo somewhere else entirely.
...
# One fastfetch would never read, because OmiSu's own comes first, is not a
...
build_sheen || fail "a config fastfetch searches after OmiSu's own still animates"
pass "a config fastfetch searches after OmiSu's own still animates"
...
config_paths=("$tmp_dir/example user/.config/fastfetch/" "$OMISU_FASTFETCH_DIR/ (*)")
...
config_paths=("$HOME/.config/fastfetch/" "$HOME/fastfetch/" "$OMISU_FASTFETCH_DIR/ (*)")
...
rm -rf "${HOME:?}/.local"
printf '%s\n' $(for i in $(seq 40); do echo '██████████'; done) >"$HOME/.config/omisu/branding/about.txt"
layout_rows=43
measure_layout() {
  LAYOUT_ROWS=$layout_rows
  LOGO_COLOR=$logo_color
...
hyprctl() {
  [[ $1 == "clients" ]] && printf '[{"class":"org.omisu.about","address":"0x1","size":[800,600]}]\n'
  return 0
}
