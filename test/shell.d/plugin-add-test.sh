...
mkdir -p "$stub_dir"
cat >"$stub_dir/omisu-shell" <<'STUB'
#!/bin/bash
exit 0
STUB
chmod +x "$stub_dir/omisu-shell"

test_home="$TMPDIR/home"
write_plugin "$test_home/.config/omisu/plugins/different-folder" "acme.same" "Installed"
...
output=$(HOME="$test_home" OMISU_PATH="$ROOT" PATH="$stub_dir:$ROOT/bin:$PATH" \
  omisu-plugin-add "$incoming" --yes 2>&1) &&
...
[[ ! -e $test_home/.config/omisu/plugins/acme.same ]] ||
...
# before `git clone` runs, matching omisu-theme-install. A git stub records
...
mkdir -p "$guard_stubs"
cat >"$guard_stubs/omisu-shell" <<'STUB'
#!/bin/bash
exit 0
STUB
chmod +x "$guard_stubs/omisu-shell"
...
add_url() {
  HOME="$test_home" OMISU_PATH="$ROOT" PATH="$guard_stubs:$ROOT/bin:$PATH" \
    omisu-plugin-add "$1" --yes 2>&1
}

# Transport helpers reach the guard, are named as such, and never reach clone.
for bad in "ext::sh -c touch /tmp/omisu-guard-test" "fd::17"; do
  rm -f "$clone_marker"
  output=$(add_url "$bad") &&
...
if script -qec true /dev/null >/dev/null 2>&1; then
  rm -f "$clone_marker"
  status=0
  raw=$(GUM_INPUT_VALUE="-oProxyCommand=x" HOME="$test_home" OMISU_PATH="$ROOT" \
    PATH="$guard_stubs:$ROOT/bin:$PATH" \
    script -qec "omisu-plugin-add --yes" /dev/null) || status=$?
  output=$(tr -d '\r' <<<"$raw")
  (( status != 0 )) ||
