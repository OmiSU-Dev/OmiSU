...
# (gum confirm), but defer it when OMISU_DEFER_REBOOT is set (the migration
# reuses them inside `omisu update`, where omisu-update-restart handles it).
...
exit "${GUM_ANSWER:-0}"
STUB
cat >"$stub_bin/omisu-system-reboot" <<'STUB'
#!/bin/bash
touch "${REBOOT_CALLED:?}"
STUB
chmod +x "$stub_bin"/*

reboot_flag="$home/.local/state/omisu/reboot-required"
...
run() { # command STUB_GROUPS GUM_ANSWER DEFER(0|1)
  ...
  local defer_env=()
  [[ ${4:-0} == 1 ]] && defer_env=(OMISU_DEFER_REBOOT=1)
...
run omisu-remove-security-sudoless-docker "wheel input docker" 0 0
...
run omisu-remove-security-sudoless-docker "wheel input docker" 1 0
...
run omisu-remove-security-sudoless-docker "wheel input docker" 0 1
...
run omisu-remove-security-sudoless-docker "wheel input" 0 0
...
run omisu-setup-security-sudoless-docker "wheel input" 0 0
