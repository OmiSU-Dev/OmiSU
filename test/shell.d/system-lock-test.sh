...
for command in omisu-shell hyprctl pkill timeout; do
...
PATH="$mock_bin:$PATH" CALL_LOG="$call_log" "$ROOT/bin/omisu-system-lock"
...
[[ ${shutdown[2]} == "pkill -f [o]rg.omisu.screensaver" ]] ||
