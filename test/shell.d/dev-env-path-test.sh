...
# Test against a copy so the test controls /etc/omisu.conf without mutating the host.
bootstrap="$tmpdir/env-bootstrap"
sed "s#/etc/omisu.conf#$tmpdir/omisu.conf#g" "$ROOT/default/bash/env-bootstrap" >"$bootstrap"

printf 'export OMISU_PATH="/usr/share/omisu"\n' >"$tmpdir/omisu.conf"
mapfile -t default_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
default_path=${default_result[1]}

[[ ${default_result[0]} == /usr/share/omisu ]] || fail "env-bootstrap resolves default OMISU_PATH" "actual: ${default_result[0]}"
pass "env-bootstrap resolves default OMISU_PATH"
...
printf 'export OMISU_PATH="%s"\n' "$tmpdir/active" >"$tmpdir/omisu.conf"
mapfile -t linked_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
linked_path=${linked_result[1]}

[[ ${linked_result[0]} == "$tmpdir/active" ]] || fail "env-bootstrap resolves linked OMISU_PATH" "actual: ${linked_result[0]}"
pass "env-bootstrap resolves linked OMISU_PATH"
