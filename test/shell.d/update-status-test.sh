...
cat >"$stub_bin/omisu-shell" <<'SH'
#!/bin/bash

printf '%s\n' "$*" >>"$SHELL_CALLS"
SH

chmod +x "$stub_bin/omisu-update-available" "$stub_bin/omisu-shell"

PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=0 \
  "$ROOT/bin/omisu-update-status"
grep -Fx -- "-q omisu.system-update refresh" "$shell_calls" >/dev/null ||
...
PATH="$stub_bin:$PATH" SHELL_CALLS="$shell_calls" UPDATE_AVAILABLE_STATUS=1 \
  "$ROOT/bin/omisu-update-status"
grep -Fx -- "-q omisu.system-update clear" "$shell_calls" >/dev/null ||
