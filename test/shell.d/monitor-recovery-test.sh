...
monitor_watch="$ROOT/bin/omisu-hyprland-monitor-watch"
monitor_internal="$ROOT/bin/omisu-hyprland-monitor-internal"
monitor_mirror="$ROOT/bin/omisu-hyprland-monitor-internal-mirror"
monitor_laptop="$ROOT/bin/omisu-hyprland-monitor-laptop"
monitor_external_active="$ROOT/bin/omisu-hyprland-monitor-external-active"
system_wake="$ROOT/bin/omisu-system-wake"
clamshell="$ROOT/bin/omisu-hyprland-monitor-clamshell"
lock_service="$ROOT/shell/plugins/lock/Service.qml"
hw_clamshell="$ROOT/bin/omisu-hw-clamshell"
hw_laptop_closed="$ROOT/bin/omisu-hw-laptop-closed"
...
grep -F 'omisu-hyprland-monitor-clamshell' "$monitor_watch" >/dev/null
...
grep -F 'omisu-hw-laptop && omisu-hyprland-monitor-external-active' "$monitor_watch" >/dev/null
...
grep -F 'omisu-hyprland-reload-guard paused' "$monitor_watch" >/dev/null
pass "modeless recovery does not reload into a package transaction"

grep -F '.disabled != true and (.width == 0 or .height == 0)' "$ROOT/bin/omisu-hyprland-monitor-modeless" >/dev/null
grep -F 'hyprctl monitors all -j' "$ROOT/bin/omisu-hyprland-monitor-modeless" >/dev/null
pass "modeless helper sees mirrors and ignores monitors disabled on purpose"

grep -F 'omisu-hw-laptop-closed && omisu-hw-external-monitors' "$hw_clamshell" >/dev/null
...
grep -F 'omisu-hyprland-monitor-internal recover >/dev/null 2>&1 || true' "$clamshell" >/dev/null
grep -F 'omisu-hyprland-monitor-internal-mirror recover >/dev/null 2>&1 || true' "$clamshell" >/dev/null
...
grep -F 'omisu-hyprland-monitor-external-active' "$clamshell" >/dev/null
grep -F 'omisu-hw-clamshell' "$clamshell" >/dev/null
...
grep -F 'omisu-hyprland-monitor-laptop' "$monitor_internal" >/dev/null
grep -F 'hyprctl monitors all -j' "$monitor_laptop" >/dev/null
grep -F 'omisu-hyprland-monitor-external-active' "$monitor_internal" >/dev/null
grep -F 'wake' "$monitor_internal" >/dev/null
grep -F 'omisu-hyprland-toggle-enabled $TOGGLE || return 0' "$monitor_internal" >/dev/null
...
grep -F 'omisu-hyprland-monitor-external-active' "$monitor_mirror" >/dev/null
pass "internal mirror helper recovers when no active external display remains"

grep -F 'switch:on:Lid Switch", nil, "omisu-system-lid-close"' "$utilities" >/dev/null
grep -F 'switch:off:Lid Switch", nil, "omisu-hyprland-monitor-clamshell"' "$utilities" >/dev/null
pass "lid switch bindings lock on close and reconcile clamshell display state"

grep -F 'omisu-hyprland-monitor-clamshell >/dev/null 2>&1 || true' "$system_wake" >/dev/null
