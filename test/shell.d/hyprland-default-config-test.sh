...
run_application_bindings() {
  ...
  HOME="$home" XDG_CONFIG_HOME="$home/.config" XDG_STATE_HOME="$home/.local/state" OMISU_PATH="$ROOT" OMISU_BINDING_PRELUDE="$prelude" lua <<'LUA'
package.path = os.getenv("HOME") .. "/.config/?.lua;" .. os.getenv("OMISU_PATH") .. "/?.lua;" .. package.path

local prelude = os.getenv("OMISU_BINDING_PRELUDE") or ""
if prelude ~= "" then
  assert(load(prelude))()
end
...
mkdir -p "$removed_home/.local/state/omisu"
touch "$removed_home/.local/state/omisu/preinstalls-removed"
...
variable_output=$(run_application_bindings "$variable_home" 'omisu_preinstalled_bindings = false')
...
no_bindings_output=$(run_omisu_bindings "$no_bindings_home" 'omisu_default_bindings = false')
[[ -z $no_bindings_output ]] || fail "default binding variable disables all OmiSu bindings" "$no_bindings_output"
pass "default binding variable disables all OmiSu bindings"
...
voxtype_output=$(PATH="$voxtype_bin:$PATH" run_omisu_bindings "$voxtype_home")
...
voxtype_without_execute_output=$(PATH="$voxtype_bin:$PATH" run_omisu_bindings \
...
missing_voxtype_output=$(PATH="$missing_bin" run_omisu_bindings "$voxtype_home")
...
scratchpad_output=$(run_omisu_bindings "$scratchpad_home")
...
mkdir -p "$panels_home"
panels_output=$(run_omisu_bindings "$panels_home")
...
-- Application bindings without OmiSu's preinstalled web apps, TUIs, or desktop apps.
o.bind("SUPER + RETURN", "Terminal", { omisu = "terminal" })
o.bind("SUPER + SHIFT + RETURN", "Browser", { omisu = "browser" })
o.bind("SUPER + SHIFT + F", "File manager", { omisu = "nautilus" })
o.bind("SUPER + ALT + SHIFT + F", "File manager (cwd)", { omisu = "nautilus-cwd" })
o.bind("SUPER + SHIFT + B", "Browser", { omisu = "browser" })
o.bind("SUPER + SHIFT + ALT + B", "Browser (private)", { omisu = "browser --private" })
o.bind("SUPER + SHIFT + N", "Editor", { omisu = "editor" })
LUA
HOME="$migration_home" OMISU_PATH="$ROOT" bash -euo pipefail "$migration" >/dev/null
...
[[ -f $migration_home/.local/state/omisu/preinstalls-removed ]] ||
...
upgrade_script="$ROOT/bin/omisu-upgrade-to-quattro"
