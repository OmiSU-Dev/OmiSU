...
// ---- widget wiring
assert(/moduleName: "omisu\.clock"/.test(panelSource), 'calendar panel declares its module name')
assert(/ipcTarget: "omisu\.clock"/.test(panelSource), 'calendar panel registers its IPC target')
...
assert(/omisu-menu-timezone/.test(widgetSource), 'clock keeps the timezone picker on middle click')
...
shell_json=$(cd "$ROOT" && jq -r '[.bar.layout.center[].id] | join(",")' config/omisu/shell.json)
[[ $shell_json == *"omisu.clock"* ]] || fail "default bar layout includes the clock widget" "center: $shell_json"
[[ $shell_json != *"omisu.calendar"* ]] || fail "clock hosts the calendar instead of a second bar pill" "center: $shell_json"
pass "default bar layout includes the clock widget"

grep -q 'o.bind("SUPER + CTRL + ALT + D", "Calendar", "omisu-shell shell toggle omisu.clock")' \
