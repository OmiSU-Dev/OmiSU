...
mkdir -p "$TMPDIR/home/.config/omisu" "$TMPDIR/bin"
CALLS="$TMPDIR/calls"
...
if [[ $* == *"listShellConfig"* ]]; then
  ...
  if [[ ${FAKE_NO_DISCOVERY:-0} == 1 ]]; then
    ...
    find "$HOME/.config/omisu/plugins" -mindepth 2 -maxdepth 2 -name manifest.json -print0 |
      xargs -0 -r jq -s 'map({id: .id, enabled: true})'
  fi
elif [[ $* == *"setPluginEnabled"* ]]; then
  printf 'omisu-shell %s\n' "$*" >>"$FAKE_CALLS"
  printf 'ok\n'
fi
exit 0
SH

for command in omisu-plugin-enable omisu-notification-send fake-editor; do
...
clone_plugin() {
  ...
  HOME="$TMPDIR/home" USER=tester OMISU_PATH="$ROOT" PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
    FAKE_CALLS="$CALLS" OMISU_TEST_ROOT="$ROOT" \
    FAKE_SHELL_CONFIG="${FAKE_CLONE_CONFIG:-$default_config}" \
    omisu-plugin-clone "$@"
}

clone_plugin omisu.clock >/dev/null
clock="$TMPDIR/home/.config/omisu/plugins/tester.clock"
...
rg -qF "omisu.clock" "$clock" -g '*.qml' -g '*.js' ||
...
jq -e '
  ...
  .omisu.clonedFrom == "omisu.clock" and
...
grep -qx 'omisu-plugin-enable tester.clock' "$CALLS" ||
  fail "clone does not enable the editable copy"
grep -qx 'omisu-notification-send -g 󰐱 Editing Cloned Plugin Original plugin has been replace by clone.' "$CALLS" ||
...
clone_plugin omisu.keyboard-layout >/dev/null
grep -qx 'omisu-plugin-enable tester.keyboard-layout' "$CALLS" ||
...
clone_plugin omisu.menu >/dev/null
menu="$TMPDIR/home/.config/omisu/plugins/tester.menu"
...
grep -qx 'omisu-plugin-enable tester.menu' "$CALLS" ||
  fail "clone does not enable a multi-kind plugin"
pass "clone preserves and enables multi-kind plugins"

remove_output=$(HOME="$TMPDIR/home" OMISU_PATH="$ROOT" PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  FAKE_CALLS="$CALLS" OMISU_TEST_ROOT="$ROOT" \
  omisu-plugin-remove tester.menu --yes)
grep -qx 'omisu-shell shell setPluginEnabled tester.menu false' "$CALLS" ||
  fail "removing an enabled clone does not disable it first"
grep -q 'Restored omisu.menu.' <<<"$remove_output" ||
...
clone_plugin omisu.active-window >/dev/null
[[ -f $TMPDIR/home/.config/omisu/plugins/tester.active-window/ActiveWindow.qml ]] ||
  fail "flat bar plugin clone is incomplete"
pass "flat bar plugins clone from adjacent manifests"

grep -qx 'omisu-plugin-enable tester.active-window' "$CALLS" ||
  fail "clone does not activate a bar widget whose source is absent"
pass "clone activates an absent bar widget"

clone_plugin omisu.indicators >/dev/null
indicators="$TMPDIR/home/.config/omisu/plugins/tester.indicators"
...
clone_plugin omisu.tray >/dev/null
[[ -f $TMPDIR/home/.config/omisu/plugins/tester.tray/TrayModel.js ]] ||
  fail "tray clone is missing its model"
pass "flat bar plugins keep local script dependencies"

clone_plugin omisu.bar >/dev/null
grep -qx 'omisu-plugin-enable tester.bar' "$CALLS" ||
  fail "clone does not select a cloned bar"
pass "clone switches full bars"

clone_plugin omisu.background >/dev/null
grep -qx 'omisu-plugin-enable tester.background' "$CALLS" ||
  fail "clone does not enable an ordinary cloned plugin"
pass "clone switches ordinary plugins"

EDITOR=fake-editor clone_plugin omisu.weather --edit >/dev/null
grep -qx "fake-editor $TMPDIR/home/.config/omisu/plugins/tester.weather" "$CALLS" ||
  fail "clone --edit does not open the clone in EDITOR"
pass "clone --edit opens the clone in EDITOR"
rm -rf "$TMPDIR/home/.config/omisu/plugins/tester.weather"

mkdir -p "$TMPDIR/home/.config/omisu/plugins/acme.example"
cat >"$TMPDIR/home/.config/omisu/plugins/acme.example/manifest.json" <<'JSON'
...
if clone_plugin omisu.weather custom.weather >/dev/null 2>&1; then
  fail "clone accepts a custom id"
fi
[[ ! -e $TMPDIR/home/.config/omisu/plugins/tester.weather ]] ||
  fail "rejected custom id leaves a clone behind"
pass "clone derives the personal id from the username"

if clone_plugin omisu.weather --replace >/dev/null 2>&1; then
...
if FAKE_NO_DISCOVERY=1 clone_plugin omisu.osd >/dev/null 2>&1; then
  fail "clone succeeds before the shell discovers it"
fi
[[ ! -e $TMPDIR/home/.config/omisu/plugins/tester.osd ]] ||
