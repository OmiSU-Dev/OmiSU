...
TEST_TMP="$test_tmp" SUCCEED_ON_ATTEMPT=3 \
  PATH="$fake_bin:$PATH" bash "$ROOT/bin/omisu-toggle-hybrid-gpu" >/dev/null
...
error=$(
  TEST_TMP="$test_tmp" \
    PATH="$fake_bin:$PATH" bash "$ROOT/bin/omisu-toggle-hybrid-gpu" 2>&1 >/dev/null
)
status=$?
set -e
...
set +e
output=$(TEST_TMP="$test_tmp" PATH="$fake_bin:$PATH" timeout 25s bash "$ROOT/bin/omisu-toggle-hybrid-gpu" 2>&1)
status=$?
set -e
