...
[[ -n ${OMISU_TEST_QS_ARGS:-} ]] && printf '%s\n' "$*" >"$OMISU_TEST_QS_ARGS"

if [[ ${OMISU_TEST_QS_HANG:-0} == 1 ]]; then
  sleep 5
elif [[ ${OMISU_TEST_QS_STARTING:-0} == 1 ]]; then
...
wrapper_error=$(PATH="$wrapper_bin:$PATH" \
  OMISU_PATH="$wrapper_root" \
  OMISU_SHELL_IPC_TIMEOUT=0.1s \
  OMISU_TEST_QS_HANG=1 \
  "$ROOT/bin/omisu-shell" shell ping 2>&1) && fail "hung shell IPC returns a failure"
[[ $wrapper_error == "omisu-shell is not responding" ]] || fail "hung shell IPC reports that the shell is unresponsive" "$wrapper_error"
...
wrapper_error=$(PATH="$wrapper_bin:$PATH" \
  OMISU_PATH="$wrapper_root" \
  OMISU_TEST_QS_STARTING=1 \
  "$ROOT/bin/omisu-shell" shell ping 2>&1) && fail "a starting shell answers IPC calls with a failure"
[[ $wrapper_error == "omisu-shell is not ready" ]] || fail "a starting shell reports that it is not ready" "$wrapper_error"
pass "shell IPC calls fail while Quickshell is still starting"

PATH="$wrapper_bin:$PATH" \
OMISU_PATH="$wrapper_root" \
OMISU_TEST_QS_STARTING=1 \
  "$ROOT/bin/omisu-shell" -q shell ping >/dev/null 2>&1 ||
...
PATH="$wrapper_bin:$PATH" \
OMISU_PATH="$wrapper_root" \
OMISU_TEST_QS_ARGS="$wrapper_args" \
  "$ROOT/bin/omisu-shell" shell ping >/dev/null
...
ln -s "$ROOT/bin/omisu-shell" "$restart_bin/omisu-shell"
ln -s "$ROOT/bin/omisu-launch-shell" "$restart_bin/omisu-launch-shell"
ln -s "$ROOT/bin/omisu-cmd-missing" "$restart_bin/omisu-cmd-missing"
ln -s "$ROOT/bin/omisu-hyprland-session-locked" "$restart_bin/omisu-hyprland-session-locked"
...
case "$*" in
  *'shell ping')
    [[ $* == *"-p $OMISU_TEST_SESSION_PATH/shell"* ]] &&
      grep -Fx '303' "$OMISU_TEST_QS_STATE" >/dev/null &&
      printf 'ok\n'
    ;;
  *'lock lock')
    touch "$OMISU_TEST_QS_STATE.locked"
    printf 'ok\n'
    ;;
  *'lock status')
    if [[ -f $OMISU_TEST_QS_STATE.locked ]]; then
...
case " $* " in
  *' kill -p '*)
    pid=$(head -n 1 "$OMISU_TEST_QS_STATE")
    [[ $pid =~ ^[0-9]+$ ]] || exit 1
    kill "$pid" 2>/dev/null
    while kill -0 "$pid" 2>/dev/null; do sleep 0.01; done
    awk 'NR > 1' "$OMISU_TEST_QS_STATE" >"$OMISU_TEST_QS_STATE.next"
    mv "$OMISU_TEST_QS_STATE.next" "$OMISU_TEST_QS_STATE"
    ;;
  *' -n -p '*)
    printf '%s\n' "${OMISU_TEST_TRANSIENT_ENV-unset}" >"$OMISU_TEST_QS_ENV_LOG"
    printf '303\n' >"$OMISU_TEST_QS_STATE"
    ;;
esac
SH
...
if [[ ${1:-} == "-j" && ${2:-} == "monitors" ]]; then
  ...
  if [[ ${OMISU_TEST_SESSION_LOCKED:-0} == 1 ]]; then
  ...
  printf '%s\n' "${2:-}" >>"$OMISU_TEST_DISPATCH_LOG"
  OMISU_PATH="$OMISU_TEST_SESSION_PATH" \
    env -u OMISU_TEST_TRANSIENT_ENV omisu-launch-shell
  printf 'ok\n'
...
OMISU_PATH="$caller_root" \
XDG_RUNTIME_DIR="$runtime_dir" \
OMISU_TEST_QS_STATE="$restart_state" \
OMISU_TEST_QS_LOG="$restart_log" \
OMISU_TEST_QS_ENV_LOG="$restart_env_log" \
OMISU_TEST_DISPATCH_LOG="$dispatch_log" \
OMISU_TEST_IPC_LOG="$ipc_log" \
OMISU_TEST_SESSION_PATH="$restart_root" \
OMISU_TEST_TRANSIENT_ENV=leaked \
  timeout 5 "$ROOT/bin/omisu-restart-shell"
...
grep -F 'hl.dsp.exec_cmd("omisu-launch-shell")' "$dispatch_log" >/dev/null || fail "restart launches the fresh shell through Hyprland"
...
locked_error=$(PATH="$restart_bin:$PATH" \
  OMISU_PATH="$restart_root" \
  XDG_RUNTIME_DIR="$runtime_dir" \
  OMISU_TEST_SESSION_LOCKED=1 \
  OMISU_TEST_QS_STATE="$restart_state" \
  OMISU_TEST_QS_LOG="$restart_log" \
  OMISU_TEST_DISPATCH_LOG="$dispatch_log" \
  OMISU_TEST_IPC_LOG="$ipc_log" \
  OMISU_TEST_SESSION_PATH="$restart_root" \
  "$ROOT/bin/omisu-restart-shell" 2>&1) && fail "restart refuses while the shell lock is active"

[[ $locked_error == "Refusing to restart OmiSu shell while the session is locked." ]] || fail "locked restart explains why it was refused" "$locked_error"
...
OMISU_TEST_IPC_LOG="$ipc_log" \
OMISU_TEST_SESSION_PATH="$restart_root" \
  timeout 5 "$ROOT/bin/omisu-restart-shell" || fail "locked restart recovers when the lock client is dead"
