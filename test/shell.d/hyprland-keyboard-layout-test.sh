...
resolved_input() {
  OMISU_PATH="$ROOT" OMISU_VCONSOLE="${1-}" lua <<'LUA'
package.path = os.getenv("OMISU_PATH") .. "/?.lua;" .. package.path

local vconsole = os.getenv("OMISU_VCONSOLE")
...
hooks_conf="$ROOT/etc/mkinitcpio.conf.d/omisu_hooks.conf"
...
[[ -n $hooks_layouts ]] || fail "non-latin layout list is readable from omisu_hooks.conf"
