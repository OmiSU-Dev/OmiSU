...
printf 'sudo' >>"$OMISU_DEV_UNLINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$OMISU_DEV_UNLINK_TEST_LOG"
done
printf '\n' >>"$OMISU_DEV_UNLINK_TEST_LOG"

if [[ $1 == "tee" ]]; then
  cat >"$OMISU_DEV_UNLINK_TEST_CONF"
fi
SH
...
printf 'gum' >>"$OMISU_DEV_UNLINK_TEST_LOG"
for arg in "$@"; do
  printf '\t%s' "$arg" >>"$OMISU_DEV_UNLINK_TEST_LOG"
done
printf '\n' >>"$OMISU_DEV_UNLINK_TEST_LOG"
...
run_unlink() {
  HOME="$test_tmp/home" \
    OMISU_DEV_UNLINK_TEST_LOG="$log_file" \
    OMISU_DEV_UNLINK_TEST_CONF="$conf_file" \
    PATH="$stub_bin:$PATH" \
    "$ROOT/bin/omisu-dev-unlink" "$@"
}

: >"$log_file"
run_unlink --no-reboot

grep -Fx $'sudo\ttee\t/etc/omisu.conf' "$log_file" >/dev/null ||
  fail "dev unlink writes the package path without rebooting" "$(cat "$log_file")"
[[ $(<"$conf_file") == 'export OMISU_PATH="/usr/share/omisu"' ]] ||
...
grep -Fx $'sudo\trm\t-f\t/etc/sudoers.d/omisu-dev-path' "$log_file" >/dev/null ||
...
grep -F 'Usage: omisu dev unlink [--no-reboot]' "$test_tmp/invalid.err" >/dev/null ||
