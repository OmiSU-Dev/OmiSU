...
output=$(env -u WAYLAND_DISPLAY "$ROOT/bin/omisu-shell" omisu.indicators refresh)
[[ $output == "display=[wayland-1]" ]] || fail "shell ipc recovers a missing display" "$output"
pass "shell ipc recovers a missing display"

output=$(WAYLAND_DISPLAY=wayland-9 "$ROOT/bin/omisu-shell" omisu.indicators refresh)
