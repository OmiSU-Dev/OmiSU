...
list_bindings() {
  ...
  HOME="$home" XDG_CONFIG_HOME="$home/.config" XDG_STATE_HOME="$home/.local/state" OMISU_PATH="$ROOT" OMISU_BINDING_EPILOGUE="$epilogue" lua <<'LUA'
...
local epilogue = os.getenv("OMISU_BINDING_EPILOGUE") or ""
if epilogue ~= "" then
  assert(load(epilogue))()
end

-- X11 keycodes are evdev codes plus 8. Only the rows OmiSu binds by code
