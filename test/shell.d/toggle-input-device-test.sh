...
cat >"$stub_dir/omisu-osd" <<'EOF'
#!/bin/bash
:
EOF
chmod +x "$stub_dir/omisu-osd"

stub_device() {
  local kind=$1
  local name=$2
  cat >"$stub_dir/omisu-hw-$kind" <<EOF
#!/bin/bash
printf '%s\n' '$name'
EOF
  chmod +x "$stub_dir/omisu-hw-$kind"
}
...
run_toggle() {
  HOME="$home_dir" \
    ...
    "$ROOT/bin/omisu-toggle-input-device" "$@"
}
...
HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" OMISU_PATH="$ROOT" MARKER="$marker" lua - <<'LUA'
local seen = {}
hl = {
...
HOME="$home_dir" XDG_STATE_HOME="$xdg_decoy" OMISU_PATH="$ROOT" \
...
cat >"$stub_dir/omisu-hw-touchpad" <<'EOF'
#!/bin/bash
printf 'evil\nname\n'
EOF
chmod +x "$stub_dir/omisu-hw-touchpad"
...
reload_state="$reload_home/.local/state/omisu/toggles/hypr"
...
HOME="$reload_home" XDG_STATE_HOME="$reload_home/.local/state" OMISU_PATH="$ROOT" lua - <<'LUA'
local disabled = {}
