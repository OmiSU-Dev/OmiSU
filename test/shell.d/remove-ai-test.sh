...
cat >"$tmp_dir/bin/omisu-pkg-drop" <<'SCRIPT'
#!/bin/bash
printf 'drop:%s\n' "$*" >>"$TEST_LOG"
SCRIPT
chmod +x "$tmp_dir/bin/omisu-pkg-drop"
...
"$ROOT/bin/omisu-remove-ai-chatgpt" >/dev/null
...
"$ROOT/bin/omisu-remove-ai-lm-studio" >/dev/null
...
"$ROOT/bin/omisu-remove-ai-t3-code" >/dev/null
...
# ~/.grok belongs to the Grok CLI that omisu-default-agent installs.
fresh_home
mkdir -p "$HOME/.config/Grok Bot" "$HOME/.grokbot" "$HOME/.grok"
"$ROOT/bin/omisu-remove-ai-grok-bot" >/dev/null
...
# builds omisu-pkg-drop will not touch.
ollama_row=$(grep '^  "remove.ai.ollama":' "$ROOT/default/omisu/omisu-menu.jsonc")
[[ $ollama_row == *'"when":"omisu-pkg-present ollama"'* ]] ||
