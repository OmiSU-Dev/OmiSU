...
state_dir="$home_dir/.local/state/omisu/toggles/hypr"
...
if [[ $1 == "monitors" && $2 == "all" && $3 == "-j" ]]; then
  if [[ ${OMISU_TEST_INTERNAL_DISABLED:-false} == "true" ]]; then
    printf '[{"name":"eDP-1","disabled":true,"scale":null}]'
  else
    printf '[{"name":"eDP-1","disabled":false,"scale":%s}]' "${OMISU_TEST_INTERNAL_SCALE:-2}"
  fi
elif [[ $1 == "eval" ]]; then
  printf '%s\n' "$2" >>"$OMISU_TEST_HYPRCTL_EVAL_LOG"
elif [[ $1 == "reload" ]]; then
  printf 'reload\n' >>"$OMISU_TEST_HYPRCTL_EVAL_LOG"
...
cat >"$stub_bin/omisu-hyprland-monitor-internal" <<'SH'
#!/bin/bash
exit 0
SH

cat >"$stub_bin/omisu-hyprland-monitor-internal-mirror" <<'SH'
#!/bin/bash
exit 0
SH

cat >"$stub_bin/omisu-hyprland-monitor-laptop" <<'SH'
#!/bin/bash
echo eDP-1
SH

cat >"$stub_bin/omisu-hyprland-monitor-external-active" <<'SH'
#!/bin/bash
[[ ${OMISU_TEST_EXTERNAL_ACTIVE:-false} == "true" ]]
SH

cat >"$stub_bin/omisu-hw-clamshell" <<'SH'
#!/bin/bash
[[ ${OMISU_TEST_CLAMSHELL:-false} == "true" ]]
SH
...
local omisu_monitor_scale = "auto"
LUA
}
...
hl.env("GDK_SCALE", tostring(omisu_gdk_scale))
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = omisu_monitor_scale })
LUA
}
...
# A specific-output rule that references the omisu_monitor_scale variable
# (the default template's pattern) instead of a literal value. The variable
# must be resolved, not captured as the literal string "omisu_monitor_scale".
write_internal_monitor_var_config() {
  cat >"$monitor_lua" <<'LUA'
local omisu_gdk_scale = 1.5
local omisu_monitor_scale = 1.5
hl.env("GDK_SCALE", tostring(omisu_gdk_scale))
hl.monitor({ output = "eDP-1", mode = "preferred", position = "auto", scale = omisu_monitor_scale })
LUA
}

# An explicit non-numeric scale on the internal rule, alongside an unrelated
# omisu_monitor_scale local that must not be substituted for it.
...
# references the omisu_monitor_scale local. Quoted here, and commented below.
...
write_commented_catch_all_var_config() {
  cat >"$monitor_lua" <<'LUA'
local omisu_monitor_scale = 1.25 -- HiDPI panel
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = omisu_monitor_scale })
LUA
}
...
write_internal_monitor_position_var_config() {
  cat >"$monitor_lua" <<'LUA'
local omisu_monitor_position = "0x0"
hl.monitor({ output = "eDP-1", mode = "preferred", position = omisu_monitor_position, scale = 1.25 })
LUA
}
...
run_clamshell() {
  HOME="$home_dir" \
    PATH="$stub_bin:$PATH" \
    OMISU_TEST_HYPRCTL_EVAL_LOG="$eval_log" \
    OMISU_TEST_INTERNAL_SCALE="${OMISU_TEST_INTERNAL_SCALE:-2}" \
    OMISU_TEST_INTERNAL_DISABLED="${OMISU_TEST_INTERNAL_DISABLED:-false}" \
    OMISU_TEST_EXTERNAL_ACTIVE="${OMISU_TEST_EXTERNAL_ACTIVE:-false}" \
    OMISU_TEST_CLAMSHELL="${OMISU_TEST_CLAMSHELL:-false}" \
    "$ROOT/bin/omisu-hyprland-monitor-clamshell"
}
...
write_auto_monitor_config
: >"$eval_log"
OMISU_TEST_INTERNAL_SCALE=3 run_clamshell
...
write_internal_monitor_config
: >"$eval_log"
OMISU_TEST_INTERNAL_SCALE=1.25 run_clamshell
...
write_auto_monitor_config
: >"$eval_log"
OMISU_TEST_INTERNAL_SCALE=1.6 OMISU_TEST_EXTERNAL_ACTIVE=true OMISU_TEST_CLAMSHELL=true run_clamshell
...
: >"$eval_log"
OMISU_TEST_INTERNAL_DISABLED=true run_clamshell
...
# Regression: specific-output rule referencing the omisu_monitor_scale
# variable must resolve to the variable's value, not fall back to the default.
write_internal_monitor_var_config
rm -f "$scale_state"
: >"$eval_log"
OMISU_TEST_INTERNAL_DISABLED=true run_clamshell
grep -F 'scale = 1.5' "$eval_log" >/dev/null || fail "clamshell recovery resolves omisu_monitor_scale variable reference"
pass "clamshell recovery resolves omisu_monitor_scale variable reference"

# An explicit "auto" on the internal rule is a value, not a variable reference,
# so it must not pick up an unrelated omisu_monitor_scale local.
write_internal_monitor_auto_config
remember_scale 1.75
: >"$eval_log"
OMISU_TEST_INTERNAL_DISABLED=true run_clamshell
grep -F 'scale = 1.75' "$eval_log" >/dev/null || fail "clamshell recovery keeps auto scale off the omisu_monitor_scale local"
pass "clamshell recovery keeps auto scale off the omisu_monitor_scale local"
