...
snapshot="$ROOT/bin/omisu-snapshot"
...
cat >"$fake_bin/omisu-cmd-missing" <<'STUB'
#!/bin/bash
exit 1
STUB
chmod +x "$fake_bin/omisu-cmd-missing"

cat >"$fake_bin/omisu-version" <<'STUB'
#!/bin/bash
echo 4.0.0
STUB
chmod +x "$fake_bin/omisu-version"
...
grep -qF 'omisu-snapshot create || (($? == 127))' "$ROOT/bin/omisu-update" ||
...
grep -qF 'omisu-snapshot create || (($? == 127))' "$ROOT/bin/omisu-upgrade-to-quattro" ||
  fail "upgrade ignores only the missing-snapper exit code"
grep -qF 'Continuing the upgrade without a snapshot' "$ROOT/bin/omisu-upgrade-to-quattro" ||
