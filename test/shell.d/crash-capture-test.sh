...
cat >"$TMPDIR/bin/omisu-notification-send" <<'SH'
#!/bin/bash
exit 0
SH

chmod +x "$TMPDIR/bin/systemctl" "$TMPDIR/bin/omisu-notification-send"

test_home="$TMPDIR/home"
flag="$test_home/.local/state/omisu/toggles/crash-capture-off"

toggle_crash_capture() {
  PATH="$TMPDIR/bin:$ROOT/bin:$PATH" \
  SYSTEMCTL_LOG="$SYSTEMCTL_LOG" \
  HOME="$test_home" \
    "$ROOT/bin/omisu-toggle-crash-capture"
}

toggle_crash_capture
[[ -f $flag ]] || fail "crash capture toggle disables the watcher"
grep -Fqx -- "--user stop omisu-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
grep -Fqx -- "--user start omisu-crash-watch.service" "$SYSTEMCTL_LOG" ||
...
service="$ROOT/default/systemd/user/omisu-crash-watch.service"
grep -Fx 'ConditionPathExists=!%h/.local/state/omisu/toggles/crash-capture-off' "$service" >/dev/null ||
...
grep -F 'omisu-crash-watch.service' "$ROOT/install/user/first-run/enable-user-units.sh" >/dev/null ||
...
const items = menu.parseMenuJsonc(fs.readFileSync(path.join(root, 'default/omisu/omisu-menu.jsonc'), 'utf8'))
const byId = Object.fromEntries(items.map(item => [item.id, item]))

assertEqual(
  byId['trigger.toggle.crash-capture'].action,
  'omisu-toggle-crash-capture',
