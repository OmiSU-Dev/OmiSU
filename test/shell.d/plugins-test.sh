...
for (const manifestPath of manifests) {
  ...
  check(String(manifest.id).startsWith('omisu.'), `${manifest.id} must use the first-party namespace`)
  ...
  const clonePaths = manifest.omisu?.clonePaths
  if (clonePaths !== undefined) {
    check(Array.isArray(clonePaths), `${manifest.id} omisu.clonePaths must be an array`)
...
for (const [id, section] of Object.entries({
  'omisu.active-window': 'left',
  'omisu.dropbox': 'right'
})) {
  check(byId[id]?.barWidget?.defaultSection === section, `${id} must default to the ${section} bar section`)
}
check(byId['omisu.media']?.barWidget?.defaultSection === undefined, 'omisu.media must use the center fallback')
