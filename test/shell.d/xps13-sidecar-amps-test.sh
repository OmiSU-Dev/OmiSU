...
detector="$ROOT/bin/omisu-hw-dell-xps13-sidecar-amps"
...
cat >"$test_tmp/bin/omisu-hw-match" <<'SH'
#!/bin/bash
[[ ${TEST_PRODUCT_NAME:-} == *"$1"* ]]
SH

cat >"$test_tmp/bin/omisu-pkg-add" <<'SH'
#!/bin/bash
printf 'pkg-add %s\n' "$*" >>"$CALL_LOG"
...
cat >"$test_tmp/bin/omisu-state" <<'SH'
#!/bin/bash
printf 'state %s\n' "$*" >>"$CALL_LOG"
SH
...
run_detector() {
  ...
  PATH="$test_tmp/bin:$PATH" \
    ...
    OMISU_DMI_PRODUCT_SKU="${3-$sku_file}" \
    bash "$detector"
}
...
run_leaf() {
  ...
  PATH="$test_tmp/bin:$ROOT/bin:$PATH" \
    ...
    OMISU_DMI_PRODUCT_SKU="$sku_file" \
