...
# A file OmiSu writes into /usr belongs to nobody, and the
# day a package starts shipping that same path, pacman refuses the upgrade for
# everyone who has the file. omisu-update-system-pkgs-when-conflicted recovers from
...
# Paths OmiSu writes into /usr that no package owns, with the reason.
allowed = {
  ...
  # than OmiSu's, which is why neither is a candidate for omisu-settings.
  "/usr/share/chromium/extensions",
  "/usr/lib/firefox/distribution",
  # Static content that belongs in omisu-settings. It cannot move there in the
  # same release that first ships omisu-update-system-pkgs-when-conflicted: the
...
if pkgs_root is None:
  print("not ok - omisu-pkgs checkout found for package ownership check", file=sys.stderr)
  sys.exit(1)

packaged = "\n".join(p.read_text() for p in pkgs_root.glob("*/PKGBUILD"))

# Commands that put a file somewhere, as opposed to reading one.
# /etc is administrator territory that OmiSu legitimately edits. /usr is
    ...
    for lineno, line in joined:
      ...
      if redirect:
        ...
        if not tokens or not tokens[-1].startswith("/usr/"):
          continue
        hit = tokens[-1].rstrip("/")
      # OmiSu's own tree and its binaries are covered elsewhere.
      if hit.startswith(("/usr/share/omisu", "/usr/bin")) or hit.count("/") < 3:
        continue
...
if problems:
  print("not ok - OmiSu writes paths under /usr that no package owns", file=sys.stderr)
  for p in problems:
...
pass "no OmiSu script writes a path under /usr that no package owns"
