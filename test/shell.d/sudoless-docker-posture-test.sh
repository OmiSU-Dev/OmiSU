...
# (omisu-setup-security-sudoless-docker).
...
eval "$(sed -n '/^user_groups() {/,/^}/p' "$ROOT/bin/omisu-provision-owner")"
groups=$(user_groups)
...
if rg -q 'usermod -aG docker' "$ROOT/bin/omisu-upgrade-to-quattro"; then
  fail "omisu-upgrade-to-quattro must not add the user to the docker group"
fi
