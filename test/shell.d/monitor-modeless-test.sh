...
modeless_lock="$test_tmp/omisu-monitor-modeless.lock"
...
if [[ ${1:-} == "monitors" && ${2:-} == "all" && ${3:-} == "-j" ]]; then
  [[ -f $OMISU_TEST_HYPRCTL_FAIL_FLAG ]] && exit 4
  cat "$OMISU_TEST_MONITORS_FILE"
elif [[ ${1:-} == "reload" ]]; then
  printf 'reload\n' >>"$OMISU_TEST_RELOAD_LOG"
  [[ -f $OMISU_TEST_RECOVERED_MONITORS ]] &&
    cp "$OMISU_TEST_RECOVERED_MONITORS" "$OMISU_TEST_MONITORS_FILE"
fi
SH

cat >"$fake_bin/omisu-hyprland-reload-guard" <<'SH'
#!/bin/bash

[[ ${1:-} == "paused" && ${OMISU_TEST_GUARD_PAUSED:-0} == 1 ]]
SH

cat >"$fake_bin/socat" <<'SH'
#!/bin/bash

exec cat "$OMISU_TEST_EVENTS"
SH

# A desktop, so the clamshell half of the watcher stays idle.
cat >"$fake_bin/omisu-hw-laptop" <<'SH'
#!/bin/bash

exit 1
SH

cat >"$fake_bin/omisu-hyprland-monitor-clamshell" <<'SH'
#!/bin/bash
...
chmod +x "$fake_bin"/*
ln -s "$ROOT/bin/omisu-hyprland-monitor-modeless" "$fake_bin/omisu-hyprland-monitor-modeless"
...
assert_state() {
  ...
  PATH="$fake_bin:$PATH" OMISU_TEST_MONITORS_FILE="$monitors_file" \
  OMISU_TEST_HYPRCTL_FAIL_FLAG="$hyprctl_fail" \
    "$ROOT/bin/omisu-hyprland-monitor-modeless" || actual=$?
...
PATH="$fake_bin:$PATH" OMISU_TEST_MONITORS_FILE="$monitors_file" \
OMISU_TEST_HYPRCTL_FAIL_FLAG="$hyprctl_fail" \
  "$ROOT/bin/omisu-hyprland-monitor-modeless" || unreachable=$?
rm -f "$hyprctl_fail"
...
start_watcher() {
  ...
  OMISU_TEST_EVENTS="$events" \
  OMISU_TEST_MONITORS_FILE="$monitors_file" \
  OMISU_TEST_RELOAD_LOG="$reload_log" \
  OMISU_TEST_RECOVERED_MONITORS="$recovered" \
  OMISU_TEST_GUARD_PAUSED="${1:-0}" \
  OMISU_TEST_HYPRCTL_FAIL_FLAG="$hyprctl_fail" \
    "$ROOT/bin/omisu-hyprland-monitor-watch" &
  watch_pid=$!
