...
run_pkg_prune() {
  PATH="$stub_bin:$PATH" "$ROOT/bin/omisu-update-pkg-prune"
}
...
# before the snapshot.
line_of() {
  grep -n "^[[:space:]]*$1\b" "$ROOT/bin/omisu-update" | head -1 | cut -d: -f1
}

prune_line=$(line_of omisu-update-pkg-prune)
snapshot_line=$(line_of omisu-snapshot)
pkgs_line=$(line_of omisu-update-system-pkgs)
[[ -n $prune_line && -n $snapshot_line && -n $pkgs_line ]] ||
  fail "omisu-update runs the cache prune, the snapshot, and the packages update"
