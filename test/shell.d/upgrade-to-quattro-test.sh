...
upgrade_to_quattro="$ROOT/bin/omisu-upgrade-to-quattro"
...
grep -F 'omisu-snapshot create || (($? == 127))' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade snapshots the system before mutation"
...
grep -F 'pacman -Syy --noconfirm archlinux-keyring omisu-keyring' "$upgrade_to_quattro" >/dev/null
if grep -F 'pacman -Sy --noconfirm archlinux-keyring omisu-keyring' "$upgrade_to_quattro" >/dev/null; then
  fail "OmiSu 4 upgrade forces a database refresh before installing keyrings"
fi
pass "OmiSu 4 upgrade forces a database refresh before installing keyrings"

grep -F 'pacman -Syu --needed' "$upgrade_to_quattro" >/dev/null
grep -F 'omisu-update-aur-pkgs' "$upgrade_to_quattro" >/dev/null
grep -F 'omisu-update-available' "$upgrade_to_quattro" >/dev/null
grep -F 'omisu-update-mise' "$upgrade_to_quattro" >/dev/null
grep -F 'run_final_system_package_upgrade' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade completes package update checks"

grep -F 'run_post_upgrade_migrations' "$upgrade_to_quattro" >/dev/null
grep -F 'omisu-migrate' "$upgrade_to_quattro" >/dev/null
grep -F 'dust' "$upgrade_to_quattro" >/dev/null
grep -F 'satty' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade applies packaged migrations"

if grep -F 'skip-first-run-update-notification' "$upgrade_to_quattro" >/dev/null; then
  fail "OmiSu 4 upgrade does not use notification-specific first-run state"
fi
pass "OmiSu 4 upgrade completes first-run as one lifecycle"
...
pass "OmiSu 4 upgrade completes first-run and migrates legacy completion markers"
...
if grep -F '"$root/bin/omisu-done"' "$upgrade_to_quattro" >/dev/null; then
  fail "OmiSu 4 upgrade writes completion markers without the packaged omisu-done"
fi
pass "OmiSu 4 upgrade writes completion markers without the packaged omisu-done"

for guarded_step in omisu-refresh-applications 'omisu-bar defaults'; do
  grep -F "run_as_user_omisu $guarded_step ||" "$upgrade_to_quattro" >/dev/null ||
    fail "OmiSu 4 upgrade survives a packaged tree without $guarded_step"
done
pass "OmiSu 4 upgrade survives a packaged tree missing top-level commands"

grep -F 'configure_snapper_policy' "$upgrade_to_quattro" >/dev/null
grep -F '/usr/share/omisu/install/config/snapper.sh' "$upgrade_to_quattro" >/dev/null
grep -F 'bash -euo pipefail "$snapper_config_script"' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade normalizes Snapper retention"

grep -F 'configure_lock_authentication' "$upgrade_to_quattro" >/dev/null
grep -F 'OMISU_INSTALL_USER="$target_user"' "$upgrade_to_quattro" >/dev/null
grep -F '"$apply_lock"' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade configures lock screen authentication for the target user"

grep -F 'OMISU_UPGRADE_TO_QUATTRO_LIVE=1' "$upgrade_to_quattro" >/dev/null
...
pass "OmiSu 4 upgrade retires systemd-networkd for NetworkManager"
...
if function_body cleanup_retired_services | grep -F 'systemctl disable iwd' >/dev/null; then
  fail "OmiSu 4 upgrade does not retire iwd in a step separate from the NetworkManager enable"
fi
grep -A1 -F '  enable_system_service NetworkManager.service' "$upgrade_to_quattro" |
  ...
  fail "OmiSu 4 upgrade retires iwd in the step that enables NetworkManager"
pass "OmiSu 4 upgrade switches from iwd to NetworkManager atomically"
...
grep -Fx 'trap cleanup_on_exit EXIT' "$upgrade_to_quattro" >/dev/null ||
  fail "OmiSu 4 upgrade reports an aborted run instead of exiting silently"
...
grep -F 'exit "$exit_status"' <<<"$cleanup_body" >/dev/null ||
  fail "OmiSu 4 upgrade preserves the failing exit status"
grep -F '>&2' <<<"$cleanup_body" >/dev/null ||
  fail "OmiSu 4 upgrade reports an aborted run on stderr"
...
pass "OmiSu 4 upgrade reports an aborted run instead of exiting silently"
...
! grep -q 'start_omisu_shell_session' "$upgrade_to_quattro" ||
  fail "OmiSu 4 upgrade does not start the shell in the session it is replacing"
! grep -q 'stop_retired_session_processes' "$upgrade_to_quattro" ||
  fail "OmiSu 4 upgrade leaves the retired session processes running until reboot"
pass "OmiSu 4 upgrade leaves the OmiSu 3 session alone until the reboot"

grep -F 'omisu-bar defaults' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade restores service-aware bar defaults"
...
pass "OmiSu 4 upgrade backfills hardware support from the legacy release"

grep -F 'omisu-refresh-applications' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade refreshes application launchers"

grep -F '/etc/systemd/system.conf.d/99-omisu-nofile.conf' "$upgrade_to_quattro" >/dev/null
grep -F '/etc/systemd/user.conf.d/99-omisu-nofile.conf' "$upgrade_to_quattro" >/dev/null
pass "OmiSu 4 upgrade removes stale nofile drop-ins"

cmdline_line=$(grep -n '^preserve_kernel_cmdline_root$' "$upgrade_to_quattro" | cut -d: -f1)
packages_line=$(grep -n '^install_omisu_quattro_packages$' "$upgrade_to_quattro" | cut -d: -f1)
...
pass "OmiSu 4 upgrade preserves the kernel cmdline root parameters"
...
pass "OmiSu 4 upgrade asks limine-entry-tool whether root= survives"
...
pass "OmiSu 4 upgrade repair path refuses a partial dm-crypt cmdline"
...
grep -F "as_root find /boot/EFI/Linux -maxdepth 1 -name 'omisu_linux*.efi'" "$upgrade_to_quattro" >/dev/null
...
pass "OmiSu 4 upgrade verifies the UKIs and refuses to reboot unverified"
