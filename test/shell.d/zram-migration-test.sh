...
dropin="$TMPDIR/90-omisu.conf"
: >"$dropin"

# omisu-migrate runs each migration with `bash -euo pipefail` and stops the
# whole chain on a non-zero exit, so match that invocation exactly.
run_migration() {
  local conf="$1"
  PATH="$stub_bin:$PATH" OMISU_ZRAM_CONF="$conf" OMISU_ZRAM_DROPIN="$dropin" \
...
printf '[zram0]\ncompression-algorithm = zstd\n' >"$conf"
PATH="$stub_bin:$PATH" PACMAN_OWNS=1 OMISU_ZRAM_CONF="$conf" OMISU_ZRAM_DROPIN="$dropin" \
  bash -euo pipefail "$migration" >/dev/null ||
...
printf '[zram0]\ncompression-algorithm = zstd\n' >"$conf"
PATH="$stub_bin:$PATH" OMISU_ZRAM_CONF="$conf" OMISU_ZRAM_DROPIN="$TMPDIR/absent-dropin.conf" \
  bash -euo pipefail "$migration" >/dev/null ||
