...
sleep_monitor="$ROOT/bin/omisu-system-sleep-monitor"
tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

mock_bin="$tmpdir/bin"
mock_omisu="$tmpdir/omisu"
producer_pid_file="$tmpdir/producer-pid"
lock_log="$tmpdir/lock-log"
mkdir -p "$mock_bin" "$mock_omisu/bin"
...
cat >"$mock_omisu/bin/omisu-system-sleep-lock" <<'SH'
#!/bin/bash
...
chmod +x \
  ...
  "$mock_omisu/bin/omisu-system-sleep-lock"
ln -s "$sleep_monitor" "$mock_omisu/bin/omisu-system-sleep-monitor"

start_us=${EPOCHREALTIME//[!0-9]/}
OMISU_PATH="$mock_omisu" \
  PATH="$mock_bin:$PATH" \
