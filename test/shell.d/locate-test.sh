...
OMISU_UPDATEDB_CONF_PATH="$conf" bash -euo pipefail "$config_script" >/dev/null
...
OMISU_UPDATEDB_CONF_PATH="$test_tmp/missing.conf" bash -euo pipefail "$config_script" >/dev/null
...
# checked-out tree may not have yet, and omisu-migrate runs under set -e.
: >"$test_tmp/calls.log"
