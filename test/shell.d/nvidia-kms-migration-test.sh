...
packaged_hooks="$ROOT/etc/mkinitcpio.conf.d/omisu_hooks.conf"
...
cat >"$stub_bin/omisu-cmd-present" <<'SH'
#!/bin/bash
...
hooks_conf="$test_tmp/omisu_hooks.conf"
...
run_migration() {
  PATH="$stub_bin:$PATH" \
    TEST_LOG="$calls" \
    OMISU_MKINITCPIO_HOOKS_CONF="$hooks_conf" \
    OMISU_MKINITCPIO_NVIDIA_CONF="$nvidia_conf" \
    OMISU_KMS_REBUILD_MARKER="$rebuild_marker" \
    OMISU_PCI_DEVICES_PATH="$test_tmp/devices" \
