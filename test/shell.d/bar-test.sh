...
# OMISU_PATH. Point it at this checkout rather than whichever install launched
# the test (or nothing at all on a non-OmiSu development machine).
...
const drawn = { moduleName: 'omisu.clock', visible: true, width: 28, height: 81 }
const placeholder = { moduleName: 'omisu.clock', visible: false, width: 0, height: 0 }
...
const internal = { moduleName: 'omisu.audio', visible: true, width: 28, height: 81 }
const external = { moduleName: 'omisu.audio', visible: true, width: 28, height: 81 }
...
assertDeepEqual(bar.entrySettings({ id: 'omisu.clock', format: 'HH:mm' }), { format: 'HH:mm' }, 'bar extracts entry settings')
assertEqual(bar.entryId({ id: 'omisu.clock' }), 'omisu.clock', 'bar extracts object entry ids')
assertEqual(bar.entryId('omisu.clock'), 'omisu.clock', 'bar extracts string entry ids')

const entries = [{ id: 'a' }, { id: 'omisu.tray' }, { id: 'b' }]
assertDeepEqual(bar.pinTrayToInner(entries, 'left').map(bar.entryId), ['a', 'b', 'omisu.tray'], 'bar pins tray to left inner edge')
assertDeepEqual(bar.pinTrayToInner(entries, 'right').map(bar.entryId), ['omisu.tray', 'a', 'b'], 'bar pins tray to right inner edge')
...
const settingsLayout = { left: [{ id: 'omisu.power' }], center: [{ id: 'omisu.clock', format: 'HH:mm' }], right: [] }
assertDeepEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'omisu.power', showPercentage: true }], center: [{ id: 'omisu.clock', format: 'HH:mm' }], right: [] }),
  [{ region: 'left', index: 0, entry: { id: 'omisu.power', showPercentage: true } }],
...
assertEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'omisu.clock', format: 'HH:mm' }], center: [{ id: 'omisu.power' }], right: [] }),
  null,
  'bar treats reordered entries as structural'
)
assertEqual(
  bar.inlineSettingsDelta(settingsLayout, { left: [{ id: 'omisu.power' }, { id: 'omisu.battery' }], center: settingsLayout.center, right: [] }),
  null,
...
assertDeepEqual(bar.entriesBefore(entries, 'b').map(bar.entryId), ['a', 'omisu.tray'], 'bar returns entries before target')
assertDeepEqual(bar.entriesAfter(entries, 'a').map(bar.entryId), ['omisu.tray', 'b'], 'bar returns entries after target')
...
assertEqual(
  bar.customModulePath({ id: 'local.weather' }, '/home/dhh', '/home/dhh/.config/omisu'),
  '/home/dhh/.config/omisu/bar/modules/local.weather.qml',
...
mkdir -p "$put_tmp/bin"
ln -s "$ROOT/bin/omisu-shell-config" "$put_tmp/bin/omisu-shell-config"

cat >"$put_tmp/bin/omisu-shell" <<'STUB'
#!/bin/bash
case ${OMISU_TEST_SHELL_STATE:-ready} in
  missing)
    echo "omisu-shell is not running" >&2
    exit 1
    ;;
  starting)
    echo "omisu-shell is not ready" >&2
    exit 1
    ;;
  crashing)
    # Seen coming up, then gone.
    if [[ -e $OMISU_TEST_SHELL_MARKER ]]; then
      echo "omisu-shell is not running" >&2
    else
      touch "$OMISU_TEST_SHELL_MARKER"
      echo "omisu-shell is not ready" >&2
    fi
    exit 1
    ;;
  oldshell)
    ...
    if [[ $4 == *"after"* ]]; then
      echo "could not find target widget omisu.clock"
    else
      echo "ok"
    fi
    exit 0
...
echo "ok"
STUB
chmod +x "$put_tmp/bin/omisu-shell"

put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=missing \
  OMISU_SHELL_ABSENT_ATTEMPTS=2 \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) ||
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=spawning \
  OMISU_TEST_SHELL_MARKER="$put_tmp/spawned" \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) ||
  fail "put waits for a shell that is being spawned" "$put_output"
[[ $put_output == "omisu.keyboard-layout is on the bar" ]] || fail "put places once the shell answers" "$put_output"
pass "put waits for a shell that is being spawned"

put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=starting OMISU_SHELL_READY_ATTEMPTS=2 \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=crashing \
  OMISU_TEST_SHELL_MARKER="$put_tmp/started" \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=oldshell \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) ||
  fail "put falls back against a shell that has not restarted yet" "$put_output"
[[ $put_output == "omisu.keyboard-layout is on the bar" ]] || fail "put places without the missing neighbour" "$put_output"
pass "put falls back against a shell that has not restarted yet"

put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=vanishing \
  OMISU_TEST_SHELL_MARKER="$put_tmp/vanished" \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=unsupported \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) &&
...
put_output=$(PATH="$put_tmp/bin:$ROOT/bin:$PATH" OMISU_TEST_SHELL_STATE=scanning \
  OMISU_TEST_SHELL_MARKER="$put_tmp/scanned" \
  "$ROOT/bin/omisu-bar" put omisu.keyboard-layout --after omisu.clock 2>&1) ||
  fail "put asks again while the shell is still reading its plugins" "$put_output"
[[ $put_output == "omisu.keyboard-layout is on the bar" ]] || fail "put places once the plugins are read" "$put_output"
...
[[ $put_output == "omisu.keyboard-layout is on the bar" ]] || fail "put reports the placed widget" "$put_output"
