...
weather_location() {
  HOME="$test_tmp" "$ROOT/bin/omisu-weather-location" "$@"
}

weather_location --set "Malibu" "34.02577,-118.7804"
[[ $(jq -c . "$test_tmp/.local/state/omisu/settings/weather.json") == '{"name":"Malibu","latitude":34.02577,"longitude":-118.7804}' ]] || fail "weather location stores name and coordinates as JSON"
...
[[ $(jq -c . "$test_tmp/.local/state/omisu/settings/weather.json") == '{"name":"New York"}' ]] || fail "weather location stores a bare name as JSON"
...
[[ ! -e "$test_tmp/.local/state/omisu/settings/weather.json" ]] || fail "weather location clear removes the state file"
