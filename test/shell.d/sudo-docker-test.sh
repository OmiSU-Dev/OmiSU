#!/bin/bash
#
# omisu-sudo-docker is the single answer to "does Docker need sudo", and it
...
command="$ROOT/bin/omisu-sudo-docker"
...
run() { # SOCKET GROUPS [--configured]
  env PATH="$TMPDIR/bin:$PATH" OMISU_DOCKER_SOCKET="$1" STUB_GROUPS="$2" USER=tester \
    bash "$command" ${3:+"$3"}
}
