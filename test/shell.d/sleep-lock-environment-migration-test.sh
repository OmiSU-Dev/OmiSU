...
case "$*" in
  ...
  '--user show --property=ActiveState --value graphical-session.target')
    ...
    ;;
  '--user show --property=ActiveState --value omisu-sleep-lock.service')
    printf '%s\n' "${SLEEP_LOCK_STATE:-inactive}"
    ;;
  '--user restart omisu-sleep-lock.service')
    if [[ ${FAIL_SYSTEMCTL_ACTION:-} == "restart" ]]; then
      echo "restart failed" >&2
      exit 1
    fi
    ;;
  '--user stop omisu-sleep-lock.service')
    if [[ ${FAIL_SYSTEMCTL_ACTION:-} == "stop" ]]; then
      echo "stop failed" >&2
      exit 1
    fi
    ;;
  '--user reset-failed omisu-sleep-lock.service') ;;
  *) exit 1 ;;
esac
STUB
...
printf '%s\n' '[Service]' 'ExecStart=/usr/bin/omisu-system-sleep-monitor' \
  >"$active_home/.config/systemd/user/omisu-sleep-lock.service"

run_migration "$active_home" "$active_calls" \
  env GRAPHICAL_STATE=active SLEEP_LOCK_STATE=active >/dev/null

dropin="$active_home/.config/systemd/user/omisu-sleep-lock.service.d/90-omisu-session-environment.conf"
...
grep -Fx 'ConditionEnvironment=OMISU_PATH' "$dropin" >/dev/null ||
  fail "sleep lock migration gates a retained unit on the OmiSu path"
...
grep -Fx -- '--user reset-failed omisu-sleep-lock.service' "$active_calls" >/dev/null ||
  fail "sleep lock migration cannot recover a start-limited monitor"
grep -Fx -- '--user restart omisu-sleep-lock.service' "$active_calls" >/dev/null ||
  fail "sleep lock migration leaves an active monitor with its inherited environment"
grep -Fx -- '--user stop omisu-sleep-lock.service' "$active_calls" >/dev/null &&
...
grep -Fx -- '--user stop omisu-sleep-lock.service' "$inactive_calls" >/dev/null ||
  fail "sleep lock migration leaves a stale monitor running after logout"
grep -Fx -- '--user restart omisu-sleep-lock.service' "$inactive_calls" >/dev/null &&
...
grep -F 'will be retried by omisu-migrate' "$test_tmp/failed-output" >/dev/null ||
...
grep -F 'Could not stop stale omisu-sleep-lock.service' "$test_tmp/stop-failed-output" >/dev/null ||
...
touch "$deferred_home/.config/systemd/user/omisu-sleep-lock.service"

run_migration "$deferred_home" "$deferred_calls" \
  env MANAGER_AVAILABLE=false >/dev/null

[[ -f $deferred_home/.config/systemd/user/omisu-sleep-lock.service.d/90-omisu-session-environment.conf ]] ||
