...
#!/bin/bash
[[ ! -e $OMISU_TEST_INSTALLED_DIR/$1 ]]
SH

cat >"$mock_bin/omisu-launch-floating-terminal-with-presentation" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >"$OMISU_TEST_TERMINAL_LOG"
SH

cat >"$mock_bin/omisu-notification-send" <<'SH'
#!/bin/bash
printf '%s\0' "$@" >>"$OMISU_TEST_NOTIFICATION_LOG"
SH

cat >"$mock_bin/omisu-test-setup-call" <<'SH'
#!/bin/bash
printf '%s:%s\n' "${0##*/}" "$*" >>"$OMISU_TEST_SETUP_LOG"
[[ ${OMISU_TEST_SETUP_FAIL:-} != "${0##*/}" ]]
SH

cat >"$mock_bin/sudo" <<'SH'
#!/bin/bash
printf 'sudo:%s\n' "$*" >>"$OMISU_TEST_SETUP_LOG"
[[ ${OMISU_TEST_SETUP_FAIL:-} != "sudo" ]]
SH

cat >"$mock_bin/xdg-settings" <<'SH'
#!/bin/bash
case $1 in
get) [[ -f $OMISU_TEST_BROWSER_FILE ]] && cat "$OMISU_TEST_BROWSER_FILE" ;;
set) printf '%s\n' "$3" >"$OMISU_TEST_BROWSER_FILE" ;;
esac
SH

cat >"$mock_bin/omisu-test-installer" <<'SH'
#!/bin/bash
installer=${0##*/}

if [[ $installer == "omisu-install-browser" && ${OMISU_TEST_REAL_BROWSER_INSTALL:-false} == "true" ]]; then
  exec "$ROOT/bin/omisu-install-browser" "$@"
fi

case $installer in
omisu-pkg-add)
  package=$1
  printf 'pkg:%s\n' "$package" >>"$OMISU_TEST_INSTALL_LOG"
  case $package in
  ...
  ;;
omisu-install-browser)
  selection=$1
  printf 'browser:%s\n' "$selection" >>"$OMISU_TEST_INSTALL_LOG"
  case $selection in
  chromium) command=chromium ;;
  ...
  ;;
omisu-install-terminal)
  command=$1
  printf 'terminal:%s\n' "$command" >>"$OMISU_TEST_INSTALL_LOG"
  ;;
omisu-install-editor-*)
  editor=${installer#omisu-install-editor-}
  printf 'editor:%s\n' "$editor" >>"$OMISU_TEST_INSTALL_LOG"
  case $editor in
  vscode) command=code ;;
  zed) command=zeditor ;;
  helix) command=helix ;;
...
[[ ${OMISU_TEST_INSTALL_FAIL:-false} != "true" ]] || exit 1
touch "$OMISU_TEST_INSTALLED_DIR/$command"
SH
...
for installer in \
  ...
  omisu-install-browser \
  omisu-install-terminal \
  omisu-install-editor-vscode \
  omisu-install-editor-zed \
  omisu-install-editor-helix \
  omisu-install-editor-emacs; do
  ln -s omisu-test-installer "$mock_bin/$installer"
done
for setup_command in \
  omisu-install-chromium-copy-url \
  omisu-install-chromium-ytdlp \
  omisu-theme-set-browser; do
  ln -s omisu-test-setup-call "$mock_bin/$setup_command"
done
...
export OMISU_TEST_INSTALLED_DIR="$installed_dir"
export OMISU_TEST_INSTALL_LOG="$install_log"
export OMISU_TEST_TERMINAL_LOG="$terminal_log"
export OMISU_TEST_NOTIFICATION_LOG="$notification_log"
export OMISU_TEST_SETUP_LOG="$setup_log"
export OMISU_TEST_BROWSER_FILE="$browser_file"
...
assert_missing_opens_installer() {
  ...
  : >"$terminal_log"
  "omisu-default-$type" "$selection"
  mapfile -d '' -t terminal_args <"$terminal_log"
  [[ ${terminal_args[*]} == "omisu-default-$type --install $selection" ]] ||
...
for entry in "${browser_cases[@]}"; do
  ...
  : >"$install_log"
  omisu-default-browser --install "$selection"
  [[ $(<"$install_log") == "$installer" ]] || fail "$selection uses its browser installer"
  [[ $(omisu-default-browser) == "$selection" ]] || fail "$selection becomes the default browser after installation"
done
...
OMISU_TEST_REAL_BROWSER_INSTALL=true omisu-default-browser --install chromium >/dev/null
[[ $(<"$install_log") == "pkg:chromium" ]] || fail "Chromium browser installer installs the package"
[[ $(omisu-default-browser) == "chromium" ]] || fail "Chromium becomes the default after its full installer succeeds"
...
grep -Fxq 'omisu-install-chromium-copy-url:' "$setup_log" ||
  fail "Chromium browser installer registers the Copy URL host"
grep -Fxq 'omisu-install-chromium-ytdlp:' "$setup_log" ||
  fail "Chromium browser installer registers the yt-dlp host"
grep -Fxq 'omisu-theme-set-browser:' "$setup_log" ||
  fail "Chromium browser installer applies the current theme"
pass "Chromium browser installer restores the complete OmiSu setup"

omisu-default-browser zen
rm -f "$installed_dir/chromium"
if OMISU_TEST_REAL_BROWSER_INSTALL=true OMISU_TEST_INSTALL_FAIL=true \
  omisu-default-browser --install chromium >"$test_tmp/browser-package-failure" 2>&1; then
  fail "failed Chromium package installation returns an error"
fi
[[ $(omisu-default-browser) == "zen" ]] || fail "failed Chromium package installation preserves the default browser"
...
if OMISU_TEST_REAL_BROWSER_INSTALL=true OMISU_TEST_SETUP_FAIL=sudo \
  omisu-default-browser --install chromium >"$test_tmp/browser-install-failure" 2>&1; then
  fail "failed Chromium setup returns an error"
fi
[[ $(omisu-default-browser) == "zen" ]] || fail "failed Chromium setup preserves the default browser"
...
for entry in "${terminal_cases[@]}"; do
  ...
  : >"$install_log"
  omisu-default-terminal --install "$selection"
...
for entry in "${editor_cases[@]}"; do
  ...
  rm -f "$installed_dir/$command"
  : >"$install_log"
  omisu-default-editor --install "$selection"
  [[ $(<"$install_log") == "$installer" ]] || fail "$selection uses its editor installer"
  [[ $(omisu-default-editor) == "$command" ]] || fail "$selection becomes the default editor after installation"
done
...
: >"$terminal_log"
omisu-default-browser zen
omisu-default-terminal kitty
omisu-default-editor nvim
[[ ! -s $install_log && ! -s $terminal_log ]] || fail "installed defaults skip installation"
pass "installed defaults are selected immediately"

previous_editor=$(omisu-default-editor)
rm -f "$installed_dir/vim"
if OMISU_TEST_INSTALL_FAIL=true omisu-default-editor --install vim >"$test_tmp/install-failure" 2>&1; then
  fail "failed default installation returns an error"
fi
[[ $(omisu-default-editor) == "$previous_editor" ]] || fail "failed installation preserves the default"
