...
flag_dir="$home_dir/.local/state/omisu/toggles/hypr"
...
make_stub omisu-notification-send ':'
make_stub omisu-hyprland-monitor-external-active 'exit 0'
make_stub omisu-hyprland-toggle-disabled 'exit 0'
make_stub omisu-hyprland-toggle ':'
make_stub omisu-hyprland-monitor-internal ':'
make_stub omisu-hyprland-monitor-internal-mirror ':'
make_stub omisu-hw-clamshell 'exit 0'
make_stub omisu-hyprland-monitor-laptop 'printf "%s\n" "$LAPTOP_NAME"'
...
run_monitor omisu-hyprland-monitor-internal off
...
LAPTOP_NAME='eDP-1", disabled = false })os.execute("calc")--' \
  run_monitor omisu-hyprland-monitor-internal off >/dev/null 2>&1
status=$?
set -e
...
run_monitor omisu-hyprland-monitor-internal-mirror on
...
set +e
run_monitor omisu-hyprland-monitor-internal-mirror on >/dev/null 2>&1
status=$?
set -e
...
printf '[{"name":"eDP-1"}]\n' >"$monitors_json"
rm -f "$clamshell_flag"
run_monitor omisu-hyprland-monitor-clamshell
...
LAPTOP_NAME='eDP-1", disabled = true })os.execute("calc")--' \
  run_monitor omisu-hyprland-monitor-clamshell >/dev/null 2>&1
status=$?
set -e
...
printf '[{"name":"eDP-1","focused":true,"scale":1.0,"width":1920,"height":1080,"refreshRate":60.0}]\n' \
  >"$monitors_json"
run_monitor omisu-hyprland-monitor-scaling 1.6
...
printf '[{"name":"eDP-1\\" })os.execute(\\"calc\\")--","focused":true,"scale":1.0,"width":1920,"height":1080,"refreshRate":60.0}]\n' \
  >"$monitors_json"
set +e
run_monitor omisu-hyprland-monitor-scaling 1.6 >/dev/null 2>&1
status=$?
set -e
