...
assert(/function toggleBluetooth\(\)[\s\S]*?execDetached\(\["omisu-bluetooth-power", adapter\.enabled \? "off" : "on"\]\)/.test(panelSource), 'bluetooth toggles the radio through the rfkill soft block')
...
# $ROOT/bin so omisu-bluetooth-device resolves the real omisu-bluetooth-power.
bluetooth_run() {
  local powered="$1"
  shift
  ...
  PATH="$mock_bin:$ROOT/bin:$PATH" BLUETOOTHCTL_LOG="$device_tmp/log" \
    OMISU_BLUETOOTH_POWER_WAIT_SECONDS=0 "$@" ||
    fail "$* exits cleanly with Powered: $powered"
  printf '%s' "$device_tmp/log"
}

bluetooth_power() {
  bluetooth_run "$1" "$ROOT/bin/omisu-bluetooth-power" "$2"
}
...
bluetooth_device_log() {
  bluetooth_run "$1" "$ROOT/bin/omisu-bluetooth-device" connect AA:BB:CC:DD:EE:FF
}
