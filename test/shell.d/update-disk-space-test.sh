...
TEST_AVAILABLE_BYTES=$((9 * 1024 * 1024 * 1024)) \
  PATH="$stub_bin:$ROOT/bin:$PATH" \
  "$ROOT/bin/omisu-update-requires-free-space" >/dev/null
status=$?
set -e
...
[[ $output == *"You need at least 10 GiB free to safely update OmiSu."* ]] || fail "low disk space emits a warning"
...
[[ $output == *"You need at least 10 GiB free to safely update OmiSu."* ]] || fail "interactive low-space update explains the requirement"
...
rm -f "$snapshot_marker" "$gum_marker"
output=$(OMISU_UPDATE_FORCE=1 run_update -y)
