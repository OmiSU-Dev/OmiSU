...
run_guard() {
  OMISU_PACMAN_CMDLINE="$1" "$ROOT/bin/omisu-update-pacman-guard"
}
...
grep -q 'omisu update' "$test_tmp/direct.err" || fail "pacman guard explains omisu update entrypoint"
...
OMISU_UPDATE_PACMAN=1 run_guard "pacman -Syu --noconfirm" >"$test_tmp/omisu.out" 2>"$test_tmp/omisu.err"
[[ ! -s $test_tmp/omisu.err ]] || fail "pacman guard stays quiet for omisu update pacman call"
pass "pacman guard allows omisu update pacman call"

OMISU_ALLOW_DIRECT_PACMAN=1 run_guard "pacman -Syu --noconfirm" >"$test_tmp/override.out" 2>"$test_tmp/override.err"
