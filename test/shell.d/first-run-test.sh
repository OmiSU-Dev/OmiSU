...
cat >"$mock_bin/omisu-done" <<'SH'
#!/bin/bash
[[ $1 == "check" && $2 == "first-run-user" ]]
SH
cat >"$mock_bin/omisu-provision-user" <<'SH'
#!/bin/bash
touch "$OMISU_TEST_FINALIZE_CALLED"
SH
chmod +x "$mock_bin/omisu-done" "$mock_bin/omisu-provision-user"

finalize_called="$test_tmp/finalize-called"
HOME="$test_tmp/home" PATH="$mock_bin:$PATH" OMISU_TEST_FINALIZE_CALLED="$finalize_called" \
  bash "$ROOT/bin/omisu-provision-first-run" >"$test_tmp/output"
...
if grep -F 'user-migration-notify-watch-enabled' "$ROOT/bin/omisu-provision-first-run" >/dev/null; then
