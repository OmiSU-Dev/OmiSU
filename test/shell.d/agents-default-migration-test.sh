...
home="$test_dir/home"
config="$home/.config/omisu/shell.json"
...
write_config() {
  rm -rf "$home"
  mkdir -p "$home/.config/omisu"
  jq "${1:-.}" "$ROOT/config/omisu/shell.json" >"$config"
}

without_widget='del(.bar.layout[][] | select((if type == "object" then .id else . end) == "omisu.agents"))'
...
jq -e '[.bar.layout.right[].id] | index("omisu.agents")' "$ROOT/config/omisu/shell.json" >/dev/null ||
...
[[ $(ids right) == '["omisu.tray","omisu.agents","omisu.bluetooth","omisu.network","omisu.audio","omisu.monitor","omisu.power"]' ]] ||
  fail "migration inserts the agents widget after the tray" "$(ids right)"
pass "migration inserts the agents widget after the tray"

(($(wc -l <"$SHELL_RESTARTS") == 0)) || fail "migration leaves the shell restart to omisu update"
pass "migration leaves the shell restart to omisu update"
...
write_config "$without_widget | .bar.layout.center += [{ id: \"omisu.agents\" }]"
run_migration

[[ $(ids center) == *'"omisu.agents"'* ]] || fail "migration leaves a user-placed widget alone" "$(ids center)"
[[ $(ids right) != *'"omisu.agents"'* ]] || fail "migration does not add a second copy" "$(ids right)"
pass "migration respects a widget the user already placed"

# Layouts written before entries grew options are bare id strings.
write_config "$without_widget | .bar.layout.right = [\"omisu.tray\", \"omisu.agents\", \"omisu.power\"]"
run_migration

[[ $(ids right) == '["omisu.tray","omisu.agents","omisu.power"]' ]] ||
...
write_config "$without_widget | del(.bar.layout.right[] | select(.id == \"omisu.tray\"))"
run_migration

[[ $(ids right) == '["omisu.agents",'* ]] || fail "migration places the widget without a tray" "$(ids right)"
