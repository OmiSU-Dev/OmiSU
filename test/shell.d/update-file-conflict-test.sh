...
run_update() {
  OMISU_REPLACED_DIR="$replaced" \
    ...
    bash "$ROOT/bin/omisu-update-system-pkgs"
}
...
fresh_work
stray="$work/omisu-fcitx5.service"
echo "stray content" >"$stray"
write_report omisu-settings-dev "$stray"
...
# A real fight between packages, not OmiSu's leftovers. pacman appends
# "(owned by ...)" here.
fresh_work
echo "theirs" >"$stray"
write_report omisu-settings-dev "$stray" someone-else
...
fresh_work
echo "stray" >"$stray"
write_report omisu-chromium-bin "$stray"
if run_update >"$test_tmp/out" 2>"$test_tmp/err"; then
  fail "an optional omisu-prefixed package gets its conflicts auto-resolved"
fi
pass "only the packages that own system paths get their conflicts resolved"

# Not OmiSu's conflict to resolve.
fresh_work
echo "stray" >"$stray"
...
pass "a conflict from a non-OmiSu package is left for a human"

# The path is used literally, so glob characters in a name mean nothing.
fresh_work
globby="$work/omisu-[1].conf"
echo "globby" >"$globby"
write_report omisu-settings-dev "$globby"
run_update >"$test_tmp/out" 2>"$test_tmp/err" ||
...
fresh_work
conflict_dir="$work/omisu-dir"
mkdir -p "$conflict_dir"
write_report omisu-settings-dev "$conflict_dir"
run_update >"$test_tmp/out" 2>"$test_tmp/err" ||
...
fresh_work
spaced="$work/omisu theme.conf"
echo "spaced" >"$spaced"
write_report omisu-settings-dev "$spaced"
run_update >"$test_tmp/out" 2>"$test_tmp/err" ||
...
echo "ours" >"$stray"
write_raw_report \
  "omisu-settings-dev: $stray exists in filesystem" \
...
echo "ours" >"$stray"
write_report omisu-settings-dev "$stray"
if PATH="$stub_bin:$ROOT/bin:$PATH" OMISU_REPLACED_DIR="$replaced" \
  bash "$ROOT/bin/omisu-update-system-pkgs-when-conflicted" "$test_tmp/report" \
