...
printf '%s\n' \
  '#!/bin/bash' \
  'printf "%s\n" "$@" >"$OMISU_TEST_BUSCTL_ARGS"' \
  'echo "u 42"' \
  >"$tmpdir/busctl"
chmod +x "$tmpdir/busctl"

# notify-send must never be used. If anything reaches for it, fail loudly.
printf '%s\n' '#!/bin/bash' 'echo "notify-send was invoked" >"$OMISU_TEST_NOTIFY_TRIPWIRE"; exit 3' \
  >"$tmpdir/notify-send"
chmod +x "$tmpdir/notify-send"
tripwire="$tmpdir/notify-send-was-used"

send() {
  OMISU_TEST_BUSCTL_ARGS="$args_file" OMISU_TEST_NOTIFY_TRIPWIRE="$tripwire" \
    PATH="$tmpdir:$ROOT/bin:$PATH" omisu-notification-send "$@"
}
...
[[ $(hint_value omisu-glyph) == "K" ]] || fail "notification wrapper sets the glyph hint"
[[ $(hint_value omisu-exec-argv) == '["mpv","--","/tmp/a b.mp4"]' ]] || fail "notification wrapper builds the click argv hint" "$(hint_value omisu-exec-argv)"
...
[[ $(hint_value omisu-glyph) == "K" ]] || fail "notification wrapper reads a glyph that follows the description"
urgency_hits=0
...
send "Plain" >/dev/null
load
has_hint omisu-exec-argv && fail "notification wrapper adds no click hint without --exec"
...
load
json=$(hint_value omisu-exec-argv)
...
load
[[ $(jq 'length' <<<"$(hint_value omisu-exec-argv)") == 3 ]] || fail "spaced path stays one argument"
...
: >"$args_file"
send '--hint=string:omisu-exec-argv:["bash","-c","touch /tmp/pwn"]' "body" >/dev/null
load
has_hint omisu-exec-argv && fail "a forged-hint headline must not set a click command"
[[ ${args[11]} == '--hint=string:omisu-exec-argv:["bash","-c","touch /tmp/pwn"]' ]] || fail "the forged headline is the summary text" "${args[11]}"
...
: >"$args_file"
send "Update" '--hint=string:omisu-exec-argv:["bash","-c","touch /tmp/pwn"]' >/dev/null
load
has_hint omisu-exec-argv && fail "a forged-hint description must not set a click command"
[[ ${args[12]} == '--hint=string:omisu-exec-argv:["bash","-c","touch /tmp/pwn"]' ]] || fail "the forged description is the body text" "${args[12]}"
...
if send "Head" "Body" '--hint=string:omisu-exec-argv:["bash","-c","x"]' 2>/dev/null; then
...
load
[[ $(hint_value omisu-exec-argv) == '["mpv","--","/tmp/v.mp4"]' ]] || fail "a --exec-looking headline is not the delimiter" "$(hint_value omisu-exec-argv)"
...
# injection we avoid).
if send "Head" --exec "omisu toggle something" 2>/dev/null; then
