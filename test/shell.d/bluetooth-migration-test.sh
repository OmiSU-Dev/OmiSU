...
cat >"$test_dir/bin/omisu-bluetooth-power" <<'STUB'
#!/bin/bash

printf 'omisu-bluetooth-power %s\n' "$*" >>"$CALLS"
[[ $1 == "is-on" ]] || exit 0
[[ ${POWERED:-} == "yes" ]]
STUB
...
run_migration() {
  ...
  OMISU_BLUETOOTH_MIGRATION_MARKER="$marker" \
    OMISU_BLUETOOTH_MAIN_CONF="$main_conf" \
...
grep -qx 'omisu-bluetooth-power on' "$CALLS" ||
...
grep -qx 'omisu-bluetooth-power off' "$CALLS" ||
...
grep -qx 'sudo omisu-bluetooth-power off' "$CALLS" ||
...
# Only the exact line OmiSu wrote is reverted, so a hand-edited opt-out stands.
reset_machine
printf '[Policy]\nAutoEnable = false\n' >"$main_conf"
POWERED=yes run_migration
