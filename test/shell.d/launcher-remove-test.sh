...
#!/bin/bash
printf '%s:%s:%s\\n' '$prefix' "\${OMISU_REMOVE_NOTIFY:-}" "\$*" >>"\$TEST_LOG"
SCRIPT
  chmod +x "$tmp_dir/bin/$name"
}

write_fake_command omisu-webapp-remove web
write_fake_command omisu-tui-remove tui
write_fake_command omisu-launch-floating-terminal-with-presentation terminal

cat >"$tmp_dir/bin/omisu-notification-send" <<'SCRIPT'
#!/bin/bash
printf 'notify::%s\n' "$*" >>"$TEST_LOG"
SCRIPT
chmod +x "$tmp_dir/bin/omisu-notification-send"
...
[Desktop Entry]
Name=Basecamp
Exec=omisu-launch-webapp https://example.com
DESKTOP
...
"$ROOT/bin/omisu-remove-launcher-entry" Basecamp.desktop Basecamp
"$ROOT/bin/omisu-remove-launcher-entry" Docker.desktop Docker
"$ROOT/bin/omisu-remove-launcher-entry" native.desktop Native
"$ROOT/bin/omisu-remove-launcher-entry" aliens.desktop Aliens
