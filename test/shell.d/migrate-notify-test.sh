...
cat >"$stub_bin/omisu-migrate" <<'SH'
#!/bin/bash
if [[ ${1:-} == "--pending" && ${OMISU_TEST_PENDING_MIGRATIONS:-0} == 1 ]]; then
...
SH
chmod +x "$stub_bin/omisu-migrate"
...
cat >"$stub_bin/omisu-notification-wait" <<'SH'
#!/bin/bash
[[ ${OMISU_TEST_LOCK_DURING_WAIT:-0} == 1 ]] || exit 0

lock="$XDG_RUNTIME_DIR/omisu-update.lock"
held="$lock.held"
rm -f "$held"
: >"$lock"
...
echo "$!" >"$OMISU_TEST_LOCK_HOLDER_PID"
...
exit 1
SH
chmod +x "$stub_bin/omisu-notification-wait"

cat >"$stub_bin/omisu-notification-send" <<'SH'
#!/bin/bash
[[ ${OMISU_TEST_NOTIFY_SEND:-send} == "fail" ]] && exit 1
printf '%s\n' "$@" >"$OMISU_TEST_NOTIFY_ARGS"
SH
chmod +x "$stub_bin/omisu-notification-send"
...
run_notify() {
  ...
  OMISU_TEST_PENDING_MIGRATIONS="$1" \
  OMISU_TEST_NOTIFY_ARGS="$test_tmp/notify-args" \
  OMISU_TEST_NOTIFY_SEND="${2:-send}" \
  OMISU_TEST_LOCK_DURING_WAIT="${OMISU_TEST_LOCK_DURING_WAIT:-0}" \
  OMISU_TEST_LOCK_HOLDER_PID="$test_tmp/lock-holder-pid" \
    "$ROOT/bin/omisu-migrate-notify"
}
...
grep -q 'OmiSu has pending migrations' "$test_tmp/pending.err" || fail "migration notifier explains pending migrations without notification system"
...
grep -Fx 'Pending OmiSu Migrations' "$test_tmp/notify-args" >/dev/null || fail "migration notifier uses pending migrations title"
...
# `omisu update` applies migrations itself, so nothing may notify about them
# while it holds its lock -- a stale trigger firing mid-transaction is exactly
# how the retired omisu-update-user-notify.path used to interrupt updates.
rm -f "$test_tmp/notify-args"
update_lock="$runtime_dir/omisu-update.lock"
: >"$update_lock"
...
pass "migration notifier stays quiet while omisu update holds its lock"

exec {update_lock_fd}>&-

run_notify 1 >/dev/null 2>&1
notify_args_written &&
  grep -Fx 'Pending OmiSu Migrations' "$test_tmp/notify-args" >/dev/null ||
...
rm -f "$test_tmp/notify-args"
OMISU_TEST_LOCK_DURING_WAIT=1 run_notify 1 >"$test_tmp/raced.out" 2>"$test_tmp/raced.err"
if [[ -s $test_tmp/lock-holder-pid ]]; then
  kill "$(<"$test_tmp/lock-holder-pid")" 2>/dev/null || true
  for _ in {1..200}; do
    flock -n "$runtime_dir/omisu-update.lock" true 2>/dev/null && break
    sleep 0.05
  done
fi
...
rm -f "$test_tmp/notify-args" /tmp/omisu-update.lock
foreign_lock="$test_tmp/foreign/omisu-update.lock"
...
grep -Fx 'omisu-launch-floating-terminal-with-presentation' "$test_tmp/notify-args" >/dev/null &&
  grep -Fx 'omisu-migrate' "$test_tmp/notify-args" >/dev/null ||
  fail "migration notifier points the click command at omisu-migrate"
