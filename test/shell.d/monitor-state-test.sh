...
cat >"$test_bin/omisu-brightness-display" <<'EOF'
#!/bin/bash
echo 42
EOF

cat >"$test_bin/omisu-hyprland-monitor-scaling" <<'EOF'
#!/bin/bash
echo 1.5
EOF
...
monitor_state() {
  ...
  mapfile -t state_lines < <(
    FAKE_MONITORS="$monitors_file" PATH="$test_bin:$PATH" \
      bash "$ROOT/bin/omisu-monitor-state" 2>/dev/null
  )
}
...
# OmiSu mirrors by pointing the external at the internal, so `mirrorOf` lands
