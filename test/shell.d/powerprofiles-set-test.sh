...
export OMISU_POWERPROFILES_STATE_DIR="$tmp_dir/state"

"$ROOT/bin/omisu-powerprofiles-set" ac balanced
...
"$ROOT/bin/omisu-powerprofiles-set" ac
[[ $(tail -n 1 "$tmp_dir/calls") == "balanced" ]] || fail "power profile restores AC preference"
pass "power profile restores AC preference"

if POWERPROFILES_SET_FAIL=1 "$ROOT/bin/omisu-powerprofiles-set" ac performance; then
...
"$ROOT/bin/omisu-powerprofiles-set" battery performance
...
ON_BATTERY=1 "$ROOT/bin/omisu-powerprofiles-set"
...
rm "$tmp_dir/state/ac"
ON_BATTERY=0 "$ROOT/bin/omisu-powerprofiles-set"
...
"$ROOT/bin/omisu-powerprofiles-set" ac power-saver
"$ROOT/bin/omisu-powerprofiles-init"
...
rg -F '["omisu-powerprofiles-set", pendingPowerSource]' "$ROOT/shell/plugins/services/battery/Service.qml" >/dev/null ||
  fail "battery service applies profiles through OmiSu command"
pass "battery service applies profiles through OmiSu command"

rg -F 'omisu-powerprofiles-set autodetect' "$ROOT/shell/plugins/menu/Menu.qml" >/dev/null ||
  fail "power profile menu persists selections through OmiSu command"
pass "power profile menu persists selections through OmiSu command"
