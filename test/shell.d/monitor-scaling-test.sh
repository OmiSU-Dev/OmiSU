...
scale_log="$home_dir/.local/state/omisu/monitor-scaling.log"
...
if [[ $1 == "monitors" && $2 == "-j" ]]; then
  printf '[{"name":"eDP-1","focused":true,"scale":%s,"width":%s,"height":%s,"refreshRate":120.0}]' \
    "${OMISU_TEST_MONITOR_SCALE:-2}" "${OMISU_TEST_MONITOR_WIDTH:-2880}" "${OMISU_TEST_MONITOR_HEIGHT:-1800}"
elif [[ $1 == "eval" ]]; then
  printf '%s\n' "$2" >"$OMISU_TEST_HYPRCTL_EVAL_OUT"
else
  exit 1
fi
SH
...
write_monitor_config() {
  cat >"$monitor_lua" <<'LUA'
local omisu_gdk_scale = 2
local omisu_monitor_scale = 2
LUA
}
...
run_scaling() {
  HOME="$home_dir" \
    ...
    PATH="$stub_bin:$PATH" \
    OMISU_TEST_HYPRCTL_EVAL_OUT="$eval_out" \
    OMISU_TEST_MONITOR_SCALE="${OMISU_TEST_MONITOR_SCALE:-2}" \
    "$ROOT/bin/omisu-hyprland-monitor-scaling" "$@"
}

write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 run_scaling up
grep -F 'scale = 3' "$eval_out" >/dev/null || fail "monitor scaling up reaches 3x"
grep -Fx 'local omisu_monitor_scale = 3' "$monitor_lua" >/dev/null || fail "monitor scaling up persists 3x"
...
write_monitor_config
OMISU_TEST_MONITOR_SCALE=3 run_scaling down
grep -F 'scale = 2' "$eval_out" >/dev/null || fail "monitor scaling down recovers 3x to 2x"
grep -Fx 'local omisu_monitor_scale = 2' "$monitor_lua" >/dev/null || fail "monitor scaling down persists 2x from 3x"
pass "monitor scaling down recovers 3x to 2x"

write_monitor_config
OMISU_TEST_MONITOR_SCALE=3.0000000000000004 run_scaling down
grep -F 'scale = 2' "$eval_out" >/dev/null || fail "monitor scaling down snaps floating point 3x to 2x"
grep -Fx 'local omisu_monitor_scale = 2' "$monitor_lua" >/dev/null || fail "monitor scaling down persists 2x from floating point 3x"
pass "monitor scaling down snaps floating point 3x to 2x"

write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 run_scaling 3
grep -F 'scale = 3' "$eval_out" >/dev/null || fail "monitor scaling explicit 3x remains available"
grep -Fx 'local omisu_monitor_scale = 3' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 3x persists"
grep -Fx 'local omisu_gdk_scale = 3' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 3x persists GDK scale"
...
# rounded GDK scale.
write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 run_scaling 1.6
grep -F 'scale = 1.6' "$eval_out" >/dev/null || fail "monitor scaling explicit 1.6x remains available"
grep -Fx 'local omisu_monitor_scale = 1.6' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 1.6x persists"
grep -Fx 'local omisu_gdk_scale = 2' "$monitor_lua" >/dev/null || fail "monitor scaling 1.6x persists integer GDK scale 2"
pass "monitor scaling 1.6x persists integer GDK scale 2"

write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 run_scaling 1.25
grep -Fx 'local omisu_monitor_scale = 1.25' "$monitor_lua" >/dev/null || fail "monitor scaling explicit 1.25x persists"
grep -Fx 'local omisu_gdk_scale = 1' "$monitor_lua" >/dev/null || fail "monitor scaling 1.25x persists integer GDK scale 1"
pass "monitor scaling 1.25x persists integer GDK scale 1"

scale=$(OMISU_TEST_MONITOR_SCALE=3 run_scaling)
[[ $scale == "3" ]] || fail "monitor scaling reports explicit 3x scale" "actual: $scale"
pass "monitor scaling reports explicit 3x scale"

scale=$(OMISU_TEST_MONITOR_SCALE=3.2 run_scaling)
...
write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 OMISU_TEST_MONITOR_WIDTH=1280 OMISU_TEST_MONITOR_HEIGHT=800 run_scaling 3
grep -F 'scale = 3.2' "$eval_out" >/dev/null || fail "monitor scaling approximates explicit 3x as 3.2x"
grep -Fx 'local omisu_monitor_scale = 3.2' "$monitor_lua" >/dev/null ||
  fail "monitor scaling persists approximated 3.2x"
pass "monitor scaling approximates explicit 3x as 3.2x"

write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 OMISU_TEST_MONITOR_WIDTH=1280 OMISU_TEST_MONITOR_HEIGHT=800 run_scaling up
...
write_monitor_config
OMISU_TEST_MONITOR_SCALE=4 OMISU_TEST_MONITOR_WIDTH=1280 OMISU_TEST_MONITOR_HEIGHT=800 run_scaling down
...
write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 OMISU_TEST_MONITOR_WIDTH=6016 OMISU_TEST_MONITOR_HEIGHT=3384 run_scaling 1.25
...
write_monitor_config
OMISU_TEST_MONITOR_SCALE=2 OMISU_TEST_MONITOR_WIDTH=1280 OMISU_TEST_MONITOR_HEIGHT=800 run_scaling 3.2
...
write_monitor_config
OMISU_TEST_MONITOR_SCALE=4 OMISU_TEST_MONITOR_WIDTH=1280 OMISU_TEST_MONITOR_HEIGHT=804 run_scaling down
grep -F 'scale = 2' "$eval_out" >/dev/null || fail "monitor scaling down skips duplicate 4x approximation"
grep -Fx 'local omisu_monitor_scale = 2' "$monitor_lua" >/dev/null ||
