...
OmiSu configures Hyprland in Lua. User files are loaded after OmiSu's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads OmiSu defaults, then user files)
...
- Use `omisu refresh hyprland` to reset the Lua config files to defaults

The two `.conf` files are read by separate processes, so `hyprctl` neither
applies nor validates them:
- `hyprsunset.conf` (night light): apply changes with `omisu restart hyprsunset`; reset with `omisu refresh hyprsunset`
...
View current bindings: `omisu menu keybindings --print`

**IMPORTANT: When re-binding an existing key:**

1. First check existing bindings: `omisu menu keybindings --print`
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer OmiSu's `o.window(match, rules)` helper — see examples in `$OMISU_PATH/default/hypr/windows.lua`.
