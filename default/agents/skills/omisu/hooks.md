...
Hooks live in `~/.config/omisu/hooks/<name>.d/` — one directory per event,
holding any number of independent scripts. Install with
`omisu hook install <name> <script>` (copies the script in and makes it
executable). The runner also executes a flat `~/.config/omisu/hooks/<name>`
...
├── post-update.d/          # During `omisu update`, after system packages and migrations
├── pre-refresh-pacman.d/   # Before `omisu refresh pacman` re-syncs packages
