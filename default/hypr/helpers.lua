...
local function command_from(value, description)
  ...
  if value.omisu then
    return "omisu-launch-" .. value.omisu
  ...
  elseif value.tui then
    if value.focus then
      return "omisu-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "omisu-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.preinstalled_bindings_enabled()
  if _G.omisu_preinstalled_bindings ~= nil then
    return _G.omisu_preinstalled_bindings == true
  end

  return not file_exists((os.getenv("HOME") or "") .. "/.local/state/omisu/preinstalls-removed")
end
...
function o.launch_webapp(url)
  return "omisu-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "omisu-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "omisu-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "omisu-toggle-" .. toggle, options)
end

function o.notify(message)
  return "omisu-notification-send -u low " .. shell_quote(message)
end
