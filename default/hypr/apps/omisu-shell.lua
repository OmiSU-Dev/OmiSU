-- Window and layer rules for the OmiSu Quickshell surfaces. The
-- shell-wide bar / menu / popouts are layer-shell.

-- Keep the bar instant: no layer-shell fade/slide animation.
hl.layer_rule({ match = { namespace = "omisu-bar" }, no_anim = true, animation = "none" })
...
hl.layer_rule({ match = { namespace = "^(omisu-menu|omisu-image-selector|omisu-emojis|omisu-clipboard|omisu-keyboard-panel)$" }, no_anim = true, animation = "none" })

-- Dev gallery is the main shell workbench; open it maximized like
-- SUPER+ALT+F so component previews have the whole workspace.
o.window({ class = "^org.quickshell$", title = "^OmiSu shell – dev gallery$" }, { maximize = true })
