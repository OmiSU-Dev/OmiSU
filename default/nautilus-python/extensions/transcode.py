...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("omisu-launch-floating-terminal-with-presentation")
        binary = shutil.which("omisu-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="OmiSuTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("omisu-launch-floating-terminal-with-presentation")
            and shutil.which("omisu-transcode")
        )
