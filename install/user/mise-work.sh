...
# omisu-provision-owner finalizes the user at first boot.
case ${OMISU_SETUP_CONTEXT:-runtime} in
  iso-chroot) NODE_PACKAGE_DIR=/opt/packages ;;
  provision-owner) NODE_PACKAGE_DIR=/var/lib/omisu/provisioning/packages ;;
  *) NODE_PACKAGE_DIR="" ;;
esac
...
if [[ -n $NODE_PACKAGE_DIR ]]; then
  ...
  if [[ -z $NODE_TARBALL ]]; then
    if [[ ${OMISU_SETUP_CONTEXT:-} == "provision-owner" ]]; then
