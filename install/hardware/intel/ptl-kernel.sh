...
if omisu-hw-match "XPS" && omisu-hw-intel-ptl; then
  echo "Detected Dell XPS Panther Lake, installing PTL kernel..."

  omisu-pkg-add linux-ptl linux-ptl-headers
  ...
  # Named to sort after omisu-defaults.conf: drop-ins are read in order and
