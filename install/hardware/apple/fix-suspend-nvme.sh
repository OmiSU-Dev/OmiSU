...
if [[ $MACBOOK_MODEL =~ MacBook(8,1|9,1|10,1)|MacBookPro13,[123]|MacBookPro14,[123] ]]; then
  ...
  if [[ -f $NVME_DEVICE ]]; then
    ...
    sudo tee /etc/systemd/system/omisu-nvme-suspend-fix.service >/dev/null <<'EOF'
[Unit]
Description=OmiSu NVMe Suspend Fix for MacBook
...
EOF
    ...
    sudo systemctl enable omisu-nvme-suspend-fix.service
  else
