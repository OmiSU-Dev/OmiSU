# The setup form: every question OmiSu asks a human to describe their machine
...
# (omisu-provision-owner). Sourced by both, so the copies cannot drift the way
...
OMISU_KEYBOARD_LAYOUTS=$'English (US)|us
English (UK)|uk
...
OMISU_USERNAME_PATTERN='^[a-z_][a-z0-9_-]*[$]?$'
OMISU_RESERVED_USERNAMES='^(root|bin|daemon|mail|ftp|http|nobody|dbus|systemd-coredump|systemd-network|systemd-oom|systemd-journal-remote|systemd-resolve|systemd-timesync|tss|uuidd|alpm|git|avahi|cups|lp|_talkd|polkitd|rtkit|qemu|brltty|gluster|rpc|libvirt-qemu|pcscd|nvidia-persistenced|sddm)$'
OMISU_HOSTNAME_PATTERN='^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?$'
OMISU_HOSTNAME_DEFAULT='omisu'
...
omisu_prompt_keyboard() {
  local choice status
  choice=$(printf '%s\n' "$OMISU_KEYBOARD_LAYOUTS" | cut -d'|' -f1 |
    gum choose --height 10 --selected "English (US)" --header "Select keyboard layout") && status=0 || status=$?
  ((status == 0)) || return $status

  keyboard_label="$choice"
  keyboard=$(printf '%s\n' "$OMISU_KEYBOARD_LAYOUTS" | awk -F'|' -v c="$choice" '$1==c{print $2; exit}')
}

omisu_prompt_username() {
  local status
  while true; do
    ...
    if [[ "$username" =~ $OMISU_USERNAME_PATTERN ]]; then
      if [[ "$username" =~ $OMISU_RESERVED_USERNAMES ]]; then
        notice "Username is reserved for system" 1
      elif omisu_username_taken "$username"; then
...
omisu_prompt_hostname() {
  local status
  while true; do
    hostname=$(gum input --placeholder "Letters, digits, and dashes (or return for 'omisu')" --prompt.foreground="#845DF9" --prompt "Hostname> ") && status=0 || status=$?
    ((status == 0)) || return $status

    if [[ -z $hostname ]]; then
      hostname="$OMISU_HOSTNAME_DEFAULT"
      return 0
    elif [[ "$hostname" =~ $OMISU_HOSTNAME_PATTERN ]]; then
      return 0
    else
