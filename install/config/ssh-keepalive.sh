...
if [[ ! -f /etc/ssh/ssh_config.d/20-omisu-keepalive.conf ]]; then
  install -d -m 755 /etc/ssh/ssh_config.d
  cat >/etc/ssh/ssh_config.d/20-omisu-keepalive.conf <<'EOF'
# OmiSu: notice dropped connections quickly instead of hanging until TCP
...
Host *
  ...
  ConnectTimeout 10
EOF
  chmod 644 /etc/ssh/ssh_config.d/20-omisu-keepalive.conf
fi
