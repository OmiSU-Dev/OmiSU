...
OmiSu has no answer for "my disk died", "my laptop was stolen", or "I deleted a folder I needed last month":
...
The ask: paste S3-compatible credentials, and OmiSu handles incremental, versioned, off-site backup from then on. Status, pause, and back-up-now live in a shell panel. Zero ongoing hassle.
...
- **Built-in excludes, not user-editable**: the backup's own secrets directory (`~/.local/share/omisu/backup/` — the repository must never contain the credentials that unlock it), restic's cache (`~/.cache/restic`), `~/.cache`, `~/.local/share/Trash`.
- **Shipped default excludes** (`$OMISU_PATH/default/backup/excludes` as `$HOME`-relative patterns, expanded to absolute paths when the setup wizard writes the effective exclude file — restic does not expand `~` in patterns): browser caches, package-manager caches, `node_modules`, thumbnail caches. Regenerable bytes only — when in doubt, include.
- The dots repo (`~/.local/share/omisu/dots.git`) is deliberately *included*: backup is what finally puts the config history off-site.
- Users extend via `~/.config/omisu/backup/excludes` (one pattern per line, restic syntax, expanded the same way). No include-list to curate — that's the hassle we're avoiding.
...
Teardown is `omisu-setup-backup --remove`: stop and wait for any running backup, unmount any browse session, disable the timer, remove the widget, delete local credentials and state — then state clearly that the repository and its snapshots remain untouched in the bucket, and that restic stays installed.

**Secrets layout**: `~/.local/share/omisu/backup/` (0700) holds `env` (`RESTIC_REPOSITORY`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, extras) and `passphrase` (0600, via `RESTIC_PASSWORD_FILE`); secrets pass to restic only via environment and files, never argv. Non-secret settings (destination label, retention, cadence) live separately in `~/.config/omisu/backup/` — `~/.config/omisu` is documented user-intent config that people version and that dots will sync, so live delete-capable credentials must not live there. Mode-restricted plain files are still the deliberate choice over the keyring: OmiSu's default keyring is configured passwordless by `install/user/default-keyring.sh` (so `secret-tool` adds a precedent while buying nothing), and a locked keyring at 3am would mean no backups. Full-disk encryption — the default, though not universal — covers the at-rest story; the threat model section covers the rest honestly.

## Automatic runs: systemd user timer

The repo's first `.timer`. `default/systemd/user/omisu-backup.timer` + `omisu-backup.service`, shipped disabled to `/usr/lib/systemd/user/` via the `omisu-settings` PKGBUILD in the omisu-pkgs repo (with the matching `package_defaults` fixture update in `test/shell.d/config-test.sh`). Enabled by the setup wizard, not `enable-user-units.sh` — backup is opt-in, following the `omisu-install-service-tailscale` enable/disable pattern.
...
- Service: oneshot running `omisu-backup-run`, with `ConditionPathExists=%h/.local/share/omisu/backup/env` (unconfigured = inert). **Pause is not a unit condition**: a condition-gated unit never executes while paused and so could never notice a pause expiring. Instead the runner always starts, reads the pause record (a resume-at timestamp or `manual` sentinel in the state directory), clears it if expired, and exits as a logged skip if still active.
- Further guards before touching the network: discharging with battery below 20% (a new quiet predicate command — `omisu-battery-low` is a notifier, not a check) → skip; a run already active (per-machine lock file; restic's repository locks handle cross-machine, below) → skip. Skips are logged as skips and recorded distinctly from failures. Missing restic exits 127, the `omisu-snapshot` convention for "tool not installed".
...
`omisu-backup-run` maintains `~/.local/state/omisu/backup/status.json`, written atomically (rename) on every phase change and throttled during upload:
...
- Actions call `omisu backup now|pause|resume` detached, with the dropbox panel's optimistic-state trick (`_desired` overrides reality until the state file catches up) so buttons react instantly.
- IPC target `omisu.backup` (`manageIpc: false`, custom verbs `refresh` / `run` / `status` on top of the inherited open/close/toggle), reachable as `omisu-shell omisu.backup toggle`.
...
`GROUP_DESCRIPTIONS[backup]` added in `bin/omisu`; the router derives verbs from filenames, so these are just `bin/omisu-backup-*` executables with `# omisu:summary=` metadata:
...
- Fully opt-in: no packages installed, no units enabled, no scheduled work, no widget until _Setup > Backup_ runs (the unit files themselves ship disabled with `omisu-settings`, like every other default unit).
...
6. Naming: plain `omisu backup` vs a branded name in the dots tradition. Plain reads better in a disaster ("omisu backup restore") — leaning plain.
