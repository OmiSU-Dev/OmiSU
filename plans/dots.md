...
rev 3 names the feature **dots** (`omisu dots ...`) and adds the
multi-machine sync design.

## Problem

OmiSu declares `~/.config` "your files" but does little to preserve them:

- `omisu-refresh-config` litters `*.bak.<timestamp>` files next to originals.
- `omisu-reinstall-configs` clobbers everything back to `/etc/skel` defaults.
...
- **`~/.config/omisu` only**: too narrow; misses hypr, terminals, `.bashrc`.
...
- **Raw git passthrough / lazygit over `$HOME`** (rejected in review):
  ...
  raw invocation themselves; OmiSu will not bless it.
...
```bash
git init --bare ~/.local/share/omisu/dots.git   # mode 0700
```
...
- Repo-local config only: synthetic identity (`OmiSu <omisu@localhost>`),
...
- Never export `GIT_DIR`/`GIT_WORK_TREE` (would leak into `omisu-plugin-*`,
  `omisu-theme-update`, `omisu-update-dev`, user hooks).
...
- **Best-effort everywhere**: a failed snapshot warns and continues. History
  ...
  `omisu-update`/`omisu-migrate` (`set -e` there must not see git errors).

### Audited manifest, not a derived whitelist

Tracking is `git add -f --pathspec-from-file=<manifest>` only. The manifest is
a hand-audited file shipped with OmiSu — explicitly **not** derived from
`$OMISU_PATH/config` (which ships Chromium Preferences, fcitx5,
`opencode/opencode.json` where users put API keys, etc.):

- Include: `.config/hypr/*.{lua,conf}`, `.config/omisu/shell.json`,
  `.config/omisu/extensions/**`, `.config/omisu/hooks/**`,
  terminal configs (alacritty/foot/ghostty/kitty), `.config/btop/btop.conf`,
  `.config/starship.toml`, `.bashrc`, `.XCompose`.
- Exclude (deliberately): `.config/omisu/plugins/**` and
  `.config/omisu/themes/**` (nested git clones managed by
  `omisu-plugin-*` / `omisu-theme-update`), backgrounds (multi-MB
...
- At seed time and before each snapshot: if manifest paths are symlinks or a
  ...
  detected, mark the repo dormant and say so in `omisu dots status`.
...
internal `omisu-dots-snapshot "<label>"`:

1. **`omisu-provision-user`**: seed repo + initial commit, after `git.sh`
   and `.XCompose` are in place. Idempotent; `--force` must not re-init.
2. **`omisu-migrate`**: before/after pair labeled with version. Covers both
   `omisu-update` *and* the login-time migration path, and separates
...
3. **Batch refresh commands** (`omisu-refresh-hyprland` etc.): one pair per
   command, not one commit per `omisu-refresh-config` call (hyprland calls
   it seven times).
4. **`omisu-reinstall-configs`**: a "before" snapshot — honestly labeled.
...
5. **Timer**: an hourly-ish systemd user timer committing only if dirty, so
   ...
   fails for any edit not followed by an OmiSu operation.
...
- **`omisu dots push`** — *squash-publishes* the current manifest state
...
- **`omisu dots pull [url]`** — fetches `sync`, takes a local pre-pull
...
one `omisu dots restore` away.

Remote setup UX:

- `omisu dots push` with no remote runs setup inline: paste a git URL, or
...
- Dormant mode (dotfile-manager users) disables sync too — they have their
  own.
- v2 candidate: `omisu-provision-first-run` asks "Got a dots repo?" so a
...
invoked as `omisu dots <verb>`:

- `omisu dots snapshot [label]` — take a snapshot now
- `omisu dots log` — history with labels
- `omisu dots diff [ref]` — default range: the last before/after pair
- `omisu dots restore <file> [--at <ref>]` — accepts `~/.config/...`,
...
- `omisu dots push` / `omisu dots pull [url]` — sync (above)
- `omisu dots status` — repo state, dormant/active, remote, last
  snapshot/publish
...
- New users: seeded by `omisu-provision-user`.
...
- Docs/tests alignment: manual `31-dotfiles.md` gets the built-in story
  ...
  per the documentation layout; `default/agents/skills/omisu/SKILL.md`
...
1. Manifest location: `default/omisu/dots-manifest` vs alongside the
   helper; and whether users may extend it via
   `~/.config/omisu/dots-manifest.d/`.
2. Whether `.config/omisu/hooks/**` belongs in the manifest at all (small
   scripts, but the likeliest place for a pasted token).
3. Harden `omisu-refresh-config` against `..` path escape (documented today
