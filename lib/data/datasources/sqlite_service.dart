...
import '../../utils/rom_folder_system_resolver.dart';
...
class SqliteService {
  ...
  Future<void> _createUserTables(DatabaseExecutorAdapter db) async {
    const tables = [
      ...
      CREATE TABLE IF NOT EXISTS user_config (
        ...
        hide_tab_sync INTEGER DEFAULT 1,
        hide_tab_achievements INTEGER DEFAULT 0,
        hide_tab_scraper INTEGER DEFAULT 1,
        hide_tab_romm INTEGER DEFAULT 1,
        ...
        auto_scan_on_change INTEGER DEFAULT 1,
        auto_scan_on_resume INTEGER DEFAULT 1,
        auto_scrape_new_games INTEGER DEFAULT 1,
        auto_scrape_wifi_only INTEGER DEFAULT 0,
        gamepad_shoulder_style TEXT DEFAULT 'bumpers',
        ambient_backdrop_mode TEXT DEFAULT 'subtle'
      );
      ''',
      '''
      ...
      await db.transaction((txn) async {
        ...
        await txn.execute('''
          ...
          VALUES ('all', 0, 0, 'All Systems', 'all', '2024-01-01',
                  'Collection of all systems available in OmiSU.')
        ''');
      });
  ...
  static Future<void> saveUserConfig({
    ...
    int? autoScanOnChange,
    int? autoScanOnResume,
    int? autoScrapeNewGames,
    int? autoScrapeWifiOnly,
    String? gamepadShoulderStyle,
    String? ambientBackdropMode,
  }) async {
    final db = await instance.database;
    ...
    if (autoScanOnChange != null) {
      updates['auto_scan_on_change'] = autoScanOnChange;
    }
    if (autoScanOnResume != null) {
      updates['auto_scan_on_resume'] = autoScanOnResume;
    }
    if (autoScrapeNewGames != null) {
      updates['auto_scrape_new_games'] = autoScrapeNewGames;
    }
    if (autoScrapeWifiOnly != null) {
      updates['auto_scrape_wifi_only'] = autoScrapeWifiOnly;
    }
    if (gamepadShoulderStyle != null) {
      updates['gamepad_shoulder_style'] = gamepadShoulderStyle;
    }
    if (ambientBackdropMode != null) {
      updates['ambient_backdrop_mode'] = ambientBackdropMode;
    }
  ...
  static Future<String> getThemeName() async {
    final config = await getUserConfig();
    return config?['theme_name']?.toString() ?? 'abyss';
  }
  ...
  static Future<SystemModel> getSystemByFolderName(String folderName) async {
    ...
    SystemModel? matched;
    try {
      matched = systems.firstWhere((s) {
        final lowerInput = folderName.toLowerCase();
        final normalizedInput = normalizeRomFolderLabel(folderName);

        bool matches(String candidate) {
          final lower = candidate.toLowerCase();
          if (lower == lowerInput) return true;
          if (normalizeRomFolderLabel(candidate) == normalizedInput) {
            return true;
          }
        ...
        if (matches(s.folderName)) return true;
        if (s.folders.any(matches)) return true;

        return false;
      });
    } on StateError {
      final resolvedId = RomFolderSystemResolver.resolveSystemIdFromLabel(
        folderName,
        availableSystems: systems,
      );
      if (resolvedId != null) {
        matched = systems.where((s) => s.id == resolvedId).firstOrNull;
      }
    }
