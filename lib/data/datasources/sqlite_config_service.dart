  ...
  static Future<ConfigModel> loadConfig() async {
    try {
      ...
      return ConfigModel(
        ...
        autoScanOnChange:
            (int.tryParse(
                  userConfig?['auto_scan_on_change']?.toString() ?? '1',
                ) ??
                1) ==
            1,
        autoScanOnResume:
            (int.tryParse(
                  userConfig?['auto_scan_on_resume']?.toString() ?? '1',
                ) ??
                1) ==
            1,
        autoScrapeNewGames:
            (int.tryParse(
                  userConfig?['auto_scrape_new_games']?.toString() ?? '1',
                ) ??
                1) ==
            1,
        autoScrapeWifiOnly:
            (int.tryParse(
                  userConfig?['auto_scrape_wifi_only']?.toString() ?? '0',
                ) ??
                0) ==
            1,
        ignoreHiddenFiles:
        ...
        ambientBackdropMode: AmbientBackdropMode.fromStored(
          userConfig?['ambient_backdrop_mode']?.toString(),
        ),
        gamepadShoulderStyle: GamepadShoulderStyle.fromStored(
          userConfig?['gamepad_shoulder_style']?.toString(),
        ),
        systemSortBy:
        ...
        gameDetailsTab:
            ...
            : 'wheel',
        // Missing column/row => OmiSU defaults (tabs hidden unless user opts in).
        hideTabSync:
            (int.tryParse(userConfig?['hide_tab_sync']?.toString() ?? '1') ??
                1) ==
            1,
        hideTabAchievements:
            (int.tryParse(
            ...
            1,
        hideTabScraper:
            (int.tryParse(userConfig?['hide_tab_scraper']?.toString() ?? '1') ??
                1) ==
            1,
        hideTabRomm:
            (int.tryParse(userConfig?['hide_tab_romm']?.toString() ?? '1') ??
                1) ==
            1,
        hideTabSearch:
  ...
  static Future<void> saveConfig(ConfigModel config) async {
    try {
      await SqliteService.saveUserConfig(
        ...
        autoScanOnChange: config.autoScanOnChange ? 1 : 0,
        autoScanOnResume: config.autoScanOnResume ? 1 : 0,
        autoScrapeNewGames: config.autoScrapeNewGames ? 1 : 0,
        autoScrapeWifiOnly: config.autoScrapeWifiOnly ? 1 : 0,
        ...
        gamepadShoulderStyle: config.gamepadShoulderStyle.storageValue,
        ambientBackdropMode: config.ambientBackdropMode.storageValue,
    ...
    for (final romFolder in romFolders) {
      ...
      for (final entity in entities) {
        if (entity is Directory) {
          ...
          final systemId = await RomFolderSystemResolver.resolveSystemId(
          ...
          final matchingSystem = availableSystems
          ...
          final detectedId = matchingSystem.id.toString();
          final existing = detectedSystemsMap[detectedId];
          if (existing != null) {
            detectedSystemsMap[detectedId] = existing.copyWith(
              romCount: (existing.romCount) + romCount,
            );
          } else {
            detectedSystemsMap[detectedId] = matchingSystem.copyWith(
              detected: true,
              romCount: romCount,
            );
          }
