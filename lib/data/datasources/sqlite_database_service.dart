...
class SqliteDatabaseService {
  ...
  static Future<ScanSummary> _performSystemScan(
    ...
    for (final romFolder in romFolders) {
      ...
      // Custom / No-Intro folder names resolved outside the alias table.
      if (subdirSystemMap != null && subdirsForRoot != null) {
        for (final entry in subdirsForRoot.entries) {
          if (subdirSystemMap[entry.value] != system.id) continue;
          try {
            scanTargets.add((
              dirPath: entry.value,
            ...
            _log.e('Error resolving custom folder ${entry.key}: $e');
          }
        }
      }
    }
    ...
    if (scanTargets.isEmpty && initialCount > 0) {
      _log.w(
        'Scan[${system.realName}]: no folder targets resolved while '
        '$initialCount ROM(s) exist — skipping prune to avoid data loss',
      );
      return ScanSummary(
        added: 0,
        removed: 0,
    ...
    final primaryFolderName = system.primaryFolderName;
