...
import 'package:omisu/utils/cloud_path_builder.dart';
...
class SqliteMigrations {
  ...
  /// Migration v158: Adds library auto-scan and auto-scrape preference columns
  /// to `user_config`.
  ///
  /// Defaults match [ConfigModel] (auto-scan/scrape on, Wi‑Fi-only off) so a
  /// config written before these columns existed keeps the feature's out-of-the-box
  /// behaviour.
  ///
  /// Idempotent — each column is added only when absent.
  static Future<void> _migrateToVersion158(Database db) async {
    _log.i('Migration v158: Adding auto-scan/scrape columns to user_config');
    try {
      final tableInfo = db.select('PRAGMA table_info(user_config)');
      final columns = tableInfo.map((c) => c['name'].toString()).toList();
      if (!columns.contains('auto_scan_on_change')) {
        db.execute(
          'ALTER TABLE user_config ADD COLUMN auto_scan_on_change '
          'INTEGER DEFAULT 1',
        );
        _log.i('Column auto_scan_on_change added via v158');
      }
      if (!columns.contains('auto_scan_on_resume')) {
        db.execute(
          'ALTER TABLE user_config ADD COLUMN auto_scan_on_resume '
          'INTEGER DEFAULT 1',
        );
        _log.i('Column auto_scan_on_resume added via v158');
      }
      if (!columns.contains('auto_scrape_new_games')) {
        db.execute(
          'ALTER TABLE user_config ADD COLUMN auto_scrape_new_games '
          'INTEGER DEFAULT 1',
        );
        _log.i('Column auto_scrape_new_games added via v158');
      }
      if (!columns.contains('auto_scrape_wifi_only')) {
        db.execute(
          'ALTER TABLE user_config ADD COLUMN auto_scrape_wifi_only '
          'INTEGER DEFAULT 0',
        );
        _log.i('Column auto_scrape_wifi_only added via v158');
      }
  ...
  /// Migration v159: OmiSU shell defaults — grid home and power-user tabs hidden.
  ///
  /// Silently moves anyone still on the legacy NeoStation carousel home to grid,
  /// and hides Sync, Scraper, and RomM from the top nav (still reachable under
  /// Settings). Users can re-enable tabs from Appearance → Navigation.
  static Future<void> _migrateToVersion159(Database db) async {
    _log.i('Migration v159: Applying OmiSU shell defaults');
    try {
      db.execute(
        "UPDATE user_config SET system_view_mode = 'grid' "
        "WHERE lower(system_view_mode) = 'carousel'",
      );
      db.execute(
        'UPDATE user_config SET hide_tab_sync = 1 '
        'WHERE hide_tab_sync IS NULL OR hide_tab_sync = 0',
      );
      db.execute(
        'UPDATE user_config SET hide_tab_scraper = 1 '
        'WHERE hide_tab_scraper IS NULL OR hide_tab_scraper = 0',
      );
      db.execute(
        'UPDATE user_config SET hide_tab_romm = 1 '
        'WHERE hide_tab_romm IS NULL OR hide_tab_romm = 0',
      );
  ...
  /// Migration v160: Shoulder-button glyph preference for tab cycling hints.
  static Future<void> _migrateToVersion160(Database db) async {
    _log.i('Migration v160: Adding gamepad_shoulder_style to user_config');
    try {
      final tableInfo = db.select('PRAGMA table_info(user_config)');
      final hasColumn = tableInfo.any(
        (row) => row['name']?.toString() == 'gamepad_shoulder_style',
      );
      if (!hasColumn) {
        db.execute(
          "ALTER TABLE user_config ADD COLUMN gamepad_shoulder_style "
          "TEXT DEFAULT 'bumpers'",
        );
      }
  ...
  static Future<void> _migrateToVersion161(Database db) async {
    ...
    try {
      final tableInfo = db.select('PRAGMA table_info(user_config)');
      final hasColumn = tableInfo.any(
        (row) => row['name']?.toString() == 'ambient_backdrop_mode',
      );
      if (!hasColumn) {
        db.execute(
          "ALTER TABLE user_config ADD COLUMN ambient_backdrop_mode "
          "TEXT DEFAULT 'subtle'",
        );
      }
