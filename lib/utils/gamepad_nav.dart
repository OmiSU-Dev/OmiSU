...
class GamepadNavigation {
  ...
  void _handleGamepadEvent(GamepadEvent event) async {
    ...
    try {
      ...
      // launched emulator in front of OmiSU would otherwise drive the UI
  ...
  void _handleTranslatedEvent(TranslatedGamepadEvent event) {
    ...
    if (event.inputType == GamepadInputType.dpadUp ||
        ...
        event.inputType == GamepadInputType.rightStickX ||
        event.inputType == GamepadInputType.rightStickY ||
    ...
    if (event.isReleased) {
      ...
      } else if (event.inputType == GamepadInputType.rightStickX) {
        _stopRepeatTimer(GamepadInputType.dpadLeft);
        _stopRepeatTimer(GamepadInputType.dpadRight);
      } else if (event.inputType == GamepadInputType.rightStickY) {
        _stopRepeatTimer(GamepadInputType.dpadUp);
        _stopRepeatTimer(GamepadInputType.dpadDown);
      }
    ...
    switch (event.inputType) {
      ...
      case GamepadInputType.leftStickX:
        _handleStickAxis(
          value: event.value,
          negativeVirtual: GamepadInputType.dpadLeft,
          positiveVirtual: GamepadInputType.dpadRight,
          onNegative: onNavigateLeft,
          onPositive: onNavigateRight,
        );
        break;
      ...
      case GamepadInputType.leftStickY:
        _handleStickAxis(
          value: event.value,
          negativeVirtual: GamepadInputType.dpadDown,
          positiveVirtual: GamepadInputType.dpadUp,
          onNegative: onNavigateDown,
          onPositive: onNavigateUp,
        );
        break;
  ...
  /// Maps a stick axis to virtual D-pad navigation (left or right stick).
  void _handleStickAxis({
    required double value,
    required GamepadInputType negativeVirtual,
    required GamepadInputType positiveVirtual,
    required Function? onNegative,
    required Function? onPositive,
  }) {
    if (isWindows) {
      if (value.abs() < 0.5) {
        _stopRepeatTimer(negativeVirtual);
        _stopRepeatTimer(positiveVirtual);
        return;
      }
      if (value > 0.65) {
        _handleDirectionalAction(positiveVirtual, onPositive);
      } else if (value < -0.65) {
        _handleDirectionalAction(negativeVirtual, onNegative);
      } else {
        _stopRepeatTimer(negativeVirtual);
        _stopRepeatTimer(positiveVirtual);
      }
