...
/// Width of the tab pill only — slots plus its horizontal padding.
double navTabPillWidth({
  required int tabCount,
  double slot = 32,
  double pillPadding = 4,
}) => (pillPadding * 2) + (slot * tabCount);
...
///
/// When [includeShoulders] is false, returns only [navTabPillWidth] — used when
/// bumpers sit on a separate header row above the pill.
double navStripWidth({
  required int tabCount,
  double slot = 32,
  double shoulder = 36,
  double pillPadding = 4,
  bool includeShoulders = true,
}) =>
    navTabPillWidth(
      tabCount: tabCount,
      slot: slot,
