...
import 'package:omisu/providers/sqlite_database_provider.dart';
import 'package:omisu/providers/file_provider.dart';
import 'package:omisu/providers/theme_provider.dart';
import 'package:omisu/providers/scraping_provider.dart';
import 'package:omisu/providers/retro_achievements_provider.dart';
import 'package:omisu/providers/romm_provider.dart';
import 'package:omisu/providers/neo_sync_provider.dart';
import 'package:omisu/screens/main_screen.dart';
import 'package:omisu/services/neosync/auth_service.dart';
import 'package:omisu/services/neosync/neo_sync_service.dart';
import 'package:omisu/services/neosync/billing_service.dart';
import 'package:omisu/sync/sync_manager.dart';
import 'package:omisu/sync/providers/neo_sync_adapter.dart';
import 'package:omisu/sync/providers/romm_provider.dart';
import 'package:omisu/services/notification_service.dart';
import 'package:omisu/services/game_service.dart';
import 'package:omisu/services/game/favorites_service.dart';
import 'package:omisu/services/secondary_apps_service.dart';
import 'package:omisu/repositories/config_repository.dart';
import 'package:omisu/repositories/scraper_repository.dart';
import 'package:omisu/services/steam_scraper_service.dart';
import 'package:omisu/providers/collections_provider.dart';
import 'package:omisu/providers/system_background_provider.dart';
import 'package:omisu/providers/omisu_shell_provider.dart';
import 'package:omisu/providers/neo_assets_provider.dart';
import 'package:omisu/widgets/app_lifecycle_handler.dart';
import 'package:omisu/widgets/back_swipe_zone.dart';
import 'package:omisu/services/startup_theme_cache.dart';
...
import 'package:omisu/utils/custom_scroll_behavior.dart';
import 'package:omisu/utils/desktop_window_focus.dart';
import 'package:omisu/utils/display_metrics_log.dart';
import 'package:flutter_localization/flutter_localization.dart';
import 'package:omisu/l10n/app_locale.dart';
import 'package:omisu/services/config_service.dart';
import 'package:omisu/services/boot_status.dart';
import 'package:omisu/services/library_watch_coordinator.dart';
...
import 'package:omisu/utils/image_cache_budget.dart';
...
import 'package:omisu/screens/secondary_screen/secondary_screen.dart';
...
void main() async {
  ...
  _reportBoot(AppLocale.bootPromptBoot, AppLocale.bootDetailBoot);

  final log = LoggerService.instance;
  await log.init();
  log.i('Starting OmiSU...');

  _reportBoot(AppLocale.bootPromptMemory, AppLocale.bootDetailMemory);
  ...
  if (Platform.isAndroid) {
    _reportBoot(AppLocale.bootPromptStorage, AppLocale.bootDetailStorage);
    await _awaitUserDataStorage();
    await StreamSettingsService.load();
    await StreamingService.init();
  }

  // Inicializar window_manager para desktop con tamano minimo 640x480
  if (Platform.isWindows || Platform.isMacOS || Platform.isLinux) {
    _reportBoot(AppLocale.bootPromptShell, AppLocale.bootDetailShell);
  ...
  _reportBoot(AppLocale.bootPromptLibrary, AppLocale.bootDetailLibrary);
  ...
  if (NordiConfig.curatedBuild) {
    await NordiSafeModeService.load();
    await NordiBootstrapService.applyIfNeeded(
      configProvider: sqliteConfigProvider,
    );
    await NordiBootstrapService.applyTouchControlsOffByDefault();
  ...
  _reportBoot(AppLocale.bootPromptTheme, AppLocale.bootDetailTheme);
  final themeProvider = await ThemeProvider.create();

  _reportBoot(AppLocale.bootPromptServices, AppLocale.bootDetailServices);
  ...
  LibraryWatchCoordinator.instance.attach(sqliteConfigProvider);
  sqliteConfigProvider.addListener(
    LibraryWatchCoordinator.instance.refreshWatchers,
  );

  FavoritesService.onRomPlayedRecorded =
      sqliteDatabaseProvider.syncRomPlayedInCache;

  _reportBoot(AppLocale.bootPromptReady, AppLocale.bootDetailReady);
...
void _reportBoot(String promptKey, String detailKey) {
  BootStatus.instance.set(
    prompt: _startupString(promptKey),
    detail: _startupString(detailKey),
  );
}
              ...
              : Center(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      OmisuAsciiLogo(
                        width: 112 * scale,
                        style: OmisuLogoStyle.screensaver,
  ...
  Widget build(BuildContext context) {
    return _StartupScaffold(
      ...
      childrenBuilder: (context, colors) {
        ...
        return [
          ListenableBuilder(
            listenable: BootStatus.instance.detailLine,
            builder: (context, _) {
              final detail = BootStatus.instance.detailLine.value;
              final text = detail.isNotEmpty
                  ? detail
                  : _startupString(AppLocale.bootDetailBoot);
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(create: (context) => OmisuShellProvider()),
        ChangeNotifierProvider(
                  ...
                  child: Actions(
                    ...
                    child: MaterialApp(
                      ...
                      theme: themeProvider.currentTheme.copyWith(
                        // Keep Retro 82 / JetBrains Mono Nerd Font from the theme.
                        // Do not wrap with GoogleFonts — that replaced the brand face.
