...
import 'package:omisu/widgets/confirm_action_dialog.dart';
import 'package:omisu/providers/file_provider.dart';
import 'package:omisu/providers/sqlite_config_provider.dart';
import 'package:omisu/services/esde_import_service.dart';
...
import 'package:omisu/widgets/tv_directory_picker.dart';
...
class DirectoriesSettingsContentState
  ...
  static const int _scanSectionCount = LibraryScanSettingsSection.itemCount;

  int get _directoryNavOffset => _scanSectionCount;
  ...
  Future<void> _runEsdeImport() async {
    ...
    if (error != null) {
      ...
      if (!result.gamelistsDirFound) {
        ...
        // Valid ES-DE folder, but nothing here mapped to a OmiSU system.
        GlobalNotificationService().update(
          id: notificationId,
  ...
  Future<void> _selectUserDataLocation() async {
    try {
      ...
      // Relocating actually MOVES data (copy + delete of OmiSU's own
  ...
  int getItemCount() => _scanSectionCount + _directoryItems.length;

  void _handleScanItem(int localIndex) {
    final provider = context.read<SqliteConfigProvider>();
    switch (localIndex) {
      case 0:
        provider.updateScanOnStartup(!provider.config.scanOnStartup);
      case 1:
        provider.updateIgnoreHiddenFiles(!provider.config.ignoreHiddenFiles);
      case 2:
        provider.updateAutoScanOnChange(!provider.config.autoScanOnChange);
      case 3:
        provider.updateAutoScanOnResume(!provider.config.autoScanOnResume);
    }
  }

  void selectItem(int index) {
    if (index < _scanSectionCount) {
      _handleScanItem(index);
      return;
    }
    final directoryIndex = index - _directoryNavOffset;
    if (directoryIndex < _directoryItems.length) {
      _handleItemTap(_directoryItems[directoryIndex]);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return Column(
        ...
        children: [
          SettingsTitle(
            title: widget.useLibraryTitle
                ? AppLocale.library.getString(context)
                : AppLocale.configureDirectories.getString(context),
            subtitle: widget.useLibraryTitle
                ? AppLocale.librarySubtitle.getString(context)
            ...
            child: Builder(
              builder: (context) {
                ...
                for (var i = 0; i < _directoryItems.length; i++) {
                  // "ROM Directories" header before add_rom (directory index 2).
                  if (i == 2) {
                    visualRows.add({
                  ...
                  visualRows.add({'nav': i + _directoryNavOffset});
                }
                _ensureKeys(_scanSectionCount + _directoryItems.length);
                  ...
                  child: Column(
                    ...
                    children: [
                      LibraryScanSettingsSection(
                        ...
                        itemKeys: _itemKeys,
                        onItemSelected: _handleScanItem,
                      ),
                      ...
                      ...visualRows.map((row) {
                        ...
                        return _buildDirectoryNavRow(
                          theme,
                          navIndex: row['nav'] as int,
                        );
                      }),
                    ],
                  ),
  ...
  Widget _buildDirectoryNavRow(ThemeData theme, {required int navIndex}) {
    final directoryIndex = navIndex - _directoryNavOffset;
    final item = _directoryItems[directoryIndex];
    final isSelected =
        widget.isContentFocused && widget.selectedContentIndex == navIndex;

    final isRemoveItem = item['action'] == 'remove_rom';
    final isUserData = item['action'] == 'user_data';
    final isEsdeDisabled = _isEsdeDisabled(item['action'] as String);
    final borderColor = isSelected
        ? (isRemoveItem ? theme.colorScheme.error : theme.colorScheme.primary)
              ...
              children: [
                Row(
                  children: [
                        ...
                        children: [
                          Text(
                            isRemoveItem
                                ...
                                : (item['title'] as String).getString(context),
                            style: theme.textTheme.titleSmall?.copyWith(
                              fontWeight: FontWeight.bold,
                              ...
                              color: isSelected
                                  ? (isRemoveItem
                                        ...
                                        : theme.colorScheme.primary)
                                  : theme.colorScheme.onSurface,
                              fontFamily: isRemoveItem ? 'monospace' : null,
                            ),
                            maxLines: 2,
                            overflow: TextOverflow.ellipsis,
                          ),
                          SizedBox(height: 2.r),
                          Text(
                            (item['subtitle'] as String).getString(context),
                            style: theme.textTheme.bodySmall?.copyWith(
                              color: isSelected && isRemoveItem
                    ...
                    else if (item['action'] == 'esde_select_folder')
                      SettingsActionButton(
                        icon: Symbols.folder_special_rounded,
                        selected: isSelected,
                      )
                    else if (item['action'] == 'esde_run_import')
                      SettingsActionButton(
