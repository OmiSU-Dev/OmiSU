...
class SystemsSettingsContentState extends State<SystemsSettingsContent> {
  ...
  int getItemCount(SqliteConfigProvider provider) {
    if (widget.visibilityOnly) {
      final totalFavorites = context
          .read<SqliteDatabaseProvider>()
          .totalFavorites;
      final hasFavorites = totalFavorites > 0;
      final systemCount = provider.detectedSystems
          .where((s) => s.folderName != SystemFolderNames.favorites)
          .length;
      return (hasFavorites ? 1 : 0) + systemCount;
    }
    // home layout + hideRecent + recent card size + favorites + detectedSystems
    return 4 +
  ...
  List<_SystemSettingRow> _buildItems(SqliteConfigProvider provider) {
    ...
    return <_SystemSettingRow>[
      _SystemSettingRow(
        ...
        valueText: AppLocale.gridView.getString(context),
        onToggle: () {
    ...
    return Consumer<SqliteConfigProvider>(
      builder: (context, provider, _) {
        ...
        return Column(
          ...
          children: [
            SettingsTitle(
              title: widget.visibilityOnly
                  ? AppLocale.appearanceHiddenSystems.getString(context)
                  : AppLocale.systemsSettings.getString(context),
