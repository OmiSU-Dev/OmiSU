...
import 'package:omisu/services/ra_library_match_runner.dart';
import 'package:omisu/services/retroachievements_hash_service.dart';
import 'package:omisu/l10n/app_locale.dart';
import 'package:flutter_localization/flutter_localization.dart';
import '../widgets/fixed_header.dart';
import '../widgets/omisu/omisu_shell_footer.dart';
import '../widgets/omisu/omisu_shell_background.dart';
...
import 'nordi_sleep_screen.dart';
...
import 'package:omisu/repositories/emulator_repository.dart';
import 'dart:async';
import 'dart:io';
...
class AppScreenState extends State<AppScreen> with WidgetsBindingObserver {
  ...
  /// Built-in player OTA failed or was partial — retry on next resume when online.

  /// Avoid showing the OmiSU APK dialog twice in one cold start.
  @override
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<OmisuShellProvider>().setActiveTabIndex(_selectedTabIndex);
      ...
      // Soft launcher bed (iiSU-style presence) — original ambient loop.
      final sfxOn = context.read<SqliteConfigProvider>().config.sfxEnabled;
      MenuAmbientService.instance.setEnabled(sfxOn);
      if (sfxOn) {
        unawaited(MenuAmbientService.instance.start());
      }
    });
  ...
  Future<void> _performUpdateSequence(
    ...
    if (configProvider.config.autoUpdateSystems ||
      ...
      final ota = await UnifiedUpdateCoordinator.runStartupPass(
        context: context,
        configProvider: configProvider,
      );
      if (ota.appUpdateAccepted) {
        return;
      }
      if (ota.systemsConfigUpdated && mounted) {
        await initialScan;
    ...
    await initialScan;
    await systemsRescan;
    if (NordiConfig.curatedBuild && mounted) {
      await NordiBootstrapService.ensureDefaultSystemArt(
        configProvider: configProvider,
        neoAssets: context.read<NeoAssetsProvider>(),
      );
    }
  ...
  Future<void> _maybeRetryBuiltinPlayerUpdates() async {
    if (!mounted) return;
    final configProvider = Provider.of<SqliteConfigProvider>(
      context,
      listen: false,
    );
    await UnifiedUpdateCoordinator.retryBuiltinPlayerIfNeeded(
      context: context,
      configProvider: configProvider,
    );
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // When OmiSU's own UI regains focus and no game is actually running,
  ...
  void _onTabSelected(int index) {
    ...
    shell.setActiveTabIndex(index);
    if (index == AppTabs.systems) {
      shell.clearFocus();
    } else {
      shell.clearFooterActions();
      final tab = NavTab.values[index];
      shell.setTabContext(
        title: navTabSpec(tab).labelKey.getString(context),
      );
    }
          ...
          child: Scaffold(
            body: Stack(
              children: [
                ...
                // Top status bar (brand, shoulders, clock) — tabs are in the nav dock.
                Positioned(
                  top: 0,
                  left: 0,
                  right: 0,
