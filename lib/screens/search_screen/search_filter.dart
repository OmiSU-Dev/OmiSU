import 'package:omisu/models/database_game_model.dart';
import 'package:omisu/models/romm_rom.dart';
import 'package:omisu/utils/fuzzy_match.dart';
import 'package:omisu/utils/ra_coverage.dart';
...
/// Display name used for text search (scraped title, else ROM filename).
String searchGameDisplayName(DatabaseGameModel g) => g.realName ?? g.filename;
...
bool matchesCriteria(DatabaseGameModel g, SearchCriteria criteria) {
  ...
  if (query.isNotEmpty &&
      !fuzzyMatchesQuery(query, searchGameDisplayName(g))) {
    return false;
  }
...
bool matchesRemoteCriteria(RemoteGameFields g, SearchCriteria criteria) {
  final query = criteria.query.trim();
  if (query.isNotEmpty && !fuzzyMatchesQuery(query, g.name)) return false;
...
/// Applies [criteria] to [games] and returns the matches sorted by relevance
/// when there is a name query, otherwise alphabetically by display name.
List<DatabaseGameModel> filterAndSortGames(
  Iterable<DatabaseGameModel> games,
  SearchCriteria criteria,
) {
  final query = criteria.query.trim();
  final matches = games.where((g) => matchesCriteria(g, criteria)).toList();

  if (query.isEmpty) {
    matches.sort((a, b) {
      final an = searchGameDisplayName(a).toLowerCase();
      final bn = searchGameDisplayName(b).toLowerCase();
  ...
  matches.sort((a, b) {
    final sa = fuzzyMatchScore(query, searchGameDisplayName(a)) ?? 0;
    final sb = fuzzyMatchScore(query, searchGameDisplayName(b)) ?? 0;
    final byScore = sb.compareTo(sa);
    if (byScore != 0) return byScore;
    return searchGameDisplayName(
      a,
    ).toLowerCase().compareTo(searchGameDisplayName(b).toLowerCase());
  });
  return matches;
}
