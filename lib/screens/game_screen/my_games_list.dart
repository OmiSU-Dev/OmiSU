...
class _SystemGamesListState extends State<SystemGamesList> {
  ...
  /// it back on wake.
  ///
  /// OmiSU runs as a HOME launcher, so the activity is never paused on
  ...
  Future<void> _goBack() async {
    ...
    // OmiSU logo over a bare background after backing out of a collection.
    ...
    return AnimatedSwitcher(
      ...
      child: Builder(
        ...
        builder: (context) {
          ...
          // Per-game art is not on disk yet (unscraped library). Fall back to
          // the active theme's system background so the panel is not a void.
          final themeBg = context
              .read<NeoAssetsProvider>()
              .getBackgroundForSystemSync(imageSystemFolder);
          if (themeBg != null && File(themeBg).existsSync()) {
            return Image.file(
              File(themeBg),
              fit: BoxFit.cover,
              width: double.infinity,
