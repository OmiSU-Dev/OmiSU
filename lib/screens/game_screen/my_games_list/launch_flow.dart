...
extension _LaunchFlow on _SystemGamesListState {
  ...
  String _activeGamesListLayerId() {
    final mode = context.read<SqliteConfigProvider>().config.gameViewMode;
    return switch (mode) {
      'grid' => 'games_grid',
      'carousel' => 'games_carousel',
      _ => 'system_games_list',
    };
  }

  void _restoreGamesListGamepadFocus() {
    final layerId = _activeGamesListLayerId();
    GamepadNavigationManager.clearLaunchFocusOwner();
    if (layerId == 'system_games_list') {
      GamepadNavigationManager.restoreLayerToTop(
        layerId,
        onActivate: () => _gamepadNav.activate(),
        onDeactivate: () => _gamepadNav.deactivate(),
      );
    } else {
      GamepadNavigationManager.restoreLayerToTop(layerId);
    }
  }
  ...
  void _reactivateGamepadNavigation() async {
    ...
    // Remount grid/carousel (they unregister while [_isGameLaunching] hides the
    // list), then restore the launcher's gamepad layer — never the systems tab.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        _restoreGamesListGamepadFocus();
      });
    });
  ...
  Future<void> _selectCurrentGame() async {
    ...
    GamepadNavigationManager.rememberFocusOwner(_activeGamesListLayerId());
    ...
    try {
      ...
      await launchGameWithDialog(
        ...
        onHomeExit: _reactivateGamepadNavigation,
