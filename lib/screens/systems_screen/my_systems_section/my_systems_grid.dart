...
import 'package:omisu/constants/system_folder_names.dart';
...
import 'package:omisu/widgets/system_emulator_settings_dialog.dart';
...
import 'package:omisu/widgets/context_menu/anchored_context_menu.dart';
...
class MySystems extends StatelessWidget {
    ...
    return PopScope(
      ...
      child: Consumer2<SqliteConfigProvider, SqliteDatabaseProvider>(
        builder: (context, configProvider, dbProvider, child) {
          ...
          // PHASE 3: Content Presentation — OmiSU is grid-first (iiSU style).
          final systemsWidget = _buildSystemsGrid(context, configProvider);
  ...
  Widget _buildSystemsGrid(
    ...
    return Column(
      children: [
        Expanded(
          child: Consumer<OmisuShellProvider>(
            builder: (context, shell, _) {
              final hasTitle =
                  shell.homeChromeActive &&
                  shell.focusTitle != null &&
                  shell.focusTitle!.isNotEmpty;
              shell.setFooterActions(
                OmisuFooterActions(
                  onSecondary: () => _openSystemContextMenu(
                    context,
                    currentSystem,
                    configProvider,
                  ),
                  ...
                  secondaryLabel: AppLocale.hintOptions.getString(context),
                  primaryLabel: currentSystem.isGame
              ...
              return Padding(
                padding: EdgeInsets.only(
                  left: 6.0.r,
                  right: 6.0.r,
                  top: omisuShellContentTopInset(hasTitlePill: hasTitle).r,
                  bottom: omisuShellContentBottomInset().r,
                ),
  ...
  void _navigateToSystem(
    ...
    if (systemInfo.isGame && systemInfo.gameModel != null) {
      ...
      try {
        ...
        final usesEmbeddedPlay = Platform.isAndroid &&
            EmbeddedCoreRegistry.supports(gameSystemModel.folderName);

        // Built-in play runs fullscreen on the root navigator; hiding the grid
        // here left a black home screen behind the launch overlay and after exit.
        if (!usesEmbeddedPlay) {
        ...
        await launchGameWithDialog(
          ...
          onHomeExit: () {
            ...
            if (rootContext != null) {
              rootContext.read<SqliteDatabaseProvider>().refresh();
            }
          },
...
class _SystemCardGridViewState extends State<SystemCardGridView> {
  ...
  void dispose() {
    ...
    _cleanupGamepad();
    context.read<OmisuShellProvider>().clearFooterActions();
