...
extension _ThemeBackground on _SystemCardGridViewState {
  ...
  /// Synchronizes shell chrome + secondary display from the current selection.
  void _updateSecondaryScreenName() {
    ...
    if (!Platform.isAndroid ||
        !widget.enableSecondaryDisplay ||
        _secondaryDisplayState == null) {
      _updateShellFocus(info, systemBackground);
      return;
    }
    ...
    if (info.isGame && info.gameModel != null) {
      ...
      _updateShellFocus(info, hasFanart ? fanartPath : null);
      return;
    }
  ...
  void _updateShellFocus(SystemInfo info, String? backgroundPath) {
    if (!mounted) return;

    final shell = context.read<OmisuShellProvider>();
    final title = info.isGame
        ? "${AppLocale.lastPlayed.getString(context)}: ${info.title}"
        : info.title ?? '';

    String? subtitle;
    if (!info.isGame) {
      final folder = info.folderName ?? '';
      final unit = folder == 'android'
          ? AppLocale.apps.getString(context)
          : folder == 'music'
          ? AppLocale.tracks.getString(context)
          : AppLocale.games.getString(context);
      subtitle = '${info.numOfRoms} $unit';
    }

    shell.updateHomeFocus(
      title: title,
      subtitle: subtitle,
      backgroundPath: backgroundPath,
      asciiLabel: _asciiLabelFor(info),
    );
  }

  String _asciiLabelFor(SystemInfo info) {
    final raw = info.shortName ?? info.title ?? '';
    return OmarchyAsciiRenderer.sanitize(raw);
  }
}
