...
class ConfigModel {
  ...
  /// Whether filesystem changes under configured ROM folders trigger a silent
  /// rescan of the affected system (desktop platforms only).
  final bool autoScanOnChange;

  /// Whether returning to the app triggers a silent rescan of all detected
  /// systems.
  final bool autoScanOnResume;

  /// Whether newly discovered ROMs are auto-scraped via ScreenScraper in the
  /// background after a scan adds games.
  final bool autoScrapeNewGames;

  /// On Android, whether auto-scrape runs only when on Wi‑Fi.
  final bool autoScrapeWifiOnly;
  ...
  /// Shoulder-button glyphs for tab cycling hints (LB/RB vs LT/RT).
  final GamepadShoulderStyle gamepadShoulderStyle;

  /// Soft shell pixel-dust / accent wash: off, subtle, or reactive.
  final AmbientBackdropMode ambientBackdropMode;
  ...
  const ConfigModel({
    ...
    this.autoScanOnChange = true,
    this.autoScanOnResume = true,
    this.autoScrapeNewGames = true,
    this.autoScrapeWifiOnly = false,
    ...
    this.hideTabSync = true,
    this.hideTabAchievements = false,
    this.hideTabScraper = true,
    this.hideTabRomm = true,
  ...
  factory ConfigModel.fromJson(Map<String, dynamic> json) {
    ...
    return ConfigModel(
      ...
      autoScanOnChange:
          (json['autoScanOnChange'] ?? json['auto_scan_on_change'] ?? 1)
                  .toString() ==
              '1' ||
          (json['autoScanOnChange'] ?? true).toString().toLowerCase() == 'true',
      autoScanOnResume:
          (json['autoScanOnResume'] ?? json['auto_scan_on_resume'] ?? 1)
                  .toString() ==
              '1' ||
          (json['autoScanOnResume'] ?? true).toString().toLowerCase() == 'true',
      autoScrapeNewGames:
          (json['autoScrapeNewGames'] ?? json['auto_scrape_new_games'] ?? 1)
                  .toString() ==
              '1' ||
          (json['autoScrapeNewGames'] ?? true).toString().toLowerCase() ==
              'true',
      autoScrapeWifiOnly:
          (json['autoScrapeWifiOnly'] ?? json['auto_scrape_wifi_only'] ?? 0)
                  .toString() ==
              '1' ||
          (json['autoScrapeWifiOnly'] ?? false).toString().toLowerCase() ==
              'true',
      ignoreHiddenFiles:
      ...
      ambientBackdropMode: AmbientBackdropMode.fromStored(
        (json['ambientBackdropMode'] ?? json['ambient_backdrop_mode'])
            ?.toString(),
      ),
      gamepadShoulderStyle: GamepadShoulderStyle.fromStored(
        (json['gamepadShoulderStyle'] ?? json['gamepad_shoulder_style'])
            ?.toString(),
      ),
      systemSortBy:
  ...
  Map<String, dynamic> toJson() {
    ...
    return {
      ...
      'autoScanOnChange': autoScanOnChange,
      'autoScanOnResume': autoScanOnResume,
      'autoScrapeNewGames': autoScrapeNewGames,
      'autoScrapeWifiOnly': autoScrapeWifiOnly,
      ...
      'gamepadShoulderStyle': gamepadShoulderStyle.storageValue,
      'ambientBackdropMode': ambientBackdropMode.storageValue,
  ...
  ConfigModel copyWith({
    ...
    bool? scanOnStartup,
    bool? autoScanOnChange,
    bool? autoScanOnResume,
    bool? autoScrapeNewGames,
    bool? autoScrapeWifiOnly,
    ...
    double? sfxVolume,
    bool? use12HourClock,
    GamepadShoulderStyle? gamepadShoulderStyle,
    AmbientBackdropMode? ambientBackdropMode,
    String? systemSortBy,
    ...
    return ConfigModel(
      ...
      autoScanOnChange: autoScanOnChange ?? this.autoScanOnChange,
      autoScanOnResume: autoScanOnResume ?? this.autoScanOnResume,
      autoScrapeNewGames: autoScrapeNewGames ?? this.autoScrapeNewGames,
      autoScrapeWifiOnly: autoScrapeWifiOnly ?? this.autoScrapeWifiOnly,
      ...
      gamepadShoulderStyle:
          gamepadShoulderStyle ?? this.gamepadShoulderStyle,
      ambientBackdropMode:
          ambientBackdropMode ?? this.ambientBackdropMode,
  ...
  /// One-time iiSU alignment: the systems view no longer supports carousel.
  ///
  /// Call after loading persisted config (e.g. in [SqliteConfigProvider.initialize])
  /// to silently migrate `carousel` → `grid` without prompting the user.
  static ConfigModel migrateCarouselToGrid(ConfigModel config) {
    if (config.systemViewMode.toLowerCase() == 'carousel') {
      return config.copyWith(systemViewMode: 'grid');
    }
    return config;
  }
