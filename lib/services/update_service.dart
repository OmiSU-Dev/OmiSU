...
class UpdateService {
  static const String _neostationGithubApiUrl =
  ...
  static String get _githubApiUrl {
    if (NordiConfig.curatedBuild && NordiConfig.enableGithubAppOta) {
      return 'https://api.github.com/repos/${NordiConfig.githubReleasesRepo}/releases/latest';
    }
    return _neostationGithubApiUrl;
  }

  static bool _githubAppOtaEnabled() {
    if (kIsWeb) return false;
    return NordiUpdatePolicy.appGithubOtaEnabled;
  }
  ...
  static Future<UpdateInfo?> checkForUpdates() async {
    if (!_githubAppOtaEnabled()) return null;
    ...
    try {
      ...
      var tagBody = rawTag.startsWith('v') || rawTag.startsWith('V')
          ? rawTag.substring(1)
          : rawTag;
      tagBody = tagBody.split('-build').first;
      final latestVersion = tagBody.split('+').first;
      ...
      final latestBuild =
          parseBuildFromVersionString(rawTag)?.toString() ??
          parseBuildFromApkFilename(assetName)?.toString();

      // 3. Semver and, on tie, monotonic +build from pubspec / APK name.
      if (!isNewerRelease(
      ...
      return UpdateInfo(
        ...
        fileName: assetName,
        fileSize: int.tryParse(platformAsset['size']?.toString() ?? '0') ?? 0,
  ...
  static Future<Map<String, dynamic>?> _findPlatformAsset(
    ...
    if (Platform.isAndroid) {
      ...
      final useNordiAssets =
          NordiConfig.curatedBuild && NordiConfig.enableGithubAppOta;
      if (useNordiAssets) {
        platformPattern = Abi.current() == Abi.androidArm
            ? 'nordi-gamesir-armeabi-v7a-'
            : 'nordi-gamesir-arm64-v8a-';
      } else {
        platformPattern = Abi.current() == Abi.androidArm
            ? '-android-armeabi-v7a-'
            : '-android-arm64-v8a-';
...
''';
  ...
  static Future<bool> _installAndroid(File apkFile) async {
    try {
      ...
      final platform = const MethodChannel('com.omisu.launcher/game');
