...
class GameLaunchManager extends ChangeNotifier with WidgetsBindingObserver {
  ...
  /// Built-in libretro play dismisses the launch dialog before the game route
  /// opens so exit lands straight on the menu (Lemuroid-style).
  bool _embeddedPlayActive = false;

  /// Post-game navigation registered by [launchGameWithDialog] for embedded play.
  /// Invoked from [GameLaunchService] after the game route pops so it still runs
  /// when the launcher's [BuildContext] is already disposed.
  VoidCallback? _embeddedPostGameNav;

  /// Guards against duplicate [invokeGameClosedNavigation] calls (embedded handoff
  /// plus launch-dialog dispose both firing would pop the stack to home).
  bool _postGameNavHandled = false;

  GameLaunchPhase? get phase => _phase;
  bool get isActive => _phase != null;

  /// True while embedded play is running after the launch overlay was dismissed.
  bool get embeddedPlayActive => _embeddedPlayActive;

  /// True after [beginEmbeddedPlay] pops the launch overlay before the game route opens.
  bool get embeddedLaunchOverlayDismissed => _embeddedLaunchOverlayDismissed;

  bool _embeddedLaunchOverlayDismissed = false;
  ...
  /// Dismisses the launch overlay and enters the playing phase for embedded play.
  ///
  /// The dialog must not finalize the session on dispose while this flag is set.
  void beginEmbeddedPlay() {
    _embeddedPlayActive = true;
    _embeddedLaunchOverlayDismissed = true;
    _phase = GameLaunchPhase.playing;
    _canDismiss = true;
    notifyListeners();
    _log.i('[GameLaunchManager] Embedded play started — launch overlay dismissed.');
  }

  /// Returns false when post-game navigation already ran for this session.
  bool tryBeginPostGameNavigation() {
    if (_postGameNavHandled) return false;
    _postGameNavHandled = true;
    return true;
  }

  /// Registers exit navigation to run when embedded play finishes.
  void registerEmbeddedPostGameNav(VoidCallback callback) {
    _embeddedPostGameNav = callback;
    _postGameNavHandled = false;
  }

  /// Clears a registered embedded post-game callback without running it.
  void clearEmbeddedPostGameNav() {
    _embeddedPostGameNav = null;
  }

  /// Runs and clears the embedded post-game navigation callback.
  void runEmbeddedPostGameNav() {
    final callback = _embeddedPostGameNav;
    _embeddedPostGameNav = null;
    callback?.call();
  }

  /// Ends an embedded session: playtime, audio restore, no launch-dialog delay.
  Future<void> finalizeEmbeddedPlay() async {
    if (!_embeddedPlayActive) return;
    _embeddedPlayActive = false;
    _log.i('[GameLaunchManager] Embedded play ended — finalizing session.');
    await StreamingService.stopStream();
    _finalize();
  }

  /// Embedded launch failed after the overlay was dismissed — restore session state.
  Future<void> cancelEmbeddedPlay() async {
    if (!_embeddedPlayActive) return;
    _embeddedPlayActive = false;
    _log.w('[GameLaunchManager] Embedded play cancelled — restoring session.');
    await StreamingService.stopStream();
    _finalize();
  }

  /// Called when built-in (embedded) libretro play finishes.
  ///
  /// Legacy path when the launch dialog is still mounted (desktop preview builds).
  /// Prefer [finalizeEmbeddedPlay] for Android embedded play.
  void onEmbeddedGameEnded() {
    if (_embeddedPlayActive) return;
    if (_phase != GameLaunchPhase.playing || _isClosing) return;
    _log.i('[GameLaunchManager] Embedded game ended — closing session.');
    _triggerClose();
  }
  ...
  void onDialogDisposed() {
    if (_embeddedPlayActive) {
      _log.d(
        '[GameLaunchManager] Launch dialog disposed for embedded play — session continues.',
      );
      return;
    }
    // Dialog can be dismissed as embedded play starts while phase is still
    // launching; finalizing here killed the session mid-handoff (see logcat).
    if (_phase == GameLaunchPhase.launching) {
      _log.d(
        '[GameLaunchManager] Launch dialog disposed during launching — session continues.',
      );
      return;
    }
    if (isActive) {
  ...
  void _finalize() {
    ...
    unawaited(MenuAmbientService.instance.resumeAfterGame());
    SfxService().setEnabled(_sfxWasEnabled);
    _phase = null;
    _canDismiss = false;
    _isClosing = false;
    _embeddedPlayActive = false;
    _embeddedLaunchOverlayDismissed = false;
    _sfxWasEnabled = true;
