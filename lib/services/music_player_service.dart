...
import 'package:omisu/services/saf_directory_service.dart';
...
class MusicPlayerService extends ChangeNotifier {
  ...
  Float32List _latestFftVisualization = Float32List(0);
  StreamSubscription<AudioVisualizationData>? _visualizationSub;
  String? _tempPath;
  ...
  void _teardownEngine() {
    ...
    _currentHandle = null;
    _currentSource = null;
    _visualizationSub?.cancel();
    _visualizationSub = null;
    _latestFftVisualization = Float32List(0);
    _isPlaying = false;
  ...
  Future<void> init() async {
    ...
    try {
      ...
      await _visualizationSub?.cancel();
      _visualizationSub = _soloud!.audioVisualizationEvents.listen((data) {
        final fft = data.fftData;
        if (fft != null && fft.isNotEmpty) {
          _latestFftVisualization = fft;
        }
      });
  ...
  Future<void> pauseForGame() async {
    _wasPlayingBeforeGame = _isPlaying || _wasPlayingBeforeGame;
    if (!_isInitialized || _currentHandle == null) return;
    if (_isPlaying) {
      await pause();
    }
  ...
  /// Releases the shared SoLoud engine for embedded play while the app stays
  /// in the foreground (so [appPaused] never runs). Returns true if the engine
  /// was torn down.
  Future<bool> releaseEngineForEmbeddedPlay() async {
    await pauseForGame();
  ...
  /// Resumes playback after embedded play when the engine was torn down.
  Future<void> resumeAfterEmbeddedPlay() async {
    if (!_wasPlayingBeforeGame || _playlist.isEmpty) {
      _wasPlayingBeforeGame = false;
      return;
    }
    final index = _activeIndex >= 0 ? _activeIndex : _currentIndex;
    _wasPlayingBeforeGame = false;
    try {
      await start(index: index, updateUI: true);
    } catch (e) {
      _logger.w('Error resuming music after embedded play: $e');
    }
  }
  ...
  Float32List getAudioData() {
    if (!_isInitialized) return Float32List(0);
    return _latestFftVisualization;
  }
