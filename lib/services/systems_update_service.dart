...
/// main OmiSU frontend repository.
///
...
class SystemsUpdateService {
  static bool get _remoteManifestEnabled =>
  ...
  static String? _changelogFromManifest(Map<String, dynamic> manifest) {
    final raw =
        manifest['changelog'] ??
  ...
  static Future<SystemsUpdateInfo?> checkForUpdate() async {
    if (!_remoteManifestEnabled) return null;
    ...
    try {
      ...
      return SystemsUpdateInfo(
        ...
        changelog: _changelogFromManifest(manifest),
      );
    } catch (e) {
  ...
  static Future<SystemsUpdateResult?> checkAndUpdate({
    ...
    if (!_remoteManifestEnabled && knownUpdate == null) return null;
