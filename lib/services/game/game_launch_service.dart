...
import '../../models/embedded_core_config.dart';
import '../../screens/systems_screen/my_systems_section/my_systems_grid.dart';
...
class GameLaunchResult {
  ...
  /// True when built-in libretro play handled the session (fast exit path).
  final bool embeddedPlay;

  GameLaunchResult.success({this.embeddedPlay = false})
    : success = true,
      errorMessage = null,
      errorDetails = null;
...
class GameLaunchService {
  ...
  static Future<GameLaunchResult> launchGame(
    ...
    try {
      ...
      // Built-in libretro play (LibretroDroid + LemuroidCores) on Android only.
      // On Linux/Windows/macOS preview builds, embedded-capable systems fall
      // through to RetroArch below so ROM paths and launch can be tested before
      // installing the APK — the phone build still uses in-app cores.
      if (Platform.isAndroid &&
          EmbeddedCoreRegistry.supports(system.folderName)) {
        _logLaunchTuning(system, game);
        return await _launchEmbedded(context, system, game);
      }

      if (!Platform.isAndroid &&
          EmbeddedCoreRegistry.supports(system.folderName)) {
        _log.i(
          '[Launch] ${system.folderName}: desktop preview launching via RetroArch; '
          'the Android APK uses built-in libretro cores for this system.',
        );
      }
      ...
      if (preferredPlayerId == null && system.id != null) {
        ...
        if (userDefault != null) {
          ...
          if (defaultEmu != null) {
            ...
            source = 'auto-resolved default (device-tuned)';
          }
        }
      }

      _logLaunchTuning(system, game, preferredPlayerId: preferredPlayerId);
  ...
  /// Launches a game with an embedded libretro core (Android only).
  static Future<GameLaunchResult> _launchEmbedded(
    BuildContext context,
    SystemModel system,
    GameModel game,
  ) async {
    try {
      ...
      // Mark embedded *before* popping the launch overlay so dispose cannot
      // finalize the session mid-handoff.
      GameLaunchManager().beginEmbeddedPlay();
      if (context.mounted) {
        Navigator.of(context).pop();
      }

      final launched = await launchEmbeddedGame(context, system, game);

      if (launched) {
        await GameLaunchManager().finalizeEmbeddedPlay();
        GameLaunchManager().runEmbeddedPostGameNav();
        return GameLaunchResult.success(embeddedPlay: true);
      }

      await GameLaunchManager().cancelEmbeddedPlay();
      ...
      _log.e('Embedded launch error: $e');
      if (!context.mounted) return GameLaunchResult.failure('', '');
      return GameLaunchResult.failure(
        AppLocale.anErrorOccurred.getString(context),
        e.toString(),
    ...
    try {
      ...
      if ((Platform.isWindows || Platform.isLinux || Platform.isMacOS) &&
        ...
        if (Platform.isLinux &&
          ...
          return GameLaunchResult.failure(
            AppLocale.retroArchNotFound.getString(context),
            'RetroArch is not installed or not configured. Install RetroArch '
            '(Flatpak, distro package, or EmuDeck) or set its path under '
            'Settings → Emulators.',
          );
        }
        return GameLaunchResult.failure(
  ...
  static Future<CoreEmulatorModel?> _resolveDefaultInstalledEmulator(
    ...
    return LaunchTuningResolver.pickTunedEmulator(
          candidates: all,
          tuning: tuning,
  ...
  static void _logLaunchTuning(
    ...
    final tuning = LaunchTuningResolver.resolve(
      device: device,
      systemFolder: system.folderName,
      coreName: preferredPlayerId,
    );
    _log.i(
      '[LaunchTune] ${system.folderName}/${game.romname}: '
      'device=${device.id} tier=${device.tier.name} '
      'corePref=${tuning.corePreferenceKeyword ?? "<none>"} '
      'skipDup=${tuning.skipDuplicateFrames} vars=${tuning.coreVariables.length}'
      '${preferredPlayerId != null ? ' emulator=$preferredPlayerId' : ''}',
    );
    LaunchTuningStore.instance.record(
      systemFolder: system.folderName,
      gameName: game.romname,
      tuning: tuning,
      emulatorId: preferredPlayerId,
    );
  }
  ...
  /// screen — every D-pad press navigated OmiSU behind the emulator and
  ...
  static Future<void> handleAppResumed() async {
    if (GameSessionManager.isGameLaunched) {
      ...
      if (timeSinceLaunch.inSeconds > gracePeriod) {
        ...
        if (GameSessionManager.launchedEmulatorExe != null) {
          final exe = GameSessionManager.launchedEmulatorExe!;
          if (exe.startsWith('embedded_')) {
            // Built-in libretro play has no external process to probe.
            emulatorStillRunning = true;
          } else {
            emulatorStillRunning = await _isProcessRunning(exe);
          }
        } else {
