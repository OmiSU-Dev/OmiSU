...
/// Model representing a theme available in the OmiSU assets repository.
...
/// (themes, logos, backgrounds) from the OmiSU assets repository.
class NeoAssetsService {
  ...
  /// even offline — e.g. when OmiSU is the home launcher and cold-starts
  ...
  static Future<List<NeoAssetsTheme>> fetchThemes() async {
    ...
    if (remote != null) {
      final merged = await _mergeWithLocalThemes(
        await _mergeBundledCatalog(remote),
      );
    ...
    final fallback = await _mergeWithLocalThemes(
      await _mergeBundledCatalog(await _readCachedManifest()),
    );
  ...
  /// Bundled catalog entries (iiSU, future packs) appear only when their
  /// theme.json exists on the assets CDN — avoids offering packs that cannot
  /// download yet.
  static Future<List<NeoAssetsTheme>> _mergeBundledCatalog(
    List<NeoAssetsTheme> base,
  ) async {
    final bundled = await _readBundledCatalog();
    if (bundled.isEmpty) return base;

    final seen = base.map((t) => t.folder).toSet();
    final merged = [...base];
    for (final theme in bundled) {
      if (seen.contains(theme.folder)) continue;
      final metadata = await _fetchThemeMetadata(theme.folder);
      if (metadata == null) continue;
      merged.add(theme);
      seen.add(theme.folder);
    }
    return merged;
  }

  static Future<List<NeoAssetsTheme>> _readBundledCatalog() async {
    try {
      final raw = await rootBundle.loadString(
        'assets/data/system_art_catalog.json',
      );
      ...
      _log.w('Bundled system art catalog unavailable: $e');
      return [];
    }
  }
