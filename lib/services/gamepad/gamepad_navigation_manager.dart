...
class GamepadNavigationManager {
  ...
  /// Clears the remembered launch focus owner without activating any layer.
  /// Used when exiting to the systems home tab instead of returning to the
  /// launcher's screen.
  static void clearLaunchFocusOwner() {
    _focusOwnerId = null;
  }
  ...
  static void restoreFocusOwner() {
    ...
    restoreLayerToTop(ownerId);
  }

  /// Moves [id] to the top of the stack and activates it.
  ///
  /// When the layer was unregistered during gameplay (grid/carousel unmount while
  /// [_isGameLaunching] hid the list), optionally re-push it via [onActivate] /
  /// [onDeactivate]. Does not fall back to an unrelated top layer — that was
  /// waking [my_systems_list] after embedded play and felt like "exit to home".
  static void restoreLayerToTop(
    String id, {
    void Function()? onActivate,
    void Function()? onDeactivate,
  }) {
    var index = _stack.indexWhere((layer) => layer.id == id);
    if (index == -1) {
      if (onActivate != null && onDeactivate != null) {
        _log.i(
          '[GamepadNavigationManager] Re-registering missing layer $id after gameplay',
        );
        pushLayer(
          id,
          onActivate: onActivate,
        ...
        _log.w(
          '[GamepadNavigationManager] Layer $id not in stack; skipping focus restore',
        );
      }
      return;
    }
    ...
    if (index != _stack.length - 1) {
      ...
      _log.i(
        '[GamepadNavigationManager] Restoring $id over ${displaced.id}, '
