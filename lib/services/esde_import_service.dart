...
class EsdeImportResult {
  /// Number of ES-DE system folders matched to a OmiSU system.
  ...
  /// Number of ES-DE system folders that mapped to a OmiSU system but
...
/// OmiSU's `user_screenscraper_metadata` on a fill-gaps-only basis (never
/// clobbering existing OmiSU-scraped values). Artwork is NOT copied: the
...
/// (a later OmiSU scrape lands in OmiSU's own media folder and takes
...
class EsdeImportService {
  ...
  static Future<EsdeImportResult> import(
    ...
    for (var i = 0; i < systemDirs.length; i++) {
      ...
      if (system == null) {
        _log.i(
          'ES-DE import: no OmiSU system for "$esdeDirName", skipping',
        );
  ...
  /// (`esde_imported = 1`) that a later OmiSU scrape hasn't upgraded
  /// (`is_fully_scraped = 0`) — never OmiSU's own partially-scraped rows,
  ...
  /// ROM OmiSU has scanned. Without this the whole system's art is
  ...
  static Future<EsdeImportResult> _importSystem({
    ...
    for (var g = 0; g < games.length; g++) {
      ...
      // ES-DE says nothing about whether the user wants it in OmiSU, and
      // silently dropping it makes the import look broken to anyone who has
      // forgotten what they hid. OmiSU has its own hidden-game handling.
      ...
      // Only import for ROMs OmiSU has already scanned.
      ...
      // Only fill when OmiSU has never timed this ROM, so an import can
  ...
  /// Persists which ES-DE media subfolder backs a OmiSU system so
  ...
  /// ES-DE stores rating as a 0..1 float string; OmiSU stores it on
  ...
  /// `All but the Best (…)/`). They all collapse onto a single OmiSU
