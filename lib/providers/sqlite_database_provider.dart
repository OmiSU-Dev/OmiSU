...
class SqliteDatabaseProvider extends ChangeNotifier {
  ...
  /// Updates the in-memory catalogue after [FavoritesService.recordGamePlayed]
  /// writes `last_played` to SQLite (launch paths use rom path, not folder/name).
  Future<void> syncRomPlayedInCache(String romPath) async {
    final now = DateTime.now();
    for (final games in _database.values) {
      final gameIndex = games.indexWhere((game) => game.romPath == romPath);
      if (gameIndex != -1) {
        games[gameIndex] = games[gameIndex].copyWith(lastPlayed: now);
        notifyListeners();
        return;
      }
    }
    // ROM not in cache (e.g. list not loaded yet) — reload from disk.
    await loadDatabase();
  }
