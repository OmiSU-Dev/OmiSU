...
extension SqliteConfigMutators on SqliteConfigProvider {
  ...
  /// Updates whether tab-cycle hints show LB/RB or LT/RT glyphs.
  Future<void> updateGamepadShoulderStyle(GamepadShoulderStyle value) async {
    _config = _config.copyWith(gamepadShoulderStyle: value);
    await SqliteConfigService.saveConfig(_config);
    _notify();
  }
  ...
  Future<void> updateSfxEnabled(bool value) async {
    ...
    MenuAmbientService.instance.setEnabled(value);
  ...
  Future<void> updateAutoScanOnChange(bool value) async {
    _config = _config.copyWith(autoScanOnChange: value);
    await SqliteConfigService.saveConfig(_config);
    LibraryWatchCoordinator.instance.refreshWatchers();
    _notify();
  }

  Future<void> updateAutoScanOnResume(bool value) async {
    _config = _config.copyWith(autoScanOnResume: value);
    await SqliteConfigService.saveConfig(_config);
    LibraryWatchCoordinator.instance.refreshWatchers();
    _notify();
  }

  Future<void> updateAutoScrapeNewGames(bool value) async {
    _config = _config.copyWith(autoScrapeNewGames: value);
    await SqliteConfigService.saveConfig(_config);
    _notify();
  }

  Future<void> updateAutoScrapeWifiOnly(bool value) async {
    _config = _config.copyWith(autoScrapeWifiOnly: value);
    await SqliteConfigService.saveConfig(_config);
    _notify();
  }
  ...
  /// Sets the shell ambient backdrop mode (off / subtle / reactive).
  Future<void> updateAmbientBackdropMode(AmbientBackdropMode value) async {
    if (_config.ambientBackdropMode == value) return;
    _config = _config.copyWith(ambientBackdropMode: value);
    await SqliteConfigService.saveConfig(_config);
    _notify();
  }
}
