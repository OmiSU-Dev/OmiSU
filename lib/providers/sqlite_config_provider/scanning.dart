...
extension SqliteConfigScanning on SqliteConfigProvider {
  ...
  Future<void> scanSystems({bool waitForAndroidStorage = false}) async {
    ...
    try {
      ...
      Map<String, String>? subdirSystemMap;
      if (_config.romFolders.isNotEmpty) {
        final maps = await _buildScanFolderMaps();
        rootFoldersMap = maps.rootFoldersMap;
        subdirSystemMap = maps.subdirSystemMap;
      }
      ...
      if (Platform.isAndroid) {
        ...
        final filteredSystems = systemsForMapping.where((system) {
          if (system.id != null &&
              subdirSystemMap != null &&
              subdirSystemMap.values.contains(system.id)) {
            return true;
          }
      ...
      if (skipScan) {
        ...
        await _scanRomsInBackground(
          ...
          subdirSystemMap: subdirSystemMap,
        );
      }
  ...
  Future<void> _scanRomsInBackground({
    Map<String, Map<String, String>>? rootFoldersMap,
    Map<String, String>? subdirSystemMap,
  }) async {
    ...
    try {
      ...
      // Queue ScreenScraper for every system that still lacks per-game art.
      // Incremental file-watch rescans call onGamesAdded; a full pass must
      // cover libraries already in the DB (added=0 on every rescan).
      const virtualSystemsForScrape = {
        'android',
        'music',
        'all',
        'steam',
      ...
      final scrapeTargets = systemsToKeep
          .where((s) => !virtualSystemsForScrape.contains(s.folderName))
          .map((s) => s.folderName)
          .toList();
      if (scrapeTargets.isNotEmpty) {
        _queueAutoScrapeForSystems(scrapeTargets);
      }
  ...
  /// Builds ROM-root subdirectory maps for custom folder name resolution.
  Future<
      ({
        Map<String, Map<String, String>> rootFoldersMap,
        Map<String, String> subdirSystemMap,
      })> _buildScanFolderMaps() async {
    final rootFoldersMap =
        await SqliteDatabaseService.getExistingSubdirectories(
          _config.romFolders,
        );
    final subdirSystemMap = await RomFolderSystemResolver.buildSubdirSystemMap(
      rootFoldersMap: rootFoldersMap,
      availableSystems: _availableSystems,
    );
    return (rootFoldersMap: rootFoldersMap, subdirSystemMap: subdirSystemMap);
  }
  ...
  Future<ScanSummary> _scanSystemRoms(
    ...
    try {
      ...
      if (rootFoldersMap == null || subdirSystemMap == null) {
        final maps = await _buildScanFolderMaps();
        rootFoldersMap ??= maps.rootFoldersMap;
        subdirSystemMap ??= maps.subdirSystemMap;
      }
  ...
  Future<void> refreshSystem(
    ...
    try {
      ...
      final updatedSystem = system.id != null
          ? await SystemRepository.getSystemById(system.id!)
          : await SystemRepository.getSystemByFolderName(system.folderName);
  ...
  Future<void> _loadConfig() async {
    ...
    final migrated = ConfigModel.migrateCarouselToGrid(_config);
    if (migrated.systemViewMode != _config.systemViewMode) {
      _config = migrated;
      await SqliteConfigService.saveConfig(_config);
    }
  ...
  /// Queues a background ScreenScraper pass for every hardware system in
  /// [folderNames] that still has unscraped ROMs.
  void _queueAutoScrapeForSystems(Iterable<String> folderNames) {
    final targets = folderNames.toList(growable: false);
    if (targets.isEmpty) return;
    unawaited(AutoScrapeCoordinator.instance.onLibraryScanComplete(targets));
  }
}
