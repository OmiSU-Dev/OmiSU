...
import 'package:omisu/services/neo_assets_service.dart';
...
import '../widgets/setup/setup_wizard_shell.dart';
...
class _SetupWizardState extends State<SetupWizard> with WidgetsBindingObserver {
  ...
  void _updateSecondaryScreen(int bgColor, bool isOled) {
    ...
    _secondaryDisplayState!.updateState(
      ...
      systemName: AppLocale.welcomeOmiSU.getString(context),
      useFluidShader: true,
  ...
  /// Whether the user can return to the previous wizard step.
  bool get _canGoBack =>
      _currentStep > 0 &&
      !_isSelectingFolder &&
      !_isSelectingUserDataFolder &&
      !_isImportingEsde &&
      !_isDownloadingArt;
  ...
  List<String> _stepLabels(BuildContext context) {
    ...
    if (Platform.isAndroid) {
      labels.add(AppLocale.storagePermission.getString(context));
    }
    labels.addAll([
  ...
  String _stepTitle(BuildContext context) {
    ...
    if (_currentStep >= 0 && _currentStep < labels.length) {
      return labels[_currentStep];
    }
    return AppLocale.setupLibrary.getString(context);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SetupWizardShell(
      ...
      stepTitle: _stepTitle(context),
      welcomeTitle: AppLocale.welcomeOmiSU.getString(context),
      welcomeSubtitle: AppLocale.letsGetSetup.getString(context),
      content: _buildStepContent(theme),
      footer: _buildNavigationButtons(theme),
    );
  }

  // Legacy layout helpers removed — shell chrome lives in [SetupWizardShell].
  ...
  Future<void> _selectUserDataLocationWizard() async {
    ...
    try {
      ...
      // unknowingly store OmiSU's data inside an existing library.
  ...
  NeoAssetsTheme _recommendedArtPack(List<NeoAssetsTheme> themes) {
    return themes.firstWhere((t) => !t.isAi, orElse: () => themes.first);
  }

  String _resolvedArtPackFolder(NeoAssetsProvider neoAssets) {
    if (neoAssets.themes.isEmpty) return '';
    final selected = _selectedArtPackFolder;
    if (selected != null &&
        neoAssets.themes.any((theme) => theme.folder == selected)) {
      return selected;
    }
    final recommended = _recommendedArtPack(neoAssets.themes);
    _selectedArtPackFolder = recommended.folder;
    return recommended.folder;
  }
            ...
            children: [
              ...
              Text(
                hasTheme
                    ...
                    : '${AppLocale.wizardArtPackDesc.getString(context)}\n\n${AppLocale.wizardArtPackPick.getString(context)}',
                style: TextStyle(
                  fontSize: textSize,
                  color: theme.colorScheme.onSurface.withValues(alpha: 0.7),
                  height: 1.3,
              ...
              // Pack picker when more than one compatible pack is published.
              if (!hasTheme && !unavailable && neoAssets.themes.length > 1) ...[
                SizedBox(height: isLandscape ? 10.r : 14.r),
                Wrap(
                  spacing: 8.r,
                  runSpacing: 8.r,
                  alignment: WrapAlignment.center,
                  children: neoAssets.themes.map((pack) {
                    final selected =
                        _resolvedArtPackFolder(neoAssets) == pack.folder;
                    return ChoiceChip(
                      label: Text(pack.name),
                      selected: selected,
                      onSelected: neoAssets.downloading
                          ? null
                          : (_) => setState(
                              () => _selectedArtPackFolder = pack.folder,
                            ),
                    );
                  }).toList(),
                ),
              ...
              // Small preview thumbnail of the selected pack.
              if (!hasTheme && !unavailable && !neoAssets.downloading) ...[
                Builder(
                  builder: (context) {
                    final folder = _resolvedArtPackFolder(neoAssets);
                    final selected = neoAssets.themes.firstWhere(
                      (t) => t.folder == folder,
                      orElse: () => _recommendedArtPack(neoAssets.themes),
                    );
  ...
  /// Downloads and applies the recommended OmiSU art pack (the first
      ...
      return Consumer<SqliteConfigProvider>(
        builder: (context, provider, child) {
          return Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              if (_canGoBack) _buildBackButton(theme) else SizedBox(width: 64.r),
              ElevatedButton(
        ...
        return Row(
          ...
          children: [
            Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                if (_canGoBack) _buildBackButton(theme),
                if (showSkip) ...[
                  if (_canGoBack) SizedBox(width: 4.r),
                  _buildSkipButton(theme),
                ],
                if (!_canGoBack && !showSkip) SizedBox(width: 64.r),
              ],
            ),
