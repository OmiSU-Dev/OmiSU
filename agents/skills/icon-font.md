...
Read this before adding a branded glyph to `default/fonts/omisu/omisu.ttf`.

The OmiSu icon font is a small private-use font carrying the marks Nerd
Fonts does not have: the OmiSu logo and the agent and app brand marks. The
menu draws one by naming the font on an entry:

```jsonc
"setup.default.agent.grok": {"icon":"","iconFont":"omisu","label":"Grok", ...}
```
...
`default/fonts/omisu/README.md` lists every glyph with the URL its artwork
came from. Keep that list accurate — it is the only record of provenance.

## Adding a glyph

`omisu dev font` does the work:

```bash
omisu dev font list
omisu dev font add ollama https://simpleicons.org/icons/ollama.svg
```
...
- Point the menu entry at the new codepoint with `"iconFont":"omisu"`.
...
The font is package-owned: `omisu-settings` installs it to
`/usr/share/fonts/omisu/omisu.ttf`, so a new glyph reaches the desktop
through a settings release, not through `omisu update`. Between the merge and
...
magick -background white -fill black -font default/fonts/omisu/omisu.ttf \
...
`omisu menu refresh` alone will not pick up a changed font.
