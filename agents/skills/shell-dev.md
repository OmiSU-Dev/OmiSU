...
Run `omisu-restart-shell` after making changes to QML files.
...
- First-party plugins live directly under `shell/plugins/` or one category
  ...
  at `~/.config/omisu/plugins/<id>/` with a `manifest.json` at the root.
- Every plugin manifest declares `schemaVersion`, `id`, `name`, `version`,
  `kinds`, and `entryPoints`. See
  [`docs/omisu-shell.md`](../../docs/omisu-shell.md) and
...
- Entry-point QML files are `Item`s (not `ShellRoot`), and accept the
  shell-injected properties `omisuPath`, `shell`, `manifest`, and
...
- `bin/omisu-shell` is the canonical IPC entry point. It forwards to
...
- The `shell` IPC target exposes lifecycle and configuration methods including
  ...
  `image-selector`, which drives the `omisu.image-picker` panel.
- Individual plugins register their own IPC targets, named for the plugin rather
  ...
  bar widgets register one target each — `omisu.indicators`,
  `omisu.system-update`, `omisu.clock`. There is no `bar` target.
