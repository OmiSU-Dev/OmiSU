...
`test/acceptance.d/*-test.sh`. It exercises a real installed OmiSu desktop,
...
Run acceptance tests in a disposable VM through the sibling `omisu-iso`
...
```bash
cd ../omisu-iso
./bin/omisu-iso-test release/<iso>.iso --reuse-base --sync-omisu ../omisu --no-preview
```

Use `--sync-all ../omisu` instead of `--sync-omisu ../omisu` when the
...
```bash
cd ../omisu-iso
./bin/omisu-iso-make --no-boot-offer --local-source ../omisu ../omisu-pkgs
./bin/omisu-iso-test release/<generated-iso>.iso --no-preview
```
