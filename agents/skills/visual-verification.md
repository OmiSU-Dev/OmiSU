...
Read this before finishing any change with a visual effect: OmiSu shell
...
```bash
omisu capture screenshot fullscreen save
```

The command prints the saved path and writes to the configured Pictures
directory. Use `omisu screenshot` for the interactive smart-region flow.
...
```bash
omisu screenrecord --fullscreen
# Exercise the changed behavior.
omisu screenrecord --stop-recording
```
