...
zram_conf="${OMISU_ZRAM_CONF:-/etc/systemd/zram-generator.conf}"
zram_dropin="${OMISU_ZRAM_DROPIN:-/usr/lib/systemd/zram-generator.conf.d/90-omisu.conf}"

# The tuning ships as /usr/lib/systemd/zram-generator.conf.d/90-omisu.conf.
...
# that sets nothing at all, which omisu-migrate's -e would take as a failed
...
if [[ -z $settings || $settings =~ ^\[zram0\]compression-algorithm=[[:alnum:]-]+$ ]]; then
  ...
  echo "OmiSu's drop-in overrides it. Move your changes to"
