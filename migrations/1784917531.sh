...
# race is fixed upstream. The packaged omisu-defaults.conf now carries the
# same parameter for fresh installs.

if omisu-cmd-present limine-mkinitcpio &&
  [[ -f /etc/limine-entry-tool.d/omisu-defaults.conf ]] &&
  ! grep -rqs "initramfs_async" /etc/limine-entry-tool.d/ /etc/default/limine; then
  echo 'KERNEL_CMDLINE[default]+=" initramfs_async=0"' |
    sudo tee -a /etc/limine-entry-tool.d/omisu-defaults.conf >/dev/null
