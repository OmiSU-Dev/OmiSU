...
# config, but omisu-settings ships omisu-defaults.conf with that parameter
...
# landed. Such a machine boots without any of omisu-defaults.conf's command
# line — including initramfs_async=0, so Plymouth still loses the LUKS prompt.

defaults_conf="${OMISU_LIMINE_DEFAULTS_CONF:-/etc/limine-entry-tool.d/omisu-defaults.conf}"
running_cmdline="${OMISU_RUNNING_CMDLINE:-/proc/cmdline}"
rebuild_marker="${OMISU_LIMINE_REBUILD_MARKER:-/var/lib/omisu/migrations/1786482992}"
