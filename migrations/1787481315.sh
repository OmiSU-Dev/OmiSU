...
theme_name_path="$HOME/.local/state/omisu/current/theme.name"
...
if [[ ! -d $OMISU_PATH/themes/$theme_name && ! -d $HOME/.config/omisu/themes/$theme_name ]]; then
  echo "Theme '$theme_name' no longer exists; applying Phoenix instead"
  omisu-theme phoenix
  exit 0
fi
