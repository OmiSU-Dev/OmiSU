...
# omisu-mise-install gained `--quiet` on its `mise use -g` line so a wrapper
...
# caller cannot parse. Rewrite them through omisu-mise-install so the
...
for wrapper in "$bin_dir"/*; do
  ...
  omisu-mise-install "$package" "${wrapper##*/}" "$bin"
done
