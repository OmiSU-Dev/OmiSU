...
# rewrite the host), so OmiSu no longer puts users in it by default. Bring
...
# it from prompting mid-update — omisu-update-restart handles the reboot once
# the whole update has finished.
if id -nG "$USER" | grep -qw docker; then
  OMISU_DEFER_REBOOT=1 omisu-remove-security-sudoless-docker
fi
...
if [[ -f $dest ]]; then
  cp "$OMISU_PATH/applications/Docker.desktop" "$dest"
fi
