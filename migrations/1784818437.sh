...
# omisu-setup-security-fingerprint.
#
# The gate points at the fixed /usr/bin path the omisu package always
# provides, so it keeps working across package installs and dev-link (which
# overlays $OMISU_PATH but leaves /usr/bin untouched). pam_exec needs a
# literal absolute path — it does not expand env vars.

gate="auth      [success=1 default=ignore] pam_exec.so quiet /usr/bin/omisu-hw-laptop-closed"

for pam in /etc/pam.d/sudo /etc/pam.d/polkit-1; do
  if [[ -f $pam ]] &&
    grep -q 'pam_fprintd\.so' "$pam" &&
    ! grep -q 'omisu-hw-laptop-closed' "$pam"; then
