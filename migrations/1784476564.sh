...
# omisu_hooks.conf now applies the same condition on every rebuild.

hooks_conf="/etc/mkinitcpio.conf.d/omisu_hooks.conf"

# vconsole.conf only guarantees KEYMAP, and it need not exist at all. Sourcing
# one that isn't there fails, and under omisu-migrate's `bash -euo pipefail`
