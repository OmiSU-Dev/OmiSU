echo "Backfill hardware support and tmux settings added before OmiSu quattro"
...
hardware_packages=()
if lspci | grep -iE '(Multimedia audio controller|Audio device).*Intel' >/dev/null && omisu-pkg-missing sof-firmware; then
  hardware_packages+=(sof-firmware)
fi
if lspci | grep -iE '(VGA|Display).*Intel' >/dev/null && omisu-pkg-missing vulkan-intel; then
  hardware_packages+=(vulkan-intel)
fi
if lspci | grep -iE '(VGA|Display).*AMD' >/dev/null && omisu-pkg-missing vulkan-radeon; then
  hardware_packages+=(vulkan-radeon)
fi
if lspci | grep -iE '(VGA|Display).*Apple' >/dev/null && omisu-pkg-missing vulkan-asahi; then
  hardware_packages+=(vulkan-asahi)
fi

if (( ${#hardware_packages[@]} > 0 )); then
  omisu-pkg-add "${hardware_packages[@]}"
  omisu-state set reboot-required
fi

if omisu-hw-match "DX13260"; then
