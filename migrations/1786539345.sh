...
# Both halves are set up in paths that only run once -- omisu-provision-user
...
if ! systemctl --user enable omisu-crash-watch.service >/dev/null 2>&1; then
  wants_dir="$HOME/.config/systemd/user/graphical-session.target.wants"
  mkdir -p "$wants_dir"
  ln -sfn /usr/lib/systemd/user/omisu-crash-watch.service \
    "$wants_dir/omisu-crash-watch.service"
fi
...
if systemctl --user is-active --quiet graphical-session.target; then
  systemctl --user start omisu-crash-watch.service >/dev/null 2>&1 || true
fi
