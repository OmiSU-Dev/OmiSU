...
# Drop only the hooks OmiSu installed, wherever the migrations that wrote
...
if [[ -f $tmux_config ]]; then
  ...
  if awk '
    ...
    /^[[:space:]]*set-hook -g alert-(bell|activity|silence) .*omisu\.indicators refresh/ ||
    /^[[:space:]]*set-hook -g (after-select-window|client-session-changed|client-focus-(in|out))(\[[0-9]+\])? .*(omisu-tmux-alert track|omisu\.indicators refresh)/ {
      header = ""
      dropped = 1
      next
    }
...
if tmux has-session 2>/dev/null; then
  while read -r hook; do
    ...
    awk '$0 ~ /omisu-tmux-alert|omisu\.indicators refresh/ { print $1 }')

  # The removed track subcommand stamped this on every window it saw.
  while read -r window; do
    [[ -n $window ]] || continue
    tmux set-option -wqu -t "$window" @omisu_unfocused_activity 2>/dev/null || true
...
if [[ -s $config_file ]] && grep -q 'TmuxAlert' "$config_file"; then
  ...
  if jq '
    ...
    def cleaned:
      if type == "object" and .id == "omisu.indicators" then stripped("items") | stripped("indicators") else . end;

    def emptied:
      type == "object" and .id == "omisu.indicators" and
