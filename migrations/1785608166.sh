...
# The old unit started before UWSM finished importing OMISU_PATH and
# WAYLAND_DISPLAY. A full unit retained from before OmiSu 4 also shadows the
# corrected package unit, so add the lifecycle constraints as a drop-in without
# discarding any user customizations.
user_config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
sleep_lock_unit="$user_config_home/systemd/user/omisu-sleep-lock.service"
sleep_lock_dropin_dir="$user_config_home/systemd/user/omisu-sleep-lock.service.d"
sleep_lock_dropin="$sleep_lock_dropin_dir/90-omisu-session-environment.conf"
...
if [[ -f $sleep_lock_unit ]]; then
  ...
  printf '%s\n' \
    ...
    'ConditionEnvironment=OMISU_PATH' \
...
if ! error=$(systemctl --user show-environment 2>&1); then
  if [[ -S $user_manager_socket ]]; then
    ...
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  fi
  exit 0
fi
...
if [[ $graphical_state == "active" ]]; then
  if ! error=$(systemctl --user reset-failed omisu-sleep-lock.service 2>&1); then
    echo "Could not reset omisu-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  elif ! error=$(systemctl --user restart omisu-sleep-lock.service 2>&1); then
    echo "Could not restart omisu-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  elif ! sleep_lock_state=$(systemctl --user show --property=ActiveState --value omisu-sleep-lock.service 2>&1); then
    echo "Could not inspect omisu-sleep-lock.service: $sleep_lock_state"
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  elif [[ $sleep_lock_state != "active" ]]; then
    echo "omisu-sleep-lock.service did not stay active after restart."
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  fi
else
  ...
  if ! error=$(systemctl --user stop omisu-sleep-lock.service 2>&1); then
    echo "Could not stop stale omisu-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  elif ! error=$(systemctl --user reset-failed omisu-sleep-lock.service 2>&1); then
    echo "Could not reset omisu-sleep-lock.service: $error"
    echo "The pre-suspend lock repair will be retried by omisu-migrate."
    exit 1
  fi
