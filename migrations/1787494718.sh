...
# omisu-migrate records this migration as complete whenever it exits zero, so
...
report_unrepairable() {
  echo "  $1"
  echo "  $2"
  omisu-notification-send -u critical -g  "FIDO2 authfile needs attention" "$1 $2" || true
}
