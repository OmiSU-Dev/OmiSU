...
marker="${OMISU_BLUETOOTH_MIGRATION_MARKER:-/var/lib/omisu/migrations/1786380259}"
main_conf="${OMISU_BLUETOOTH_MAIN_CONF:-/etc/bluetooth/main.conf}"
...
if omisu-bluetooth-power is-on; then
  sudo omisu-bluetooth-power on
else
  sudo omisu-bluetooth-power off
fi

# OmiSu set AutoEnable=false believing bluetoothd would then restore the last
...
# OmiSu wrote is reverted, so a hand-edited opt-out survives.
if [[ -f $main_conf ]]; then
