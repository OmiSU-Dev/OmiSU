...
if changed:
    shutil.copy2(path, path.with_name(path.name + ".omisu-copy-url-repair.bak"))
...
unverified_repairs_exist() {
  ...
  for profile_root in "${profile_roots[@]}"; do
    for backup in "$profile_root"/*/Preferences.omisu-copy-url-repair.bak; do
      [[ -f $backup ]] && return 0
    done
  done
...
while affected_profile_open; do
  if ! gum confirm "Close the browser windows to repair the Copy URL shortcut, then continue"; then
    ...
    echo "Close the browser windows, then run: omisu-migrate" >&2
    exit 1
  fi
done
