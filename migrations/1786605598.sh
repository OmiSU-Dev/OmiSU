...
# omisu_hooks.conf now filters the kms hook out of HOOKS when the proprietary
...
# dropped out. A user-edited omisu_hooks.conf (pacman leaves the packaged
# update as a .pacnew) keeps kms and correctly skips the rebuild.

hooks_conf="${OMISU_MKINITCPIO_HOOKS_CONF:-/etc/mkinitcpio.conf.d/omisu_hooks.conf}"
nvidia_conf="${OMISU_MKINITCPIO_NVIDIA_CONF:-/etc/mkinitcpio.conf.d/nvidia.conf}"
rebuild_marker="${OMISU_KMS_REBUILD_MARKER:-/var/lib/omisu/migrations/1786605598}"
