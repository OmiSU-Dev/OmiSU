...
# omisu-update-user-notify.path watched /usr/share/omisu/migrations, but
# pacman writes that directory during every update -- including the blessed
# `omisu update`, which runs omisu-migrate a step later. The watcher fired a
...
systemctl --user stop omisu-update-user-notify.path >/dev/null 2>&1 || true
...
# usually runs from inside `omisu update`, and starting the notifier here would
...
# `omisu update` over SSH does not have, so fall back to writing precisely the
# symlink it would have written rather than silently doing nothing.
if ! systemctl --user enable omisu-migrate-notify.service >/dev/null 2>&1; then
  mkdir -p "$wants_dir"
  ln -sfn /usr/lib/systemd/user/omisu-migrate-notify.service \
    "$wants_dir/omisu-migrate-notify.service"
fi

# Drop the retired enablement by hand instead of through `systemctl disable`.
# The package ships omisu-update-user-notify.service as a compatibility
...
rm -f "$wants_dir/omisu-update-user-notify.path" \
  "$wants_dir/omisu-update-user-notify.service"

systemctl --user reset-failed omisu-update-user-notify.path >/dev/null 2>&1 || true
systemctl --user daemon-reload >/dev/null 2>&1 || true
