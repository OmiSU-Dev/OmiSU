echo "Install herdr from the OmiSu package repo and seed its config"

# The package was briefly published as omisu-herdr; herdr replaces it
omisu-pkg-drop omisu-herdr
omisu-pkg-add herdr
...
# Only seed. A user who already has a herdr config keeps it; omisu-refresh-herdr
# is the explicit way to take the shipped defaults.
[[ -f "$HOME/.config/herdr/config.toml" ]] || omisu-refresh-config herdr/config.toml
