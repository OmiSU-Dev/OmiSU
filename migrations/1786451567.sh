...
# literal "~/.config/omisu/current/..." string. The replacement used the same
...
#
# No shipped OmiSu code ever wrote these six targets with a literal tilde, so
...
relink_if_dangling() {
  ...
  case "$target" in
    "~/"*/omisu/current/theme/"${expected_target##*/}") ;;
    *) return 0 ;;
  esac
...
relink_if_dangling "$HOME/.config/helix/themes/omisu.toml" \
  "$current_state_dir/theme/helix.toml"

for vscode_dir in "$HOME/.vscode" "$HOME/.vscode-insiders" "$HOME/.vscode-oss" "$HOME/.cursor"; do
  relink_if_dangling "$vscode_dir/extensions/omisu-theme/themes/omisu-color-theme.json" \
