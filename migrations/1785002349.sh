...
current_absolute_target="$HOME/.local/state/omisu/current/theme/neovim.lua"
...
# one ("../../../../.config/omisu/current/theme/neovim.lua"), leaving those
# links dangling. Match the shared suffix instead so every spelling is repaired.
case "$target" in
  */omisu/current/theme/neovim.lua) ;;
  *) exit 0 ;;
esac
