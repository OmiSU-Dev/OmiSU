...
rm -f "$HOME/.local/state/omisu/toggles/hypr/voxtype.lua"

# The Quattro upgrade runs the packaged migrations against a still-running
# OmiSu 3 session whose config is mid-swap, so a reload here only re-parses a
...
if [[ ${OMISU_UPGRADE_TO_QUATTRO_LIVE:-0} != "1" ]]; then
