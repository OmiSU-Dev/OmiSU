...
snapper_config_script=/usr/share/omisu/install/config/snapper.sh
if [[ ! -f $snapper_config_script ]]; then
  snapper_config_script="$OMISU_PATH/install/config/snapper.sh"
fi
...
as_root env OMISU_PATH="$OMISU_PATH" bash -euo pipefail "$snapper_config_script"
