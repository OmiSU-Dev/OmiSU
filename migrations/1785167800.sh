...
# `omisu update` from a TTY does not have, so fall back to writing exactly the
# symlink it would have written rather than silently leaving this unenabled.
if ! systemctl --user enable omisu-fcitx5.service >/dev/null 2>&1; then
  wants_dir="$HOME/.config/systemd/user/graphical-session.target.wants"
  mkdir -p "$wants_dir"
  ln -sfn /usr/lib/systemd/user/omisu-fcitx5.service \
    "$wants_dir/omisu-fcitx5.service"
fi
...
if systemctl --user is-active --quiet graphical-session.target; then
  ...
  if ! error=$(systemctl --user start omisu-fcitx5.service 2>&1); then
    echo "Could not start omisu-fcitx5.service: $error"
