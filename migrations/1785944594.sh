...
limine_conf="${OMISU_T2_LIMINE_CONF:-/etc/limine-entry-tool.d/t2-mac.conf}"
fan_conf="${OMISU_T2_FAN_CONF:-/etc/t2fand.conf}"
running_cmdline="${OMISU_T2_RUNNING_CMDLINE:-/proc/cmdline}"
repair_marker="${OMISU_T2_REPAIR_MARKER:-/var/lib/omisu/migrations/1785944594}"
...
if omisu-pkg-present tiny-dfr; then
  sudo systemctl disable --now tiny-dfr.service || true
  omisu-pkg-drop tiny-dfr
fi
